...
*
* \brief   This provides services for SPI initialization for U5500
* \author  ST-Ericsson
*/
/*****************************************************************************/
/*--------------------------------------------------------------------------*
...
PUBLIC void SER_SPI_Init(t_uint8 mask)
{
    ...
    /*Enable SPI0, 1 ,2 ,3*/
	 (*(volatile t_uint32 *)(PRCC_5_CTRL_REG_BASE_ADDR)) |= SER_SPI_PRCC_ENABLE;
    /*Kernel Clock is not used */    	
	...
	switch(mask)
	{
		case INIT_SPI0:
            ...
            spi_base_address = SPI_0_CTRL_REG_BASE_ADDR;
            gic_line = GIC_SPI_0_LINE;
            interrupt_handler = SER_SPI0_InterruptHandler;

            gpio_config.mode =  GPIO_MODE_ALT_FUNCTION_A;
			/*From Pin 204-206 is CS so disable if not being used */
            gpio_pin = GPIO_PIN_204;
            gpio_error = GPIO_SetPinConfig(gpio_pin,gpio_config);
            if(gpio_error != GPIO_OK)PRINT("Error in GPIO Configuration");
            gpio_pin = GPIO_PIN_205;
            gpio_error = GPIO_SetPinConfig(gpio_pin,gpio_config);
            if(gpio_error != GPIO_OK)PRINT("Error in GPIO Configuration");
            gpio_pin = GPIO_PIN_206;
            gpio_error = GPIO_SetPinConfig(gpio_pin,gpio_config);
            if(gpio_error != GPIO_OK)PRINT("Error in GPIO Configuration");
            gpio_pin = GPIO_PIN_207;
            gpio_error = GPIO_SetPinConfig(gpio_pin,gpio_config);
            if(gpio_error != GPIO_OK)PRINT("Error in GPIO Configuration");
			gpio_pin = GPIO_PIN_208;
            gpio_error = GPIO_SetPinConfig(gpio_pin,gpio_config);
            if(gpio_error != GPIO_OK)PRINT("Error in GPIO Configuration");
		...
		case INIT_SPI1:
            ...
            spi_base_address = SPI_1_CTRL_REG_BASE_ADDR;
            gic_line = GIC_SPI_1_LINE;
            interrupt_handler = SER_SPI1_InterruptHandler;
            gpio_config.mode =  GPIO_MODE_ALT_FUNCTION_A;
            /*From Pin 165-167 is CS so disable if not being used */
            gpio_pin = GPIO_PIN_165;
            gpio_error = GPIO_SetPinConfig(gpio_pin,gpio_config);
            if(gpio_error != GPIO_OK)PRINT("Error in GPIO Configuration");
            gpio_pin = GPIO_PIN_166;
            gpio_error = GPIO_SetPinConfig(gpio_pin,gpio_config);
            if(gpio_error != GPIO_OK)PRINT("Error in GPIO Configuration");
            gpio_pin = GPIO_PIN_167;
            gpio_error = GPIO_SetPinConfig(gpio_pin,gpio_config);
            if(gpio_error != GPIO_OK)PRINT("Error in GPIO Configuration");
            gpio_pin = GPIO_PIN_168;
            gpio_error = GPIO_SetPinConfig(gpio_pin,gpio_config);
            if(gpio_error != GPIO_OK)PRINT("Error in GPIO Configuration");
			gpio_pin = GPIO_PIN_169;
            gpio_error = GPIO_SetPinConfig(gpio_pin,gpio_config);
            if(gpio_error != GPIO_OK)PRINT("Error in GPIO Configuration");
			gpio_pin = GPIO_PIN_170;
            gpio_error = GPIO_SetPinConfig(gpio_pin,gpio_config);
            if(gpio_error != GPIO_OK)PRINT("Error in GPIO Configuration");
        ...
        case INIT_SPI2:
            ...
            spi_base_address = SPI_2_CTRL_REG_BASE_ADDR;
            gic_line = GIC_SPI_2_LINE;
            interrupt_handler = SER_SPI2_InterruptHandler;
            
           // *(volatile t_uint32 *)(PRCMU_REG_BASE_ADDR + 0x138) |= 0x800000;/*PRCM_GPIOCR - not used in U5500*/
            gpio_config.mode =  GPIO_MODE_ALT_FUNCTION_A;
            /*From Pin 212-214 is CS so disable if not being used */
        ...
        case INIT_SPI3:
            ...
            spi_base_address = SPI_3_CTRL_REG_BASE_ADDR;
            gic_line = GIC_SPI_3_LINE;
            interrupt_handler = SER_SPI3_InterruptHandler;
            
            gpio_config.mode =  GPIO_MODE_ALT_FUNCTION_A;
            /*From Pin 185-187 is CS so disable if not being used */
            gpio_pin = GPIO_PIN_185;
            gpio_error = GPIO_SetPinConfig(gpio_pin,gpio_config);
            if(gpio_error != GPIO_OK)PRINT("Error in GPIO Configuration");
            gpio_pin = GPIO_PIN_186;
            gpio_error = GPIO_SetPinConfig(gpio_pin,gpio_config);
            if(gpio_error != GPIO_OK)PRINT("Error in GPIO Configuration");
            gpio_pin = GPIO_PIN_187;
            gpio_error = GPIO_SetPinConfig(gpio_pin,gpio_config);
            if(gpio_error != GPIO_OK)PRINT("Error in GPIO Configuration");
            gpio_pin = GPIO_PIN_188;
            gpio_error = GPIO_SetPinConfig(gpio_pin,gpio_config);
            if(gpio_error != GPIO_OK)PRINT("Error in GPIO Configuration");
			gpio_pin = GPIO_PIN_189;
            gpio_error = GPIO_SetPinConfig(gpio_pin,gpio_config);
            if(gpio_error != GPIO_OK)PRINT("Error in GPIO Configuration");
			gpio_pin = GPIO_PIN_190;
            gpio_error = GPIO_SetPinConfig(gpio_pin,gpio_config);
            if(gpio_error != GPIO_OK)PRINT("Error in GPIO Configuration");
			
			gpio_config.mode = GPIO_MODE_ALT_FUNCTION_B;
			/*From Pin 171-173 is CS so disable if not being used */
            gpio_pin = GPIO_PIN_185;
            gpio_error = GPIO_SetPinConfig(gpio_pin,gpio_config);
            if(gpio_error != GPIO_OK)PRINT("Error in GPIO Configuration");
            gpio_pin = GPIO_PIN_186;
