...
PUBLIC void SER_USB_Init( t_uint8 usb_ser_mask ) 
{
    ...
    t_gic_error     error_gic;
    t_uint8 data_out=0;
	t_uint8 data_in = 0xFF;
    
    /* Clock Enable for Peripheral2-USB */
    (*(volatile t_uint32 *)(PRCC_2_CTRL_REG_BASE_ADDR)) |= SER_USB_PRCC_AMBACLK_ENA;
	/* Enable the clock for the USB enumeration interrupt for USB device */
	(*(volatile t_uint32 *)(SER_USB_PRCM_CLKOCR2)) |= SER_USB_PRCM_CLKOCR2_ENA;
 ...
 /*------------------------------------4500 Settings for device mode---------------------------------*/
    ...
    {
        PRINT("SER_AB8500_CORE_Write(0x03, 0x2F, 0x01, &data_out) FAILED for data_out 0x01!!!");
        return;
    }    
   ...
   data_out = 0x04;
    if(SER_AB8500_CORE_OK != SER_AB8500_CORE_Write(0x03, 0x2F, 0x01,  &data_out))
    {
        PRINT("SER_AB8500_CORE_Write(0x03, 0x2F, 0x01,  &data_out) FAILED for data_out 0x03!!!.");
        return;
    }  
   ...
   data_out = 0x00;
    if(SER_AB8500_CORE_OK != SER_AB8500_CORE_Write(0x03, 0x2F, 0x01,  &data_out))
    {
        PRINT("SER_AB8500_CORE_Write(0x03, 0x2F, 0x01,  &data_out) FAILED for data_out 0x00!!!.");
        return;
     } 
#if 0 
	/* Detect Enable Test */
	data_out = 0x80;
	if(SER_AB8500_CORE_OK != SER_AB8500_CORE_Write(0x03, 0xF3, 0x01,  &data_out))
    {
        PRINT("SER_AB8500_CORE_Write(0x03, 0xF3, 0x01,data_out) FAILED for data_out 0x80!!!.");
        return;
     } 
	 if(SER_AB8500_CORE_OK != SER_AB8500_CORE_Write(0x03, 0xF4, 0x01,  &data_out))
    {
        PRINT("SER_AB8500_CORE_Write(0x03, 0xF4, 0x01,data_out) FAILED for data_out 0x00!!!.");
        return;
     }
#endif	 
  ...
  data_out = 0x04;
    ...
    {
        PRINT("SER_AB8500_CORE_Write(0x05, 0x87, 0x01,  &data_out) FAILED for data_out 0x04!!!.");
        return;
    }
  ...
  data_out = 0x03;
    if(SER_AB8500_CORE_OK != SER_AB8500_CORE_Write(0x05, 0x87, 0x01,  &data_out))
    {
        PRINT("SER_AB8500_CORE_Write(0x05, 0x87, 0x01,  &data_out) FAILED for data_out 0x03!!!.");
        return;
    }  

	data_out = 0x01;
	/*There is no Vbus enabling in AB5500, check up*/
	if(SER_AB8500_CORE_OK != SER_AB8500_CORE_Write(0x05, 0x8A, 0x01,  &data_out))
    {
        PRINT("SER_AB8500_CORE_Write(0x05, 0x8A, 0x01,  &data_out) FAILED for data_out 0x01!!!.");
        return;
    }  
	
	/*Enable the detection of the new accessory*/
	data_out = 0x01;
	if(SER_AB8500_CORE_OK != SER_AB8500_CORE_Write(0x05, 0x93, 0x01,  &data_out))
    {
        PRINT("SER_AB8500_CORE_Write(0x05, 0x93, 0x01,  &data_out) FAILED for data_out 0x01!!!.");
        return;
    }  	
#endif
	...
	/* USB Otg Control Settings to be done */
	data_out = 0x01;  /*Make this as 2 for Host Mode*/
	if(SER_AB8500_CORE_OK != SER_AB8500_CORE_Write(0x05, 0x87, 0x01,  &data_out))
    {
        PRINT("SER_AB8500_CORE_Write(0x05, 0x87, 0x01,data_out) FAILED for data_out 0x01!!!.");
        return;
    }
