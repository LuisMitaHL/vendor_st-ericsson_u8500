...
)
{
    ...
    {
        error = mmc_ReadBlocks(cardno, addr, p_readbuff, blocksize, no_of_blocks, mmc_device_id);
        if (MMC_OK != error)
        {
            DBGEXIT0(error);
            return(error);
...
)
{
    ...
    g_mmc_system_context[mmc_device_id].p_mmc_register->mmc_argument = (t_uint32) 4;
    g_mmc_system_context[mmc_device_id].p_mmc_register->mmc_command = MMCSET_BLOCKLEN |
        MMCRESPEXPECTED |
        MMCCMDPATHENABLE;
    error = mmc_CmdResp1Error(MMCSET_BLOCKLEN, mmc_device_id);
    ...
    g_mmc_system_context[mmc_device_id].current_card.blk_length = 4;
...
PRIVATE t_mmc_error mmc_testpin_write_fifo(t_mmc_device_id mmc_device_id)
{
    ...
    switch (mmc_device_id)
    {
        case MMC_DEVICE_ID_0:
            * (volatile t_uint32 *) (0xA0021084) = 0x0000AA55;
            break;

        case MMC_DEVICE_ID_1:
            * (volatile t_uint32 *) (0x80107084) = 0x0000AA55;
            break;

        case MMC_DEVICE_ID_2:
            * (volatile t_uint32 *) (0xA0022084) = 0x0000AA55;
            break;

        case MMC_DEVICE_ID_3:
            * (volatile t_uint32 *) (0x80108084) = 0x0000AA55;
            break;

        case MMC_DEVICE_ID_4:
            * (volatile t_uint32 *) (0x80109084) = 0x0000AA55;
            break;
...
)
{
    ...
    )
    {
        if (g_mmc_system_context[mmc_device_id].p_mmc_register->mmc_status & MMCTXFIFOEMPTY)
        {
            ...
            {
                case MMC_DEVICE_ID_0:
                    * (volatile t_uint32 *) (0xA0021084) = 0x0000005A;
                    break;

                case MMC_DEVICE_ID_1:
                    * (volatile t_uint32 *) (0x80107084) = 0x0000005A;
                    break;

                case MMC_DEVICE_ID_2:
                    * (volatile t_uint32 *) (0xA0022084) = 0x0000005A;
                    break;

                case MMC_DEVICE_ID_3:
                    * (volatile t_uint32 *) (0x80108084) = 0x0000005A;
                    break;

                case MMC_DEVICE_ID_4:
                    * (volatile t_uint32 *) (0x80109084) = 0x0000005A;
                    break;
