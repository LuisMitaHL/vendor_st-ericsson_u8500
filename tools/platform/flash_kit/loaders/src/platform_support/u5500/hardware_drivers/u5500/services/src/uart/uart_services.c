...
*
* \brief   This File provides service routines for UART for U5500
* \author  ST-Ericsson
*/
/*****************************************************************************/
...
/****************************************************************************/
/* NAME         :   SER_UART3_InterruptHandler                              */
/*--------------------------------------------------------------------------*/
/* DESCRIPTION  :   Interrupt handler implementation                        */
/*  PARAMETERS  :                                                           */
/*          IN  :   irq_src :   UART Interrupts                             */
...
PUBLIC void SER_UART3_InterruptHandler(IN t_uint32 irq_src)
{
    ...
    UART_GetIRQSrcStatus(UART3_DEVICE_ALL_IT, &uart_status);

    if (GIC_OK != GIC_DisableItLine(GIC_UART_3_LINE))
    {
        PRINT("\nError after GIC_DisableItLine in UART_DEVICE_ID_2 Handler");
        return;
    }

    if (GIC_OK != GIC_AcknowledgeItLine(GIC_UART_3_LINE))
    {
        PRINT("\nError after GIC_AcknowledgeItLine in UART_DEVICE_ID_2 Handler");
        return;
    }
...
PUBLIC void SER_UART3_InterruptHandler(IN t_uint32 irq_src)
{
    ...
    if (GIC_OK != GIC_EnableItLine(GIC_UART_3_LINE))
    {
        PRINT("\nError after GIC_EnableItLine in UART_DEVICE_ID_2 Handler");
        return;
    }
...
)
{
    ...
    ||  (UART_DEVICE_ID_3 == uart_device_id)
    )
    {
...
PUBLIC void SER_UART_Init(IN t_uint8 uart_device_id)
{
	...
	(*(volatile t_uint32 *)(SER_PRCM_UARTCLK_MGT_REG)) |= SER_UARTCLK_ENA;     /* UARTCLK  26 MHz (from SYSCLK) (clock enabled) */
    ...
    if (INIT_UART3 == (INIT_UART3 & uart_device_id))
    {
        /* Change The associated Interrupt Handler for UART 2 */
        gic_error = GIC_ChangeDatum(GIC_UART_3_LINE, SER_UART3_InterruptHandler, &old_datum);
        if (GIC_OK != gic_error)
        {
            PRINT("\nGIC error after GIC_ChangeDatum  :: %d", gic_error);
            return;
        ...
        gic_error = GIC_EnableItLine(GIC_UART_3_LINE);
        if (GIC_OK != gic_error)
        {
            PRINT("\nGIC error after GIC_EnableItLine  :: %d", gic_error);
            return;
    ...
    {
	/* Set PRCC Clocks */
        (*(volatile t_uint32 *)(PRCC_5_CTRL_REG_BASE_ADDR)) |= SER_UART_1_AMBA_KERNEL_CLK;
        (*(volatile t_uint32 *)(PRCC_5_CTRL_REG_BASE_ADDR + UART_PRCC_KCKEN_REG_OFFSET)) |= SER_UART_1_AMBA_KERNEL_CLK;
    ...
    {
	/* Set PRCC Clocks */
        (*(volatile t_uint32 *)(PRCC_5_CTRL_REG_BASE_ADDR)) |= SER_UART_2_AMBA_KERNEL_CLK;
        (*(volatile t_uint32 *)(PRCC_5_CTRL_REG_BASE_ADDR + UART_PRCC_KCKEN_REG_OFFSET)) |= SER_UART_2_AMBA_KERNEL_CLK;
    ...
    {
        /* Set PRCC Clocks */
        (*(volatile t_uint32 *)(PRCC_5_CTRL_REG_BASE_ADDR)) |= SER_UART_3_AMBA_KERNEL_CLK;
        (*(volatile t_uint32 *)(PRCC_5_CTRL_REG_BASE_ADDR + UART_PRCC_KCKEN_REG_OFFSET)) |= SER_UART_3_AMBA_KERNEL_CLK;
        
        UART_SetBaseAddress(UART_DEVICE_ID_3, UART_3_CTRL_REG_BASE_ADDR);
        if(UART_UNSUPPORTED_HW == UART_Init(UART_DEVICE_ID_3, UART_3_CTRL_REG_BASE_ADDR)) 
        {
            PRINT("UART_DEVICE_ID_3 Memory Map Failed");
            return;
        } 
    }
    ...
    if (INIT_UART0 == (INIT_UART0 & uart_device_id))
    {
        ...
        gpio_status = GPIO_EnableAltFunction(GPIO_ALT_UART0);
    ...
    if (INIT_UART1 == (INIT_UART1 & uart_device_id))
    {
        gpio_status = GPIO_EnableAltFunction(GPIO_ALT_UART1);
        if (GPIO_OK != gpio_status)
        {
    ...
    if (INIT_UART2 == (INIT_UART2 & uart_device_id))
    {
        gpio_status = GPIO_EnableAltFunction(GPIO_ALT_UART2);
        if (GPIO_OK != gpio_status)
        {
    ...
    if (INIT_UART3 == (INIT_UART3 & uart_device_id))
    {
        gpio_status = GPIO_EnableAltFunction(GPIO_ALT_UART3);
        if (GPIO_OK != gpio_status)
        {
            PRINT("Error: GPIO could not enable UART_DEVICE_ID_3!\n");
            PRINT("\nGPIO status from GPIO_EnableAltFunction  :: %d", gpio_status);
            return;
        }
    }   /* end if INIT_UART3 */
...
PUBLIC void SER_UART_Close(void)
{
    ...
    if (UART_OK != UART_Disable(UART_DEVICE_ID_3))
    {
        PRINT("Error: Could not Disable UART3!\n");
    }
}
