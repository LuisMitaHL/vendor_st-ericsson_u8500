...
/****************************************************************************/
/* NAME:    SER_MMC_5_InterruptHandler                              	            */
/*--------------------------------------------------------------------------*/
/* DESCRIPTION: This is interrupt service routine for MMC  	          		*/
/* 																            */
/* PARAMETERS: t_uint32 irq                                                 */
...
PRIVATE void SER_MMC_5_InterruptHandler(t_uint32  irq) 
{
	...
	gic_error = GIC_DisableItLine(GIC_SD_MMC_5_LINE);
	if (GIC_OK != gic_error)
	{
		PRINT("SER_MMC_InterruptHandler : GIC_DisableItLine() %d \n",gic_error);
		return;
	...
	do 
	{
		error = MMC_FilterProcessIRQSrc(&itstatus, &eventstatus, NO_FILTER_MODE,MMC_DEVICE_ID_5);
		done = (t_bool)FALSE;
		switch(error)
		{
			case 	MMC_NO_PENDING_EVENT_ERROR:
			...
			case 	MMC_REMAINING_PENDING_EVENTS:
					...
					MMC_AcknowledgeEvent(&eventstatus,MMC_DEVICE_ID_5);
					break;
  	...
  	gic_error = GIC_EnableItLine(GIC_SD_MMC_5_LINE);
  	if (GIC_OK != gic_error)
  	{
  		PRINT("SER_MMC_InterruptHandler : GIC_EnableItLine() %d \n",gic_error);
  		return;
...
PUBLIC void SER_MMC_Init( t_uint8 mmc_services_mask) 
{
	 ...
	 switch(mmc_services_mask)
	{
	 ...
	 case INIT_MMC2:
   		...
   		PRCC_PERI3_SDI2_BUS_CLKEN;
		PRCC_PERI3_SDI2_KBUS_CLKEN;
	 ...
	 case INIT_MMC5:
		 ...
		 PRCC_PERI3_SDI5_BUS_CLKEN;
		 PRCC_PERI3_SDI5_KBUS_CLKEN;

#if ((!defined (__PEPS_8500)) && (!defined (__PEPS_8500_V1)) && (!defined (__PEPS_8500_V2)))
         gpio_error = GPIO_EnableAltFunction(GPIO_ALT_MM_CARD5);
		 if (GPIO_OK != gpio_error)
		 {
			 PRINT("SER_MMC_Init(): GPIO_EnableAltFunction %d\n",gpio_error);
		 }
#endif	   
				/* Initialize the base address of MMC-SD */
				mmc_error = MMC_Init(SDI_5_REG_BASE_ADDR, MMC_DEVICE_ID_5);
				if (MMC_OK != mmc_error)
				{
					PRINT("SER_MMC_Init():MMC_Init() %d \n",mmc_error);
					return;
				...
				MMC_SetBaseAddress(SDI_5_REG_BASE_ADDR,MMC_DEVICE_ID_5);
				...
				gic_error = GIC_ChangeDatum(GIC_SD_MMC_5_LINE, SER_MMC_5_InterruptHandler, &old_datum);
				if (GIC_OK != gic_error)
				{
					PRINT("SER_MMC_Init(): GIC_ChangeDatum() %d\n",gic_error);
					return;
