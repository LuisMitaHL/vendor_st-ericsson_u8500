...
#define GPIO_MAXCALLBACKS 9

t_ser_gpio_param  g_gpio_param;
volatile t_callback g_gpio_callback[GPIO_MAXCALLBACKS]={0,0,0,0,0,0,0,0,0};
...
void SER_GPIO7_IntHandler(t_uint32 irq) 
{
	...
	for(irq_src = (t_uint32)GPIO_IRQ_SRC_224; irq_src <= (t_uint32)GPIO_IRQ_SRC_255 ;irq_src++)
	{
	    if (g_gpio_param.gpio_irq_param.gpio[irq_src] == TRUE)
	        GPIO_ClearIRQSrc((t_gpio_irq_src_id)irq_src);
	}
...
/****************************************************************************/
/* NAME:    SER_GPIO8_IntHandler	                                        */
/*--------------------------------------------------------------------------*/
/* DESCRIPTION: This is interrupt service routine for GPIO8 Block	     	*/
/* 																            */
/* PARAMETERS: unsigned int irq                                             */
/*                                                                          */
/* RETURN: None                                                             */
...
void SER_GPIO8_IntHandler(t_uint32 irq) 
{
	...
	GPIO_DisableIRQSrc((t_gpio_irq_src_id)irq);
 	gic_error = GIC_DisableItLine(GIC_GPIO_8_LINE);
 	if (GIC_OK != gic_error)
    {
        PRINT("Error During disable the GIC_GPIO_8_LINE \n");
        return;
    }
 	
 
	GPIO_GetMulptipleIRQStatus((t_uint32)GPIO_DEVICE_ID_8 , &(g_gpio_param.gpio_irq_param));

	if(g_gpio_callback[8].fct != NULL)
	{
		g_gpio_callback[8].fct(g_gpio_callback[7].param, &g_gpio_param );
				
	}


	for(irq_src = (t_uint32)GPIO_IRQ_SRC_256; irq_src <= (t_uint32)GPIO_IRQ_SRC_267 ;irq_src++)
	{
	    if (g_gpio_param.gpio_irq_param.gpio[irq_src] == TRUE)
	        GPIO_ClearIRQSrc((t_gpio_irq_src_id)irq_src);
	}
	...
	gic_error = GIC_EnableItLine(GIC_GPIO_8_LINE);
	if (GIC_OK != gic_error)
    {
        PRINT("Error During enable the GIC_GPIO_8_LINE \n");
        return;
    }
...
PUBLIC void SER_GPIO_Init( t_uint8 default_ser_mask ) 
{
    ...
    *(volatile t_uint32* )(PRCC_5_CTRL_REG_BASE_ADDR) = SER_GPIO_PRCC_CLK_ENABLE_5;		
	...
	/* DISABLE INTERRUPT LINE */
	if(GIC_OK!=GIC_DisableItLine(GIC_GPIO_8_LINE))
	{
		PRINT("error GIC_DisableItLine GIC_GPIO_8_LINE\n");
	}

	if(GIC_OK != GIC_ChangeDatum(GIC_GPIO_8_LINE, SER_GPIO8_IntHandler,&old_datum))
	{
		PRINT("error GIC_ChangeDatum GIC_GPIO_8_LINE \n");
	}
	
	/* ENABLE INTERRUPT LINE */
	if(GIC_OK!=GIC_EnableItLine(GIC_GPIO_8_LINE))
	{
		PRINT("error GIC_EnableItLine GIC_GPIO_8_LINE\n");
	}
	...
	if(GPIO_OK != GPIO_Init(GPIO_CTRL_ID_8,GPIO_8_REG_BASE_ADDR))
	{
		PRINT("error GPIO_Init for ctrl id 8 \n");
	}
	...
	GPIO_SetBaseAddress(GPIO_CTRL_ID_8,GPIO_8_REG_BASE_ADDR);
