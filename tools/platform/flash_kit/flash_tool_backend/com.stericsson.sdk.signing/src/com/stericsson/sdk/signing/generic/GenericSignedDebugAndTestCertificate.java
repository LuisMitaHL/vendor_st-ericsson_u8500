...
/**
 * Signed U5500 Debug & Test Certificate
 * 
 * @author xolabju
 * 
 */
public class GenericSignedDebugAndTestCertificate implements IGenericSignablePayload {
     ...
     *             on errors
     */
    public GenericSignedDebugAndTestCertificate(GenericSignerSettings signerSettings) throws SignerException {
        ...
        GenericDTCertificate dtCert;//new GenericDTCertificate();
        try {
            ByteBuffer content = readPayloadContent();
            byte[] contentArray = content.array();
            int magic = getMagic(contentArray);
            if (magic == GenericCertificate.MAGIC_DT_CERTIFICATE) {
                dtCert = new GenericDTCertificate();
            } else if (magic == GenericCertificate.MAGIC_U8500_DT_CERTIFICATE) {
                dtCert = new U8500DTCertificate();
            } else {
                throw new SignerException("The D&T certificate is invalid (unexpected magic number).");
            }
            dtCert.setData(contentArray);
            if (dtCert.getSignature().length != 0) {
                logger.info("Removing pre-existing signature");
                dtCert.setSignatureSize((short) 0);
                dtCert.setSignature(new byte[0]);
            }
        } catch (Exception e) {
            throw new SignerException("Failed to read D&T certificate content: " + e.getMessage());
        }
        certificate = dtCert;
    }

    private int getMagic(byte[] contentArray) throws SignerException {
        if (contentArray.length < 4) {
            throw new SignerException("The D&T certificate is invalid (unexpected magic number).");
        }
        byte[] magicArray = new byte[4];
        System.arraycopy(contentArray, 0, magicArray, 0, magicArray.length);

        ByteBuffer bb = ByteBuffer.wrap(magicArray);
