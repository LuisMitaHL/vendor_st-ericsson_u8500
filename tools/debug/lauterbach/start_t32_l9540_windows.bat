...
SET CONFIGPATH=%SCRIPTPATH%\ux500\9540
...
TYPE %CONFIGPATH%\config_sva.t32 | findstr /v LICENSE > %CONFIGPATH%\config_sva.tmp
MOVE %CONFIGPATH%\config_sva.tmp %CONFIGPATH%\config_sva.t32
...
echo LICENSE=%T32LICENSE% >> %CONFIGPATH%\config_sva.t32
:End
...
IF %choice% EQU 1 (
  ECHO "Debugging: [9540_APP]"
  SET APP=1
)

IF %choice% EQU 2 (
  ECHO "Debugging: [9540_APP+SIA]"
  SET APP=1
  SET SIA=1
)

IF %choice% EQU 3 (
  ECHO "Debugging: [9540_APP+SVA]"
  SET APP=1
  SET SVA=1
)
...
IF DEFINED APP start /Dc:\ %T32BIN%\t32marm.exe -c %CONFIGPATH%\config_app%SIM%.t32 -s "%SCRIPTPATH%\init.cmm [9540_APP]"

IF DEFINED SIA (
ping -n 8 localhost > nul
start /Dc:\ %T32BIN%\t32mmdsp.exe -c %CONFIGPATH%\config_sia%SIM%.t32 -s "%SCRIPTPATH%\init.cmm [9540_MMDSP]"
)

IF DEFINED SVA (
ping -n 8 localhost > nul
start /Dc:\ %T32BIN%\t32mmdsp.exe -c %CONFIGPATH%\config_sva%SIM%.t32 -s "%SCRIPTPATH%\init.cmm [9540_MMDSP]"
)
