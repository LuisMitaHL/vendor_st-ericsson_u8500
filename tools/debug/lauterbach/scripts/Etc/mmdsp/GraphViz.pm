...
the initial layout to suit in the given size. This achieves a tighter packing
of nodes but reduces the balance and symmetry. This feature only works in dot.
...
sub new {
  ...
  $self->{NODESEP} = $config->{nodesep} if (exists $config->{nodesep});
...
sub _as_debug {
  ...
  # { $name{$b} cmp $name{$a} }
  foreach my $clustername (sort {$clusters{$a}{name} cmp $clusters{$b}{name}} keys %cluster_nodes) {
    ...
    #Deal with ranks inside same cluster
    my %ranks;
    foreach my $name (@{$cluster_nodes{$cluster}}) {
	my $node = $self->{NODES}->{$name};
	next unless exists $node->{rank_cluster};
	push @{$ranks{$node->{rank_cluster}}}, $name;
    }
...
sub _attributes {
  ...
  foreach my $key (keys %$thing) {
    next if $key =~ /^_/;
    next if $key =~ /^(to|from|name|cluster|from_port|to_port|rank_cluster)$/;
