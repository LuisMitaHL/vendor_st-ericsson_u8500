 ...
 *
 * \file mmc_mgmt.h
 * \brief ROM code's Flash memory driver
 * \author STMicroelectronics
 *
 * \addtogroup MMC
 *
 * This module is the MMC driver header file
 *
 * @{
 */
/*---------------------------------------------------------------------------*/
...
/** SDI (MMC/SD interface) choice according to BSV/Antifuse MMC interface selection */
typedef enum {
    MMC2_MMC4=0, /**< (0) Search must be done on SDI2 and if fail on SDI4 */
    MMC2=1,      /**< (1) Search must be done on SDI2 only */
    MMC4=2       /**< (2) Search must be done on SDI4 only */
}t_MmcPosition;
...
PUBLIC t_boot_error MMC_Close(t_MmcNb mmcNb);
