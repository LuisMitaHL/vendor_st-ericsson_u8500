...
PUBLIC t_boot_rom_bridge_func hw_sec_rom_pub_bridge;
...
#if (__STN_9540 > 0)
    { 0xFFFFDBF4,       0x008540A0,     0xFFFF7588,         0xFFFF7300,     0x8540A0 },
...
/* REENTRANCY: NA                                                           */
/****************************************************************************/
PUBLIC void SECURITY_Init( t_logical_address ROMBaseAddress )
{
...
#else
            (NomadikCutDescTab[CutNb].ChecksumValue != * (t_uint32 *)((NomadikCutDescTab[CutNb].ChecksumAddr - BOOT_PHYSICAL_BASE_ADDR) + ROMBaseAddress))
#endif
         )
    {
        CutNb++;
...
#else
        hw_sec_rom_pub_bridge       = (t_boot_rom_bridge_func)   ((NomadikCutDescTab[CutNb].BridgeFunc     - BOOT_PHYSICAL_BASE_ADDR) + ROMBaseAddress);
        hw_mmu_physical_address_get = (t_boot_rom_phys_addr_func)((NomadikCutDescTab[CutNb].ConversionFunc - BOOT_PHYSICAL_BASE_ADDR) + ROMBaseAddress);
#endif
    }
}
...
#if (defined(__ARMCC_VERSION) || defined(__GNUC__) || defined(__ARMV4I_WE))
