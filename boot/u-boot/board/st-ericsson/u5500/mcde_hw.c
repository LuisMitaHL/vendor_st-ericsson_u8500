...
static void update_dpi_registers(enum mcde_chnl chnl_id, struct tv_regs *regs)
{
	...
	if (!regs->sel_mode_tv ||
			hardware_version == MCDE_CHIP_VERSION_3_0_8 || hardware_version == MCDE_CHIP_VERSION_4_0_4) {
...
static int update_channel_static_registers(struct mcde_chnl_state *chnl)
{
	...
	debug("%s: Entering 5500 hardware version %d fifo = %d", __func__, hardware_version, chnl->fifo);
...
int mcde_dsi_dcs_write(struct mcde_port *port, int dcs, u8 cmd, u8* data, int len)
{
	int i;
	u32 wrdat[4] = { 0, 0, 0, 0 };
	u32 settings;
...
#define PRCM_PLLDSI_LOCKP	(U5500_PRCMU_BASE + 0x508)
...
#define PRCMU_ENABLE_ESCAPE_CLOCK	0x03000201
...
int mcde_probe(u8 num_dsilinks, struct mcde_platform_data *pdata)
{
	...
	if (major_version == 3 && minor_version == 0 &&
		...
		dev_info(&mcde_dev->dev, " 5500 V2 HW\n");
	} else {
