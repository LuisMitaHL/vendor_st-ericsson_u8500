...
/* PLLDIV=12, PLLSW=4 */
#define PRCMU_DSI_CLOCK_SETTING		0x0000018C
...
#if defined(BOE_1024_600_LCD)
/* D=74, N=1, R=4, SELDIV2=0 dsi=307.84*/
#define PRCMU_PLLDSI_FREQ_SETTING	0x0004014a
#else
/* D=101, N=1, R=4, SELDIV2=0 */
#define PRCMU_PLLDSI_FREQ_SETTING	0x00040165
#endif
#define PRCMU_ENABLE_PLLDSI		0x00000001
#define PRCMU_DSI_PLLOUT_SEL_SETTING	0x00000201
#else
/* D=101, N=1, R=4, SELDIV2=0 */
#define PRCMU_PLLDSI_FREQ_SETTING	0x00040165
#define PRCMU_ENABLE_PLLDSI		0x00000001
...
static int update_channel_static_registers(struct mcde_chnl_state *chnl)
{
	...
	if (hardware_version == MCDE_CHIP_VERSION_3_0_5) {
		...
		case MCDE_FIFO_A:
			...
			if (port->type == MCDE_PORTTYPE_DPI) {
				...
				mcde_wfld(MCDE_CTRLA, FIFOWTRMRK,480);
#else                                        
	...
	//joe debug
	if (port->type == MCDE_PORTTYPE_DSI) {
		writel(PRCMU_CLK_EN, PRCM_LCDCLK_MGT);
	}
...
#if defined(CONFIG_BOARD_SNDA_S1) && !defined(CONFIG_BOARD_SNDA_P0)
	ljinc = -ljinc;
				...
				struct mcde_video_mode *video_mode)
{
	...
	if (port->type == MCDE_PORTTYPE_DSI) {
		...
		if (port->mode == MCDE_PORTMODE_VID) {
			/* In video mode, a scanline must be sent as
			 * one packet (pkt_div = 1).
			 * Also, no overhead data compared to command
			 * mode so the packet sizes is just ppl x bpp / 8 */
			pkt_div = 1;
...
/* DSI */
int mcde_dsi_generic_write(struct mcde_port *port, u8 cmd, u8* data, int len)
{
	int i;
	u32 wrdat[4] = { 0, 0, 0, 0 };
	u32 settings;
...
int mcde_dsi_dcs_read
	(struct mcde_port *port, u8 cmd, u8 *data, int len)
{
	int ret = 0;
	u8 link = port->link;
	u8 virt_id = port->phy.dsi.virt_id;
	...
	do {
		...
		while (wait-- &&!(error = dsi_rfld(link, DSI_DIRECT_CMD_STS,
			...
			!(ok = dsi_rfld(link, DSI_DIRECT_CMD_STS, READ_COMPLETED)))
	...
	if (ok) {
		...
		dev_warn(&mcde_dev->dev, "DCS read failed, err=%d, sts=%X\n",
...
{
	struct mcde_ovly_state *ovly = NULL;
