...
fat_register_device(block_dev_desc_t *dev_desc, int part_no)
{
	...
	int emmcpart7offset;
	char *emmcpart7offset_env;

	emmcpart7offset = 0;
	emmcpart7offset_env = getenv("emmcpart7offset");
	if (emmcpart7offset_env != NULL)
		emmcpart7offset = (int)simple_strtoul(emmcpart7offset_env, NULL, 10);

	if (!dev_desc->block_read) {
		printf("=== block_read is null\n");
		return -1;
	}
#if defined(CONFIG_ROCKBOX_FAT)
	          ...
	          ) 
	{
		...
		part_offset = 0;
#endif
	} else if (part_no==CONFIG_SD_FAT_PART_NUM && emmcpart7offset > 0) {

#if defined(CONFIG_ROCKBOX_FAT)
		cur_block_dev.cur_part = part_no;
		cur_block_dev.part_offset = emmcpart7offset;
#else
		cur_part = part_no;
		part_offset = emmcpart7offset;
#endif
	} else {
