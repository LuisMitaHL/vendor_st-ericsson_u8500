...
static void toc_recalc_entry_offsets(toc_entry_t *toc)
{
	...
	for (i = 0; i < TOC_MAX_ENTRIES; i++, toc++) {
		...
		if ((toc->flags & 0x0000ffff) == FLAGS_LBA)
			offset_sectors = toc->offset;
		else
			offset_sectors = toc->offset / SUPPORTED_SECTOR_SIZE;

		if (offset_sectors >= TOC_BOOT_IMAGE_SIZE) {
			debug("Recalculating offset on TOCEntry: %s\n",
			      toc->id);
			if ((toc->flags & 0x0000ffff) == FLAGS_LBA)
				toc->offset += toc_location;
			else
				toc->offset += (toc_location *
						SUPPORTED_SECTOR_SIZE);
			debug("recalc offset 0x%08x, flags 0x%08x\n",
						toc->offset, toc->flags);
		}
	}
}
	...
	if (toc_init(dev_desc, &toc_main_toc[0]) == 0) {
		...
		if (toc_entry != NULL) {
			...
			*loadaddr = toc_entry->loadaddr;
			...
			/* Calc entry_offset as blocks */
			if ((toc_entry->flags & 0x0000ffff) != FLAGS_LBA) {
				if (toc_entry->offset % dev_desc->blksz) {
					printf("TOC Entry: %s, offset 0x%08x "
						"not evenly divisible by "
						"0x%08x\n",
						toc_id, toc_entry->offset,
						(u32) dev_desc->blksz);
					return 1;
				}
				*offset = toc_entry->offset / dev_desc->blksz;
			} else {
				/* The returned size is still in bytes */
				if (toc_entry->size * dev_desc->blksz >
								0xffffffffU)
					printf("Size of TOC entry too big: "
						"0x%llx",
						(u64) toc_entry->size *
						dev_desc->blksz);
				else
			...
			debug("get_entry_info_toc offset 0x%08x\n", *offset);
		       ...
		       u32 offset, u32 size, u32 loadaddr)
{
	...
	entry_offset += offset / dev_desc->blksz;
