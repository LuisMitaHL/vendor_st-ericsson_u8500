...
static void gpio_init(void)
{
	...
	case OPTION_UART_GPIOPINS_18_19:
		...
		break;
	case OPTION_UART_GPIOPINS_120_121:
		/* Enable UART2 on GPIO120+121 (alt mod A) */
		setbits32(GPIO3_BASE + GPIO_AFSLA, (1 << 25) | (1 << 24));
		break;
	}
...
u32 ddr_test(void)
{
	 ...
	 */
	memsize=memsize0;
	if (memsize>AP9540_DDR0_SIZE) 
	{
		memsize=AP9540_DDR0_SIZE;
	}
	res = memtest_main((volatile datum_t *)AP9540_DDR0_OFFSET, memsize);
	loginfo("DDR0 test passed (size=%x, Result=%u).", memsize, res);

	memsize=memsize1;
	if (memsize>AP9540_DDR1_SIZE) 
	{
		memsize=AP9540_DDR1_SIZE;
	}
	res |= memtest_main((volatile datum_t *)AP9540_DDR1_OFFSET, memsize);
	loginfo("DDR1 test passed (size=%x, Result=%u).", memsize, res);
...
static bass_return_code configure_lvl0_firewalling(u32 *lvl0_config)
{
  ...
  const struct bass_imad_entry lvl0_firewall_ie[] = {
    { IMAD_SEQUENCE_WRITE_LVL0_FIREWALL, (u32)0},
  ...
  /* With respect to ICOS, here is the sequence to set Firewall */
  /* 1. PRCM_APE_MEM_REQ set to '1' */
  ((t_prcmu_hw_registers*)PRCMU_REG_START_ADDR)->prcm_ape_mem_req = 0x1;
  /* 2. XMIP_MEM_REQ set to '1' */
  *((u32*)(PRCMU_REG_START_ADDR + 0x2038)) = 0x2;
  /* 3. DMC_CTL_73 bit SECR_ARNG_DDRHOLD set to '1' */
  ((t_dmc_registers*)SDMC_REG_START_ADDR)->Ctl_73.ddr_hold = 0x1;
  /* 4. Configure Firewalling registers */
  return_code = bass_imad_exec(lvl0_firewall_ie,
			       sizeof(lvl0_firewall_ie)/sizeof(struct bass_imad_entry),
			       &faulty_entry);
  /* 5. DMC_CTL_73 bit SECR_ARNG_DDRHOLD set to '0' */
  ((t_dmc_registers*)SDMC_REG_START_ADDR)->Ctl_73.ddr_hold = 0x0;
...
//	u8 ddr_regs[DDR_REGISTERS_LEN * 2];
	 ...
	 */
	head.bin = NULL;
	ret = next_section(bin, bin_len, SECTION_TYPE_AB8540, &head);
	if (ret != BOOT_OK || head.bin == NULL) {
		logerror("Failed to find AB8540 section");
		return BOOT_INTERNAL_ERROR;
	}
	if (head.len != AB8540_SETTINGS_LEN) {
		logerror("AB8540 section bad");
		return BOOT_INTERNAL_ERROR;
	}

	/* Initialize some ABx500 registers */
	if (AB8540_init_fct((u8*)head.bin))
	{
		logerror("AB8540 init failed");
		return BOOT_INTERNAL_ERROR;
	}
	logdebug("AB8540 Init done");
...
#ifndef IMAD_CONFIG_READ_FUSES
	memcpy((u8*)&init_opp_data, (u8*)head.bin, sizeof(sPrcmuApi_InitOppData_t));
