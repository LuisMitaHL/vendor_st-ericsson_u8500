...
static u8 avs_payload_overclock[] = {
	...
	0x00, 0xDB, 0xDB, 0xDB, 0xF4, 0x2C, 0x1A, 0x04,
	0x2A, 0x1C, 0x22, 0x18, 0x28, 0x14, 0x00, 0xFF,
...
static u8 avs_payload[] = {
	/* Generic default values */
	0x00, 0xDB, 0xDB, 0xDB, 0x34, 0x2C, 0x1A, 0x04,
...
void avs8500_calculate_settings(u32 avs_fuses[2], void *avs_data)
{
	u32 FVR21_data;
	u32 FVR22_data;
	...
	volt_stack_arm_opp0 = (125.0/100.0)*(vdrop_arm_opp1 + \
	...
	volt_stack_arm_opp2 = (50.0/100.0)*(vdrop_arm_opp1 + \
		smps_transient_arm) + smps_accuracy + temp_aging;
	volt_stack_ape_opp1 = vdrop_ape_opp1 + smps_transient_ape + \
		smps_accuracy + temp_aging + pcb_drop;
	...
	/* Check AVS enhancement (Vsafe fused value not aligned with AVS
	   signoff for enhancement 00 and 01) */
	if ((avs_enhance == 0)||(avs_enhance == 1))
		volt_stack_safe_opp1 += vsafe_stack_correction;
...
int avs8500_is_maxopp_fused(u32 avs_fuses[2])
{
...
void avs8500_no_avs_calculate_settings(u32 avs_fuses[2], void *avs_data)
{
	unsigned char BB_rv;
	unsigned n;
	u8 *avd = avs_data;
