...
#define avs_enhancement_mask   0xF
...
#define vbbr_max                   100
#define vsmps_arm_max_boost        1.39375F
...
#define varm_2step_correction      25   /* 25 mV */
...
static u8 avs_payload_v1_overclock[] = {
	...
	0x00, 0x7E, 0x7E, 0x7E, 0xFF, 0x68, 0x44, 0x18,
...
static u8 avs_payload_overclock[] = {
	/* Generic default values */
	0x00, 0xFF, 0xFF, 0xFF, 0xFF, 0x68, 0x44, 0x18,
	0x34, 0x30, 0x22, 0x18, 0x28, 0x14, 0x00, 0xFF,
	/* Non fused parameters needed to compute AVS regulators */
	0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
	0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
...
static u8 avs_payload_v1[] = {
	/* Generic default values */
	0x00, 0x7E, 0x7E, 0x7E, 0x7F, 0x68, 0x44, 0x18,
...
static u8 avs_payload[] = {
	/* Generic default values */
	0x00, 0xFF, 0xFF, 0xFF, 0x7F, 0x68, 0x44, 0x18,
	0x22, 0x16, 0x22, 0x18, 0x28, 0x14, 0x00, 0xFF,
	/* Non fused parameters needed to compute AVS regulators */
	0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
	0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
...
void avs8520_calculate_settings(u32 avs_fuses[2], void *avs_data)
{
	u32 FVR21_data;
	u32 FVR22_data;
	...
	if (get_cut_id() == CUT_ID_AB8500_V1) {
		...
		if (vbbp_400 == 1) {
			loginfo("########################################################");
			loginfo("#     This chip needs a configuration that is NOT      #");
			loginfo("#           supported by AB8505 V1 chip.               #");
			loginfo("# The platform can be unstable when running at 1.15GHz #");
			loginfo("########################################################");

			vbbp_reg = 0x7;
			varm_correction = 1;
			/* => VBBP = VDDCPU - 350mV (FBB) */
			/* (not possible to have FBB 400mV with AB8505) */
		} else {
	...
	volt_stack_arm_opp0 = vdrop_arm_opp1_boost + \
		...
		varm_correction*varm_2step_correction;
	...
	volt_stack_arm_opp2 = (50.0/100.0)*(vdrop_arm_opp1 + \
		smps_transient_arm) + smps_accuracy + temp_aging;
	volt_stack_ape_opp1 = vdrop_ape_opp1_boost + \
		smps_transient_ape_boost + smps_accuracy + temp_aging + pcb_drop_boost;
...
int avs8520_is_maxopp_fused(u32 avs_fuses[2])
{
		...
		if (get_cut_id() == CUT_ID_AB8500_V1) {
			...
			if (vbbp_400 == 1) {
				...
				for (i = 0; i < delay_loop; i++)
					cpu_relax();
				loginfo("It will now continue to boot.....");
				for (i = 0; i < delay_loop; i++)
					cpu_relax();
...
void avs8520_no_avs_calculate_settings(u32 avs_fuses[2], void *avs_data)
{
	unsigned char BB_rv;
	unsigned n;
	u8 *avd = avs_data;
