                    ...
                    const size_t speed_data_len)
{
    ...
    /* ER 424735 : Remove DDR1_GFX_MIN_BW constraint */
    *(uint32_t*)(0x80159450) = 0x2; /* DDR1_GFX_MIN_BW = 25% */
...
bass_return_code bass_imad_store(const uint8_t *data, size_t data_len)
{
   ...
   op.memRefs[0].flags = TEEC_MEM_INPUT;
   return tee_invoke_command(BASS_APP_IMAD_STORE, &op);
}
...
bass_return_code bass_imad_release(void)
{
   TEEC_Operation op;
   memset(&op, 0, sizeof(op));
   return tee_invoke_command(BASS_APP_IMAD_RELEASE, &op);
}
           ...
           size_t num_entries, size_t *failed_entry)
{
    ...
    return tee_invoke_command (BASS_APP_IMAD_EXEC, &op);
}
