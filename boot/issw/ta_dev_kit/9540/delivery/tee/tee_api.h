...
TEE_Result TEE_OpenTASession(const TEE_UUID* destination,
                ...
                TEE_Param params[4], TEE_TASessionHandle* session,
...
TEE_Result TEE_AllocatePersistentObjectEnumerator(
                TEE_PropSetHandle *objectEnumerator);

void TEE_FreePersistentObjectEnumerator(TEE_PropSetHandle objectEnumerator);

void TEE_ResetPersistentObjectEnumerator(TEE_PropSetHandle objectEnumerator);

TEE_Result TEE_StartPersistentObjectEnumerator(
                TEE_PropSetHandle objectEnumerator, uint32_t storageID);

TEE_Result TEE_GetNextPersistentObject(TEE_PropSetHandle objectEnumerator,
...
TEE_Result TEE_MACFinalCompute(TEE_OperationHandle operation,
                const void *message, size_t messageLen, void *mac,
                size_t *macLen);

TEE_Result TEE_MACFinalCompare(TEE_OperationHandle operation,
                const void *message, size_t messageLen, const void *mac,
                size_t *macLen);
...
TEE_Result TEE_AEDecryptFinal(TEE_OperationHandle operation,
                const void *srcData, size_t srcLen, void *destData,
                size_t *destLen, const void *tag, size_t tagLen);
...
TEE_Result TEE_BigIntConvertFromOctetString(TEE_BigInt *dest,
                const uint8_t *buffer, size_t bufferLen, int32_t sign);
...
void TEE_BigIntComputeExtendedGcd(TEE_BigInt *gcd, TEE_BigInt *u,
                TEE_BigInt *v, const TEE_BigInt *op1, const TEE_BigInt *op2);
