...
uper_open_type_get_simple(asn_codec_ctx_t *ctx, asn_TYPE_descriptor_t *td,
	...
	if(rv.code == RC_OK) {
		...
		/*If the data type is NULL; no data will be decoded from the buffer
		which means that all data in the buffer will be considered as padding.
		This leads to 8 bit padding */
