...
/* start STE */
	...
	el_found = (asn_INTEGER_enum_map_t *)enum_bsearch(&key,
		specs->value2enum, count, sizeof(specs->value2enum[0]),
		INTEGER__compar_enum2value);
/* end STE */
	if(el_found) {
...
/* start STE */
	...
	return (asn_INTEGER_enum_map_t *)enum_bsearch(&value, specs->value2enum,
		count, sizeof(specs->value2enum[0]),
		INTEGER__compar_value2enum);
/* end STE */
}
...
INTEGER__xer_body_decode(asn_TYPE_descriptor_t *td, void *sptr, const void *chunk_buf, size_t chunk_size) {
	...
	for(value = 0, lp = lstart; lp < lstop; lp++) {
        ...
        /* coverity[dead_error_condition] TODO */
				if(sign == -1
				&& (unsigned long)value
						== ~((unsigned long)-1 >> 1)) {
					sign = 1;
...
/* start STE */
void *
enum_bsearch(const void * key, const void * base, size_t num, size_t size, int (* comparator) (const void*, const void*))
{
	asn_INTEGER_enum_map_t * list = (asn_INTEGER_enum_map_t*)base;
	int eidx = 0;
	void * ret = 0;

	ret = bsearch(key, base, num, size, comparator);

	if(ret) {
		return ret;
	}

	for(eidx = 0; eidx < num; eidx++) {
		if(comparator(key, &(list[eidx])) == 0) {
			return &(list[eidx]);
		}
	}
