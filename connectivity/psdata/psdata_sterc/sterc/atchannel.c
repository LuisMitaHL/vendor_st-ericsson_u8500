...
static struct atcontext *getAtContext() {
    ...
    if ((ac = pthread_getspecific(key)) == NULL) {
        if (s_defaultAtContext) {
            ALOGE("WARNING! external thread use default AT Context");
...
void AT_DUMP(const char *prefix, const char *buff, int len)
{
    ...
    for (i = 0; i < len; i++) {
        t += sprintf(t, "%02x ", buff[i]);
    }

    ALOGD("%s(%d): %s (\"%.*s\")", prefix, len, s, len, buff);
}
#endif
...
static void processLine(const char *line)
{
...
#ifdef RIL_MODULE_TEST_MODE_ENABLED
    pthread_mutex_trylock(&ac->commandmutex);
#else
        ...
        /* No command pending. */
    {
        ...
        default:               /* This should never be reached */
            ALOGE("Unsupported AT command type %d", ac->type);
            handleUnsolicited(line);
            break;
        }
...
static int writeline(const char *s)
{
    ...
    if (err < 0) {
        ALOGE("%s(): Failed to append \\r to string.", __func__);
    ...
    if (ac->fd < 0 || ac->readerClosed > 0) {
        ALOGE("Attempt to write to the closed AT channel.");
        ret = AT_ERROR_CHANNEL_CLOSED;
        goto error;
    }

    ALOGD("AT(%d)> %s", ac->fd, p_s);
    ...
    while (cur < len) {
        ...
        if (written < 0) {
            ALOGE("Error writing to the AT channel: %ld", (long)written);
            ret = AT_ERROR_GENERIC;
            goto error;
        }
...
static int writeCtrlZ(const char *s)
{
    ...
    if (err < 0) {
        ALOGE("%s(): Failed to append ^Z to string.", __func__);
        ret = AT_ERROR_GENERIC;
        goto exit;
    }
                                       ...
                                       ATResponse **pp_outResponse)
{
    ...
    /**
     * Initialize *pp_outResponse to NULL to make sure the
     * core-ril doesn't free uninitialized pointer.
     */
    if (pp_outResponse) {
        *pp_outResponse = NULL;
    }
...
void at_send_escape(void)
{
    ...
    ALOGI("%s() sent space on at channel to abort command", __func__);
}
