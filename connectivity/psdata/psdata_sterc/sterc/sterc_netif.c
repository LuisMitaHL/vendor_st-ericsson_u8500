...
static int parse_rtnl_message(uint8_t *buf, size_t len, struct iplink_req *req)
{
    ...
    while (len > 0) {
        ...
        if (hdr->nlmsg_type == NLMSG_ERROR) {
            ...
            if (err->error)
                STERC_DBG_TRACE(LOG_WARNING, "%s(): RTNL failed: seq:%d, error %d(%s)\n", __func__,
...
static int netlink_get_response(int sk, struct iplink_req *req)
{
    ...
    do {
         ...
         */
        if (ret == 0) {
            STERC_DBG_TRACE(LOG_WARNING, "EOF received.\n");
            errno = EIO;
            ret = -1;
            break;
        }
                          ...
                          const void *data, int datalen)
{
    ...
    if ((int)(NLMSG_ALIGN(n->nlmsg_len) + RTA_ALIGN(len)) > maxlen) {
        STERC_DBG_TRACE(LOG_WARNING, "%s(): attribute too large for message. nlmsg_len:%d, len:%d, \
                                 ...
                                 int nsapi, char loop_enabled)
{
    ...
    if (connection_type == IFLA_CAIF_IPV4_CONNID ||
        ...
        STERC_DBG_TRACE(LOG_WARNING, "%s(): Unsupported linktype.\n", __func__);
        errno = EINVAL;
        return -1;
    }
                               ...
                               int *ifindex, char loop)
{
    ...
    sk = rtnl_init(RTMGRP_LINK | RTMGRP_IPV4_IFADDR | RTMGRP_IPV4_ROUTE);
    ret = sk;
