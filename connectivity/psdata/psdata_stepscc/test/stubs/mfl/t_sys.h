 ...
 *
 * \brief   Header file containing type definitions required for SwBP
 *
 */
...
{
  GS_CLIENT_TAG_OK,
  GS_CLIENT_TAG_FAILED_UNPACKING,
  GS_CLIENT_TAG_FAILED_PACKING,
  GS_CLIENT_TAG_NOT_SUPPORTED
} ENUM8(ClientTagStatus_t);
...
TYPEDEF_ENUM
{
  ...
  REQUEST_FAILED,
  REQUEST_FAILED_RESOURCE,
  REQUEST_TIME_OUT,
  REQUEST_FAILED_PARAMETER,
  REQUEST_FAILED_APPLICATION,
  REQUEST_FAILED_UNPACKING
} ENUM8(RequestStatus_t);
...
TYPEDEF_ENUM
{
  ...
  GS_EVENT_FAILED_APPLICATION,
  GS_EVENT_FAILED_PARAMETER,
  GS_EVENT_NOT_AVAILABLE
} ENUM8(EventStatus_t);
...
typedef void (*GS_Handle_t)(const union SIGNAL** const Signal_pp);
