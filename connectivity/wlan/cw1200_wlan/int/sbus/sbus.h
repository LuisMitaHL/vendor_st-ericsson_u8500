...
#include "../umac_lib/mod_umi/export/UMI_Api.h"
...
*MACRO definitions
***************************************************************************/
#define SPI_REG_ADDR_TO_SDIO(spi_reg_addr) ((spi_reg_addr)*4)
		/*Sdio addr is 4*spi_addr*/

#define SDIO_ADDR17BIT(buf_num, mpf, rfu, reg_id_ofs) \
					(((buf_num	& 0x1F)<<7)\
					|  ((mpf		& 1)<<6)  \
					|  ((rfu		& 1)<<5)  \
					|  ((reg_id_ofs & 0x1f)<<0)	\
...
#define SDIO_READ						1
#define SDIO_WRITE						2
#define STLC9000_FN						1
	/*WLAN Function Number */
#define BIT_16_REG 						2   /*2 Bytes */
#define BIT_32_REG						4
#define DOWNLOAD_BLOCK_SIZE		  (2048)
...
/*one SPI message cannot be bigger than (2"12-1)*2 bytes
* // "*2" to cvt to bytes*/
...
#define HI_EXCEPTION_IND_ID		 (HI_IND_BASE+0x00)
/*HI exception id */
#define HI_STARTUP_IND_ID		   (HI_IND_BASE+0x01)
/*HI config (1st)id */
#define HI_TRACE_IND_ID			 (HI_IND_BASE+0x02)
/*HI trace id */
#define HI_GENERIC_IND_ID		   (HI_IND_BASE+0x03)
/*HI gereric indication id */
...
#define ST90TDS_FRAME_OUT_REG_ID	0x0007/*16 bits,Q mode read,nolen*/
...
#define ST90TDS_CONT_IRQ_RDY_ENABLE	 0xC000
/* bit 15:14 of control register*/
...
#define ST90TDS_CONFIG_CPU_RESET_BIT	0x4000
/* bit 14 of config_reg, cpu reset*/
#define ST90TDS_CONFIG_CPU_RESET_MASK	0xFFFFBFFF
/* bit 14 of config_reg*/
#define ST90TDS_CONFIG_PFETCH_BIT		0x00002000
/* bit 13 of config_reg, APB bus*/
#define ST90TDS_CONFIG_CPU_CLK_DIS_BIT	0x1000
/* bit 12 of config_reg*/
#define ST90TDS_CONFIG_CPU_CLK_DIS_MASK	0xFFFFEFFF
/* bit 12 of config_reg*/
#define ST90TDS_CONFIG_AHB_PFETCH_BIT	0x0800/* bit 11 of config_reg, AHB bus*/
#define ST90TDS_CONFIG_ACCESS_MODE_BIT	0x0400
/* bit 10 of config_reg, 0 QueueM*/
#define ST90TDS_CONFIG_ACCESS_MODE_MASK	0xFFFFFBFF
/* bit 10 of config register*/
...
typedef volatile struct{
	...
	uint32	DebugData[DOWNLOAD_CTRL_DATA_DWORDS];
...
(DOWNLOAD_CTRL_OFFSET + offsetof(download_cntl_t, ImageSize))
#define	DOWNLOAD_FLAGS_REG \
(DOWNLOAD_CTRL_OFFSET + offsetof(download_cntl_t, Flags))
#define DOWNLOAD_PUT_REG \
(DOWNLOAD_CTRL_OFFSET + offsetof(download_cntl_t, Put))
#define DOWNLOAD_TRACE_PC_REG \
(DOWNLOAD_CTRL_OFFSET + offsetof(download_cntl_t, TracePc))
#define	DOWNLOAD_GET_REG \
(DOWNLOAD_CTRL_OFFSET + offsetof(download_cntl_t, Get))
#define	DOWNLOAD_STATUS_REG	\
(DOWNLOAD_CTRL_OFFSET + offsetof(download_cntl_t, Status))
#define DOWNLOAD_DEBUG_DATA_REG	\
(DOWNLOAD_CTRL_OFFSET + offsetof(download_cntl_t, DebugData))
...
#define CUT2			2
#define CUT22			3
#define CUT11_ID_STR 		0x302e3830
#define CUT22_ID_STR1 		0x302e3132
#define CUT22_ID_STR2 		0x32302e30
#define CUT22_ID_STR3 		0x3335
#define CUT_ID_ADDR		0xFFF17F90
#define CUT2_ID_ADDR		0xFFF1FF90

#define DEVICE_UP		10
#define DEVICE_DOWN		11

#define HEADER_FOR_FWDING	32

#define MINI_HIF		32

typedef struct HI_MSG_HDR_S{
	uint16_t  MsgLen  ;
	/*length in bytes from MsgLen to end of payload */
	uint16_t  MsgId   ;
} HI_MSG_HDR;
