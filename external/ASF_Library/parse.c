	...
	if (header->subobjects > 0) {
		...
		for (i=0; i<header->subobjects; i++) {
			...
			if (current->type == GUID_HEADER_EXTENSION && !header->ext) {
				...
				current = (asfint_object_t *)realloc(current, sizeof(asf_object_headerext_t));
...
asf_parse_index(asf_file_t *file)
{
	...
	index = (asf_object_index_t *)malloc(sizeof(asf_object_index_t));
	if (!index) {
		return ASF_ERROR_OUTOFMEM;
	}
	...
	index->entries = (asf_index_entry_t *)malloc(index->entry_count * sizeof(asf_index_entry_t));
	if (!index->entries) {
		free(index);
		free(entry_data);
		return ASF_ERROR_OUTOFMEM;
