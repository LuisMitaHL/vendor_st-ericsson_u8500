...
// #define USE_90KHZ_TIMER  undefine this one if you want to use ARM loops instead of timer
    ...
    #if defined(STN8800A)||defined(STN8800B)

#define SCKRelease  *(DU32*)(SSCHndl->SCKPIOBase+0x400)&= ~(SSCHndl->SCKPIOMask);        // INPUT
#define SCKForceLow *(DU32*)(SSCHndl->SCKPIOBase+0x400)|=  (SSCHndl->SCKPIOMask);  \
                    *(DU32*)(SSCHndl->SCKPIOBase+       (4<<SSCHndl->SCKPIOBit )) =  0;

                   /* AFSEL = 0 (IO), DIR=0(INPUT)  LEV=0 */
#define SCKInit     *(DU32*)(SSCHndl->SCKPIOBase+0x420)&= ~(SSCHndl->SCKPIOMask); \
                    *(DU32*)(SSCHndl->SCKPIOBase+0x400)&= ~(SSCHndl->SCKPIOMask);
#define SCKRead  (((*(DU32*)(SSCHndl->SDAPIOBase+0x3FC))>>  SSCHndl->SCKPIOBit )&0x1)

#define SDARelease  *(DU32*)(SSCHndl->SDAPIOBase+0x400)&= ~(SSCHndl->SDAPIOMask);        // INPUT
#define SDAForceLow *(DU32*)(SSCHndl->SDAPIOBase+0x400)|=  (SSCHndl->SDAPIOMask);  \
                    *(DU32*)(SSCHndl->SDAPIOBase+       (4<<SSCHndl->SDAPIOBit )) =  0;

                   /* AFSEL = 0 (IO), DIR=0(INPUT)  LEV=0 */
#define SDAInit     *(DU32*)(SSCHndl->SDAPIOBase+0x420)&= ~(SSCHndl->SDAPIOMask); \
                    *(DU32*)(SSCHndl->SDAPIOBase+0x400)&= ~(SSCHndl->SDAPIOMask);
#define SDARead  (((*(DU32*)(SSCHndl->SDAPIOBase+0x3FC))>>  SSCHndl->SDAPIOBit )&0x1)
...
PUBLIC t_uint16 i2c_read_AD5398(t_uint8 controller_number) {
	...
	t_uint8             tab_i2c_data[3]={0,0,0};
    t_uint32                 Read;
    t_uint16                 Res=0xDEAD;

STSSC_Read  (controller_number, 0x19, tab_i2c_data,1,&Read   ,1000);
Res                 = ( (t_uint16)(((t_uint16)(tab_i2c_data[0]&0xff)) << 8)  & ((t_uint16)(tab_i2c_data[1]&0xff)) );
return(Res);
...
PUBLIC void i2c_write_AD5398(t_uint8 controller_number, t_uint16 value) {
	...
	if( STSSC_Write (1, 0x18, tab_i2c_data,2,&Written,1000,FALSE)!= ST_NO_ERROR)
    {
        // printf("e!");
    }
}
...
void            Write_16_8  (t_uint16 i2cnum,t_uint16 i2c_ad,t_uint16 i2c_sla,t_uint8 data)
{
    ...
    tab_i2c_data[1]     = (i2c_sla&0xFF);
    tab_i2c_data[2]     = data          ;
    if( STSSC_Write(i2cnum,i2c_ad,tab_i2c_data,3,&Written,1000,FALSE)!= ST_NO_ERROR)
    {
        // printf("e!");
    }
}
...
void            Write_16_16  (t_uint16 i2cnum,t_uint16 i2c_ad,t_uint16 i2c_sla,t_uint16 data)
{
    ...
    if( STSSC_Write(i2cnum,i2c_ad,tab_i2c_data,4,&Written,1000,FALSE)!= ST_NO_ERROR)
    {
        // printf("e!");
    }
}
