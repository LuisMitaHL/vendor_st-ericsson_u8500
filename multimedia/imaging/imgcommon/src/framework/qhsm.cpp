...
CScfStateMachine::CScfStateMachine () 
{
	...
	StateMachineNumber++;
    SCF_MSG1(0,"SCF_DEBUG ---> CREATE NEW SCF STATE MACHINE CScfStateMachine Number %.1d\n",StateMachineID);
...
void CScfStateMachine::ScfTransit(ScfOneState * cpToState) {
	...
	SCF_MSG3(1,"SCF_DEBUG(%d) TRANSIT FROM %s TO %s\n", StateMachineID,cpCurrentState->Name(), cpToState->Name());
...
#ifdef SCF_LOS
	SCF_MSG5(0,"SCF_DEBUG(%d) Process Event %.3d (%.25s) to State %.35s %lld 10^2us\n",StateMachineID,aEvt->sig,CError::stringSMSig(aEvt->sig),cpState->Name(),(LOS_getSystemTime()-ullTimerLast)/100);
#else
	SCF_MSG4(0,"SCF_DEBUG(%d) Process Event %.3d (%.25s) to State %.35s \n",StateMachineID,aEvt->sig,CError::stringSMSig(aEvt->sig),cpState->Name());
#endif

	/// Loop until State return 0 (event processed)
	while(cpState && go) {

		//SCF_MSG2(0,"Process Event %d to State %s\n",aEvt->sig,pscf_State->Name());
...
void CScfStateMachine::EnterHierarchy(ScfOneState *  cpFrom, ScfOneState *  cpTo) {
	s_scf_event sevt_entry = {SCF_STATE_ENTRY_SIG,{0},{}};	///< Define Entry event
	s_scf_event sevt_exit = {SCF_STATE_EXIT_SIG,{0},{}};	///< Define Entry event
...
void CScfStateMachine::ExitHierarchy(ScfOneState * pscf_From, ScfOneState * pscf_To) {
	s_scf_event sevt_exit = {SCF_STATE_EXIT_SIG,{0},{}}; ///< Define Exit event
...
ScfOneState *CScfStateMachine::CommonFather(ScfOneState *cpState_REF, ScfOneState *cpState_TWO) {
	...
	SCF_MSG2(2,"SCF_DEBUG(%d) Common Father : %s\n",StateMachineID,cpState_REF->Name());
...
ScfOneState *ScfAllState::LookState(ScfStatePtr pF, char const *s) {
	int idx = reinterpret_cast<int &>(pF) & ( SCF_ARRAY_TABLE - 1);
	ScfOneState *pcOneState;
...
void ScfOneState::LinkHierarchy(void) {
		s_scf_event sevt_Parent= {SCF_PARENT_SIG,{0},{}}; ///< Signal to request Parent
