...
#define HASH_MSG_DIGEST_SIZE    20
...
/* Enumeration for selecting between SHA-1 or MD5 algorithm */
typedef enum
{
    HASH_ALGO_SHA1  = 0x0,
    HASH_ALGO_MD5   = 0x1
...
typedef struct
{
    ...
    t_uint32        hash_state_csr[40];                         /* HASH Context Swap Registers 0-39 */
    t_uint32        hash_state_csr40;                           /* HASH Context Swap Registers 40 ie Status flags */
    t_uint32        hash_state_csr41;                           /* HASH Context Swap Registers 41 ie Input data */
...
PUBLIC t_hash_error         HASH_Init(IN t_logical_address hash_base_address);
...
/* Reset functions */
PUBLIC t_hash_error         HASH_Reset(void);
/* Configuration and Mode setting functions */
PUBLIC t_hash_error         HASH_ConfigureDmaRequest(IN t_hash_dma_req request_state);
PUBLIC t_hash_error         HASH_ConfigureLastValidBits(IN t_uint8 nblw_val);
PUBLIC t_hash_error         HASH_ConfigureDigestCal(IN t_hash_dcal_state hash_dcal_state);
PUBLIC t_hash_error         HASH_ConfigureProtection(IN t_hash_protection_config *p_protect_config);
PUBLIC t_hash_error         HASH_SetConfiguration(IN t_hash_config *p_config);
PUBLIC t_hash_error         HASH_Begin(void);
PUBLIC t_hash_error         HASH_ReadMessageDigest(OUT t_uint8 digest[HASH_MSG_DIGEST_SIZE]);

/* Status functions */
PUBLIC t_hash_device_status HASH_GetDeviceStatus(void);
PUBLIC t_bool               HASH_IsDcalOngoing(void);

#ifdef __HASH_ELEMENTARY    /* Elementary layer APIs */
PUBLIC t_hash_error         HASH_SetInputData(IN t_uint32 input_data);
PUBLIC t_hash_error         HASH_GetDataInput(OUT t_uint32 *p_data_input);
#endif /* End __HASH_ELEMENTARY */

#ifdef __HASH_ENHANCED      /* Enhanced layer APIs */
PUBLIC t_hash_error         HASH_Update(IN const t_uint8 *p_data_buffer, IN t_uint32 msg_length);
PUBLIC t_hash_error         HASH_End(OUT t_uint8 digest[HASH_MSG_DIGEST_SIZE]);
...
PUBLIC t_hash_error         HASH_SaveDeviceState(OUT t_hash_state *hash_state);
PUBLIC t_hash_error         HASH_ResumeDeviceState(IN const t_hash_state *hash_state);
