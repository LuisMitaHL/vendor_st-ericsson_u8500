...
//CHANGE THIS APPROPRIATELY FOR TESTING DIFFERENT SENSOR TUNNING DATA
...
t_cmd_list  ite_cmd_list_sensor_tunning_before_boot[] =
{
    { "SENSOR_TUNNING_help", C_ite_dbg_Before_Boot_SENSOR_TUNNING_help_cmd, "SENSOR_TUNNING_help: SENSOR_TUNNING_help\n" },
    { "Test_SENSOR_TUNNING_Support", C_ite_dbg_Before_Boot_Test_SENSOR_TUNNING_Support_cmd, "Test_SENSOR_TUNNING_Support: Test_SENSOR_TUNNING_Support \n" },
    { "Query_SENSOR_TUNNING_Idx", C_ite_dbg_Before_Boot_Query_SENSOR_TUNNING_Idx_cmd,
            "Query_SENSOR_TUNNING_Idx: Query_SENSOR_TUNNING_Idx [<all>|<t_uint16 Index1> <t_uint16 Index2> ...]\n" },
    { "QuerySensorInfo", C_ite_dbg_Before_Boot_QuerySensorInfo_cmd, "QuerySensorInfo: QuerySensorInfo\n" },
    { "Configure_SENSOR_TUNNING", C_ite_dbg_Before_Boot_Configure_SENSOR_TUNNING_cmd, "Configure_SENSOR_TUNNING: Configure_SENSOR_TUNNING <t_uint8* filename_with_path>\n" }
};
...
/* -----------------------------------------------------------------------
   ...
   ------------------------------------------------------------------------ */
void
Init_SENSOR_TUNNING_BEFORE_BOOT_ITECmdList(void)
{
    CLI_register_interface(
    "SENSOR_TUNNING_ITE_CMB",
    sizeof(ite_cmd_list_sensor_tunning_before_boot) / (sizeof(ite_cmd_list_sensor_tunning_before_boot[0])),
    ite_cmd_list_sensor_tunning_before_boot,
    1);
}


/* -----------------------------------------------------------------------
   FUNCTION : C_ite_dbg_Before_Boot_SENSOR_TUNNING_help_cmd
   PURPOSE  :
   ------------------------------------------------------------------------ */
CMD_COMPLETION
C_ite_dbg_Before_Boot_SENSOR_TUNNING_help_cmd(
int     a_nb_args,
char    **ap_args)
{
    char    mess[256];
    ...
    if (a_nb_args == 1)
    {
         ...
         LOS_Log("TestSensorTunningBeforeBootSupport: Checks whether SENSOR_TUNNING is suppported by the firmware \n");
         LOS_Log("TestSensorTunningBeforeBootSupport[Syntax]: TestSensorTunningSupport \n\n");

         LOS_Log("QuerySensorTunningBeforeBootIdx: Query indexes of sub block Ids\n");
         LOS_Log("QuerySensorTunningBeforeBootIdx[Syntax]: Query_SENSOR_TUNNING_Idx [<all>|<t_uint16 Index1> <t_uint16 Index2> ...]\n\n");

         LOS_Log("QuerySensorInfoBeforeBoot: Queries Sensor information \n");
         LOS_Log("QuerySensorInfoBeforeBoot[Syntax]: QuerySensorInfo \n\n");

         LOS_Log("ConfigureSensorTunningBeforeBoot: Pass SENSOR_TUNNING data buffer to the firmware\n");
#if READ_SENSOR_TUNNING_DATA_FROM_FILE
         LOS_Log("ConfigureSensorTunningBeforeBoot[Syntax]: Configure_SENSOR_TUNNING <t_uint8* filename_with_path>\n");
         LOS_Log("ConfigureSensorTunningBeforeBoot: filename_with_path is relative to ITE_ROOT_DIR ($(MMROOT)/imaging)\n");
         LOS_Log("ConfigureSensorTunningBeforeBoot[Example]: Configure_SENSOR_TUNNING ite_nmf/testdata/Dummy_SENSOR_TUNNING_Data.bin \n\n");
#else
         LOS_Log("ConfigureSensorTunningBeforeBoot[Syntax]: Configure_SENSOR_TUNNING\n");
#endif
    }
    else
    {
...
/* -----------------------------------------------------------------------
   FUNCTION : C_ite_dbg_Before_Boot_TestSensorTunningSupport_cmd
   PURPOSE  :
   ------------------------------------------------------------------------ */
CMD_COMPLETION
C_ite_dbg_Before_Boot_Test_SENSOR_TUNNING_Support_cmd(
int     a_nb_args,
char    **ap_args)
{
    char    mess[256];
    ...
    else
    {
        if (a_nb_args == 1)
        {
            ...
            {
                LOS_Log("*** SensorTunning Supported! ***\n");
            ...
            {
                CLI_disp_error("*** SensorTunning not Supported. ***\n");
            }
        }
        else
        {
...
/* -----------------------------------------------------------------------
   FUNCTION : C_ite_dbg_Before_Boot_Query_SENSOR_TUNNING_Idx_cmd
   PURPOSE  :
   ------------------------------------------------------------------------ */
CMD_COMPLETION
C_ite_dbg_Before_Boot_Query_SENSOR_TUNNING_Idx_cmd(
int     a_nb_args,
char    **ap_args)
{
    ...
    if (a_nb_args > 2)
    {
        if (0 == strcmp(ap_args[1], "help"))
        {
            ...
            CLI_disp_error("Query_SENSOR_TUNNING_dx[Syntax]: Query_SENSOR_TUNNING_Idx [<all>|<t_uint16 Index1> <t_uint16 Index2> ...]\n\n");
        }
        else if (0 == strcmp(ap_args[1], "all"))
        {
            ...
            {
                LOS_Log("*** SensorTunning not Supported. ***\n");
            }
        }
    }
    else if (a_nb_args > 1)
...
/* -----------------------------------------------------------------------
   FUNCTION : C_ite_dbg_Before_Boot_QuerySensorInfo_cmd
   PURPOSE  :
   ------------------------------------------------------------------------ */
CMD_COMPLETION
C_ite_dbg_Before_Boot_QuerySensorInfo_cmd(
int     a_nb_args,
char    **ap_args)
{
    char    mess[256];
...
/* -----------------------------------------------------------------------
   FUNCTION : C_ite_dbg_Before_Boot_Configure_SENSOR_TUNNING_cmd
   PURPOSE  : To be called after sensor boot and before streaming
   ------------------------------------------------------------------------ */
CMD_COMPLETION
C_ite_dbg_Before_Boot_Configure_SENSOR_TUNNING_cmd(
int     a_nb_args,
char    **ap_args)
{
    ...
    if (a_nb_args == 2)
    {
        ...
        //sensor tunning buffer ready; so now send its address to firmware
