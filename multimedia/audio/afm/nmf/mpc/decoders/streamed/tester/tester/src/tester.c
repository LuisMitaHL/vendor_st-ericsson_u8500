 ...
 */
/*****************************************************************************/
#include "decoders/streamed/tester/tester.nmf"
#include <stdio.h>
#include <stdlib.h>
#include "dbc.h"
#include "string.h"
...
ReadData(Buffer_t *Buf) {
    ...
    frameSize = 2 * DMAIN_BUFSIZE; //in bytes
...
fillInputBuffer(Buffer_p buf) {
    ...
    if ( (mNbsample *2) >= mFileSize) //mFileSize in bytes
    {
        buf->flags |= BUFFERFLAG_EOS;
        mEosReached = 1;
    }
