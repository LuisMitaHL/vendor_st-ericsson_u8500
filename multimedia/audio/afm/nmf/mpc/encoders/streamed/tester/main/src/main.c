...
#include <stdlib.h>
#include <archi.h>
#include "encoders/streamed/tester/main.nmf"
#include "dbc.h"
#include "string.h"
...
configureGraph() {
    ...
    max_consumed_bits = encoder.getMaxFrameSize();

    outputbuffer_size = max_consumed_bits / 16 + (max_consumed_bits % 16 ? 1 : 0) + 1;

    mMaxSamples = encoder.getMaxSamples();
    framebuffer_size = sizeof(t_sword)*mMaxSamples*mNbChans;

    wrapper_config.setParameter((void*)malloc_ext(framebuffer_size), framebuffer_size, (void*)malloc_ext(outputbuffer_size), outputbuffer_size);
