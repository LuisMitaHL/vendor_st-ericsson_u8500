 ...
 */
/*****************************************************************************/
#include <hst/bindings/shm/shmin.nmf>
...
#include "afm_nmf_hst_bindings_shm_shmin_src_shminTraces.h"
#endif

// TODO FIXME: Manage buffers than are not 32 bit multiple long
#define SUPPORTED_BUFFER_SIZE_MULTIPLE_IN_BYTES 4

void hst_bindings_shm_shmin::swap_bytes(t_uint8 *buffer, t_uint32 size, t_swap_mode swap_mode)
{
    swap_bytes_optim((void *) buffer, size, (int) swap_mode);
}

void hst_bindings_shm_shmin::process()
{
  if (mPorts[OUTPUT_PORT].queuedBufferCount()) 
    {
      OMX_BUFFERHEADERTYPE * bufOut = (OMX_BUFFERHEADERTYPE *)mPorts[OUTPUT_PORT].dequeueBuffer();
       
        mBufferSent = false;
		OstTraceFiltInst0(TRACE_DEBUG, "AFM_HOST: Shmin::process release input buffer");
		mPorts[INPUT_PORT].returnBuffer(bufOut);
    }

    if (mPorts[INPUT_PORT].queuedBufferCount() && mBufferSent == false)
    {
      OMX_BUFFERHEADERTYPE * bufIn = (OMX_BUFFERHEADERTYPE *)mPorts[INPUT_PORT].dequeueBuffer();
	...
	// round the length to the nearest 32bits multiple
	swap_bytes(bufIn->pBuffer, 
        ...
        mBufferSent = true;
		OstTraceFiltInst0(TRACE_DEBUG, "AFM_HOST: Shmin::process release output buffer");
	mPorts[OUTPUT_PORT].returnBuffer(bufIn);
    }
}

void hst_bindings_shm_shmin::reset()
{
    mBufferSent = false; 
}
void hst_bindings_shm_shmin::disablePortIndication(t_uint32 portIdx) {} ;
void hst_bindings_shm_shmin::enablePortIndication(t_uint32 portIdx) {} ;
void hst_bindings_shm_shmin::flushPortIndication(t_uint32 portIdx) { reset();} ;
...
void METH(fsmInit)(fsmInit_t initFsm)
{
  ...
  mPorts[INPUT_PORT].init(InputPort,  false, false, &mPorts[OUTPUT_PORT], 0, mShmConfig.nb_buffer,  &inputport,  INPUT_PORT, 
          (initFsm.portsDisabled & (1 << (INPUT_PORT)))!=0, (initFsm.portsTunneled & (1 << (INPUT_PORT)))!=0, this);
  mPorts[OUTPUT_PORT].init(OutputPort, true, false, &mPorts[INPUT_PORT], 0, 1, &outputport, OUTPUT_PORT, 
...
void METH(newFormat)(t_sample_freq sample_freq, t_uint16 chans_nb, t_uint16 sample_size) {
  ...
  if(!((Ihst_common_pcmsettings*)outputsettings.getReference())->IsNullInterface()) {
