...
*/
unsigned int g_alsasource_debug_mask;

char *log_prefix[ALSASOURCE_LOG_LAST] = {
	"INFO",
	"WARN",
	"ERR",
};

#ifndef ALSASOURCE_LOG_TO_FILE
...
{
	g_alsasource_debug_mask =
		(LOG_INFO_DEFINED << ALSASOURCE_LOG_INFO) |
		(LOG_WARN_DEFINED << ALSASOURCE_LOG_WARN) |
		(LOG_ERR_DEFINED << ALSASOURCE_LOG_ERR);
}

void alsasource_dbg_set_logs_from_properties()
{
	set_logging_defaults();
	const char* name_start = get_prop("ste.debug.alsasource.log");
	...
	while (*name_start) {
		...
		for (log_type = 0; log_type < ALSASOURCE_LOG_LAST; log_type++) {
			...
			{
				g_alsasource_debug_mask |= 1U << log_type;
			}
		}
...
{
	return (g_alsasource_debug_mask & (1U << level)) ? 1 : 0;
}
...
int alsasource_debug_print(enum ALSASOURCE_LOGLEVEL level, const char *file, int line, const char *function, const char *fmt, ...)
{
	...
	switch (level) {
	case ALSASOURCE_LOG_ERR:
		ALOGE(PRINT_FORMAT, function, buffer, insert_newline);
	break;
	case ALSASOURCE_LOG_WARN:
	case ALSASOURCE_LOG_INFO:
	default:
