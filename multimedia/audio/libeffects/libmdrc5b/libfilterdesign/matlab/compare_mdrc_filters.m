function compare_mdrc_filters(CutFreq, gains_dB, sampling_freq, biquad_size, FIR_size)
    ...
    if nargin > 4
        FIR_order = 2 * floor(FIR_size / 2) + 1;
    end;
    ...
    if n >= 1
        ...
        sum_H_flat_legacy        = zeros(1,     N);
        sum_H_gain_legacy        = zeros(1,     N);
        ...
        for i = 1 : n + 1
            ...
            sum_H_flat_legacy      = sum_H_flat_legacy      + H_legacy(i, :);
            sum_H_flat_butterworth = sum_H_flat_butterworth + H_butterworth(i, :);
            sum_H_gain_legacy      = sum_H_gain_legacy      + H_legacy(i, :)      * gains(1, i);
        ...
        H_FIR                   = zeros(n + 1, N);
        sum_H_flat_FIR          = zeros(1,     N);
        sum_H_gain_FIR          = zeros(1,     N);
        sum_H_gain_opt_FIR      = zeros(1,     N);
        ...
        gains_FIR_opt = optimize_gains_FIR(CutFreq, fs, FIR, gains);
        for i = 1 : n + 1
            sum_H_flat_FIR     = sum_H_flat_FIR     + H_FIR(i, :);
            sum_H_gain_FIR     = sum_H_gain_FIR     + H_FIR(i, :) * gains(1, i);
            sum_H_gain_opt_FIR = sum_H_gain_opt_FIR + H_FIR(i, :) * gains_FIR_opt(1, i);
        ...
        fprintf(file, '\n');
        fprintf(file, '\nRequested FIR gains (dB)     :');
        for i = 1 : n + 1
            fprintf(file, ' %5.2f', gains_dB(1, i));
        end;
        fprintf(file, '\nRequested FIR gains (linear) :');
        for i = 1 : n + 1
            fprintf(file, ' %1.8f', gains(1, i));
        end;
        fprintf(file, '\nOptimized FIR gains (linear) :');
        for i = 1 : n + 1
            fprintf(file, ' %1.8f', gains_FIR_opt(1, i));
        end;
        fprintf(file, '\n');
        ...
        subplot(2, 2, 2);
        hold on;
        plot(freq_axis_legacy, abs(sum_H_flat_legacy), '-k', 'LineWidth', 2);
        plot(freq_axis_legacy, abs(sum_H_gain_legacy), '-b', 'LineWidth', 2);
        % dB scale
        subplot(2, 2, 4);
        hold on;
        plot(freq_axis_legacy, max(min_dB, 20.0 * log10(abs(sum_H_flat_legacy))), '-k', 'LineWidth', 2);
        plot(freq_axis_legacy, max(min_dB, 20.0 * log10(abs(sum_H_gain_legacy))), '-b', 'LineWidth', 2);
        ...
        subplot(2, 1, 1);
        hold on;
        plot(freq_axis, abs(sum_H_flat_FIR),     '-k', 'LineWidth', 2);
        plot(freq_axis, abs(sum_H_gain_FIR),     '-b', 'LineWidth', 2);
        plot(freq_axis, abs(sum_H_gain_opt_FIR), '-c', 'LineWidth', 2);
        % dB scale
        subplot(2, 1, 2);
        hold on;
        plot(freq_axis, max(min_dB, 20.0 * log10(abs(sum_H_flat_FIR))),     '-k', 'LineWidth', 2);
        plot(freq_axis, max(min_dB, 20.0 * log10(abs(sum_H_gain_FIR))),     '-b', 'LineWidth', 2);
        plot(freq_axis, max(min_dB, 20.0 * log10(abs(sum_H_gain_opt_FIR))), '-c', 'LineWidth', 2);
...
function [gains_FIR_opt] = optimize_gains_FIR(CutFreq, fs, FIR, gains)
    % since FIR have the same size and are linear phase, they all have the same phase;
    ...
    % to the same linear combination of the gains of each filters
    %
    % so if we want to obtain a certain gain for the center of each sub-band (let's call them center_freq(1) ... center_freq(n)),
    % we have to solve the following linear system :
    %
    % | response(1,1) ... response(1,n) |   | g_optimized(1) |   | g_requested(1) |
    % | ............................... | x | .............. | = | .............. |
    % | response(n,1) ... response(n,n) |   | g_optimized(n) |   | g_requested(n) |
    %
    % with :
    % - g_requested the requested gains,
    % - g_optimized the gains to search,
    % - response(i,j) the gain (module of response) of FIR(j) for center_freq(i)

    n = size(FIR, 1);                           % number of FIR
    p = size(FIR, 2);                           % size of FIR
    assert(size(CutFreq, 1) == 1);
    assert(size(CutFreq, 2) == n - 1);
    assert(size(gains,   1) == 1);
    assert(size(gains,   2) == n);
    ...
    for i = 1 : n
        ...
        if i == 1
            f = CutFreq(1, 1) / 2.0;
        elseif i == n
            f = (CutFreq(1, n - 1) + fs / 2.0) / 2.0;
        else
            f = (CutFreq(1, i - 1) + CutFreq(1, i)) / 2.0;
        end;
        theta = 2.0 * pi * f / fs;

        for j = 1 : n
            % compute filter response of FIR(j) for center_freq(i)
            % Re(response(i,j)) = Sum[FIR(j,k+1) * cos(k*theta)] for k=0...p-1
            % Im(response(i,j)) = Sum[FIR(j,k+1) * sin(k*theta)] for k=0...p-1
            resp_re = 0.0;
            resp_im = 0.0;
            theta1  = 0.0;          % theta1 used for cos(x)
            theta2  = pi / 2.0;     % theta2 used for sin(x) = cos(pi/2-x)
            for k = 1 : p
                resp_re = resp_re + FIR(j, k) * cosine(theta1);
                resp_im = resp_im - FIR(j, k) * cosine(theta2);
                theta1  = theta1 + theta;
                theta2  = theta2 - theta;
                if theta1 > pi
            ...
            response(i, j) = sqrt(resp_re * resp_re + resp_im * resp_im);   % module of response for center frequency i and filter j
        end;
    end;

    % solve linear system
    gains_FIR_opt = solve_linear_system(response, gains')';     % equivalent to gains_FIR_opt = (inv(response) * gains')';


function [b] = solve_linear_system(a, c)
    ...
    fprintf(1, '*** solve_linear_system ***\n');
    for i = 1 : n
        fprintf(1, '| ');
        for j = 1 : n
            fprintf(1, '%+1.8f ', a(i, j));
        end;
        if i - 1 == floor(n / 2)
            fprintf(1, '| x | b%d | = ', i);
        else
    ...
    for i = 1 : n
        % search line j with max(abs(a(j,i))) for j=i...n
        a_max = abs(a(i, i));
        i_max = i;
        for j = i + 1 : n
            if abs(a(j, i)) > a_max
    ...
    % Thus it's easy to compute the last b(n), to replace its value in previous lines, then to compute b(n-1), and so on...
    for i = n : -1 : 1
        assert(a(i, i) ~= 0);
        b(i, 1) = c(i, 1) / a(i, i);
        for j = 1 : i - 1
