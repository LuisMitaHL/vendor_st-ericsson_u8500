...
T=1/fs;
checkdisto=0;
totalDelay = delay_37_34 + delay_4_3;
...
res1 = upfirdn(res,deci1_4_3,4,3);
res1 = res1 * 2^(shift_4_3);
res1 = res1 * mult_4_3;
figure;
i=1:fftsiz;
freq=(i-1)*fs*(148/102)/fftsiz;
spect=(4/fftsiz)*fft(res1(NRES-fftsiz+1:NRES).*chebwin(fftsiz,160)',fftsiz);
spectdb=20*log10(abs(spect)+epsilon);
plot(freq,spectdb);
grid on;
zoom on;
axis([0 (fs*(148/102)/2) -180 0]);
...
fid = fopen('testval_hamaca11_16_fast.c','w');
...
fprintf(fid,'const YMEM Float out11_16[%d] = {\n',NRES);
for k = 1:NRES-1
     fprintf(fid,'\tFORMAT_FLOAT(%.15f,MAXVAL),  \t\t// %d\n',res1(k),k-1);
end
k = NRES;
