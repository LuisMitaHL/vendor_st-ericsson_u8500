...
res1 = upfirdn(res,deci1_2x,2,1);
res1 = res1 * 2^(shift_2x);
res1 = res1 * mult_2x;
figure;
i=1:fftsiz;
freq=(i-1)*fs*(74/34)/fftsiz;
spect=(4/fftsiz)*fft(res1(NRES-fftsiz+1:NRES).*chebwin(fftsiz,160)',fftsiz);
spectdb=20*log10(abs(spect)+epsilon);
plot(freq,spectdb);
grid on;
zoom on;
axis([0 (fs*(74/34)/2) -180 0]);
...
fprintf(fid,'const YMEM Float out22_48[%d] = {\n',NRES);
for k = 1:NRES-1
     fprintf(fid,'\tFORMAT_FLOAT(%.15f,MAXVAL),  \t\t// %d\n',res1(k),k-1);
end
k = NRES;
