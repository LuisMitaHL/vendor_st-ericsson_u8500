...
/****************************************************************/
#ifndef _spl_algo_M4_h_
#define _spl_algo_M4_h_
/*
...
//#define wfmul1_M4(x,y) (int)((((long long) x) * ((long long) y)) >> 23)
//#define spl_mul48x24_M4(a,b)  2*((long long)b*(long long)((int)((a+(long long)0x800000)>>24)))+wfmul1_M4(b,(int)((a<<40)>>40))
...
typedef struct {
	int MemoryPreset;    /* Memory preset */
...
typedef struct
{
	...
	int DataInOffset;
	int DataOutOffset;
	int *SysMainInPtr[SPL_MAIN_CH_MAX];
	int *SysMainOutPtr[SPL_MAIN_CH_MAX];
	// IO related
	int RmsMeasure;
	int ConsumedSamples;
	int ValidSamples;
	int MainInBuf[SPL_MAIN_CH_MAX][SPL_BLOCK_SIZE_MAX];
	int MainOutBuf[SPL_MAIN_CH_MAX][SPL_BLOCK_SIZE_MAX*2]; /* x2 to accomodate mismatch b/w burst processing rate and outputting */
	...
	int LimiterLABuf [SPL_MAIN_CH_MAX][SPL_LOOKAHEAD_LEN_MAX]; /* buffer to subband lookahead */
	int LimiterLaIdx;
	int PowerAttackCoef; // attack coef for power estimation
	int PowerReleaseCoef;
	int GainAttackCoef;  // attack coef for limiter
	int GainReleaseCoef;
	int ThreshAttackCoef;  // attack coef for limiter
	int ThreshReleaseCoef;
	int ThresholddB;
	int ThresholdExp;
	int ThresholdMant;
	int ThresholdInvExp;
	int ThresholdInvMant;
	int Threshold;
	int TargetThreshold;
	...
	int GainMant;    // last gain applied
	int GainExp;     // last gain applied
	int GainHoldRem;
	int GainHtSamp;
	int ThreshHtSamp;
	int MeanGain;
 ...
 * functions
 */
extern void spl_main(int *inbuf, int *outbuf, short size, SPL_LOCAL_STRUCT_T *Heap);
/* init:                                     */
...
extern int spl_init(SPL_LOCAL_STRUCT_T * Heap, SplParam_t *pParam, 
...
extern int spl_get_gain(SPL_LOCAL_STRUCT_T * Heap);
#endif //_spl_algo_h_
