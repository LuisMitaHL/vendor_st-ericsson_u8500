...
#ifndef _GSMHR_DECODER_H_
#define _GSMHR_DECODER_H_

#include "AFM_Decoder.h"
#include "GSMHRHostDecAlgo.h"

/// @defgroup gsmhrdec GSMHR Decoder
/// @ingroup afmdec
/// @{
OMX_ERRORTYPE gsmhrhostdecFactoryMethod(ENS_Component_p *ppENSComponent);

class GSMHRDecoder: public AFM_Decoder {
public:

	/// Constructor
	OMX_ERRORTYPE  construct(void);
    virtual ~GSMHRDecoder(void) {};

 
    GSMHRDecoder(bool isHost) : mIsHost(isHost) { };
