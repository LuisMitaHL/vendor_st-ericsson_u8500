...
static int CheckIncompabilities(sqlite3* db_p, u85xx_device_t* u8540_devs, u85xx_d2d_t* u8540_d2ds)
{
	int dev_mic_nch = Alsactrl_DB_GetDevChannelCount(db_p, ALSACTRL_DEVSTR_MIC);
	UNUSED_PAR(u8540_d2ds);
...
int Alsactrl_Hwh_U8540(sqlite3* db_p, hwh_dev_next_t dev_next, hwh_d2d_next_t dev_next_d2d, fadeSpeed_t fadeSpeed)
{
	char* dev_names = NULL;
	...
	ret = Alsactrl_Hwh_U8540_Dev(db_p, dev_next, chip_id);

	// Execute device-handlers for d2d-connections
	ret = Alsactrl_Hwh_U8540_D2D(db_p, dev_next_d2d);
	...
	ret = Alsactrl_Hwh_U8540_Dev_UpdateSwitches(db_p, chip_id);

	// Update all on/off-switches for d2d-paths
	ret = Alsactrl_Hwh_U8540_D2D_UpdateSwitches();
...
int Alsactrl_Hwh_U8540_Init(enum audio_hal_chip_id_t chip_id_in, sqlite3* db_p)
{
	int ret_func = ERR_GENERIC;
	int ret;
