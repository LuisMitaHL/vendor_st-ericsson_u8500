                          ...
                          t_uint16 incr,t_mtf_swap_cfg swap_cfg)
{
  ...
  /* Only FIFO 16 is used */
...
#else
    while((DMA_GET_REG(DMA_ENR) & MTF_R) != 0);
#endif
    ...
    DMA_SET_REG_32(DMA_BSM_L, DMA_BSM_H, (read_value & ~(t_uint16)MTF_R)|(pgm_dma_swap ? MTF_R : 0));
#else
	read_value = DMA_GET_REG(DMA_BSM);
    DMA_SET_REG(DMA_BSM, (read_value & ~(t_uint16)MTF_R)|(pgm_dma_swap ? MTF_R : 0));
#endif
    ...
    /*DMA_SET_REG(DMA_DIR,MTF_R);*/ /* removed according to latest HW spec */
...

{
    ...
    t_uint16 EMU_unit_maskit_prev=0; /*  current value of EMU_unit_maskit */
...
#else
    while((DMA_GET_REG(DMA_ENR) & MTF_W) != 0);
#endif
...
#ifdef T1XHV_SIA
    read_value = DMA_GET_REG_32(DMA_BSM_L, DMA_BSM_H);
    DMA_SET_REG_32(DMA_BSM_L, DMA_BSM_H, (read_value & ~(t_uint16)MTF_W)|(pgm_dma_swap ? MTF_W : 0));
#else
	read_value = DMA_GET_REG(DMA_BSM);
    DMA_SET_REG(DMA_BSM, (read_value & ~(t_uint16)MTF_W)|(pgm_dma_swap ? MTF_W : 0));
#endif
