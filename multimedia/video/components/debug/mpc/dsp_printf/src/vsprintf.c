...
static char *prts( char *p, char *sarg, mysize_t width,
	mysize_t precision, int right );
static char *prtu( char *p, register unsigned uarg, register unsigned base,
	char zero, const char *alt, int right, mysize_t width, mysize_t precision,
	int upper );
...
static	mysize_t mystrlen(char *s)
{
 mysize_t n=0;
 if (s==0) return 0;
 while(*s++) n++;
...
static	void strncpy(char *dst,char *src,mysize_t n)
{
 while(n--) *dst++=*src++;
}
		...
		if( c == '%' ){
			...
			while( done == 0 ){
				...
				case 'c':
					...
					p = prts( p, (char *) ucstr, width, (mysize_t)dwidth, right );
					done = 1;
					break;
				case 's':
					/* print string */
					...
					p = prts( p, sarg, width, (mysize_t) dwidth, right );
					done = 1;
					break;
				case 'd':
				case 'i':
...
static char *
prts( char *p, char *sarg, mysize_t width, mysize_t precision, int right )
{
	mysize_t len = mystrlen( sarg );
	register char *b;
	char *d;
...
prtu( char *p, register unsigned uarg, register unsigned base, char zero,
	 const char *alt, int right, mysize_t width, mysize_t precision, int upper )
{
	char *mapc;
