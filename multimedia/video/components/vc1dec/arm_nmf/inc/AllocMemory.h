...
typedef enum VFM_Mem_Type {
    ...
    /// @brief CM_Alloc is used, for memory that is accessible from ARM and/or from the SVA using the DMA.
    /// @todo Will be deprecated soon. New coming HW buffer API should be used instead
    VFM_MEM_CM_ALLOC, 
    
    /// @brief CM_MPCAlloc for memory that is accessible directly by the DSP. This memory type
    /// should be used as few as possible
    VFM_MEM_CM_MPC_ALLOC, 
...
IMPORT_C void *VFM_CreateMemoryList(t_uint8 fullsw);
...
IMPORT_C void VFM_AddMemoryExternalSource(void *ctxt, t_uint8 *LogicalAddress, t_uint8 *PhysicalAddress, t_uint32 size);
...
IMPORT_C void VFM_CloseMemoryList(void *ctxt);
#ifdef __cplusplus
}
#endif
