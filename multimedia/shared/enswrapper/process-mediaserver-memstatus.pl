...
sub help {
    ...
    print "\t-e, --editor               print line info such that editors can exploit them\n";
    ...
    foreach my $entry (@{$entries}) {
        ...
        if(defined $info[2]) {
            ...
            if(-e "$symbols_dir$info[2]") {
                my $infoline = `prebuilt/linux-x86/toolchain/arm-linux-androideabi-4.4.x/bin/arm-linux-androideabi-addr2line -e $symbols_dir$info[2] $info[1]`;
                if ($editor) {
                    chomp($infoline);
                    $infoline .= ":";
                    printf "%-130s %s %s\n", $infoline, $info[1], $info[2];
                } else {
                    $infoline =~ s|$pwd/||;
