...
#undef LOG_TAG
#define LOG_TAG "ENSWrapperDumpMemInfo"
...
void memStatus(void)
{
    ...
    time_t t;
    char filename[SIZE];
    static const char * filenameFormat = "/data/ste-debug/memstatus-%d.txt";
    ...
    mapinfo * milist = get_process_mapinfo();

    sprintf(filename, filenameFormat, getpid());

    fd = open(filename, O_CREAT|O_RDWR|O_APPEND, S_IRWXU|S_IRWXG|S_IRWXO);
    if (fd == -1) {
	ALOGE("Unable to open file %s : %s\n", filename, strerror(errno));
	free_mapinfo(milist);
	return;
    }

    ALOGI("Starting memStatus dump to %s", filename);
