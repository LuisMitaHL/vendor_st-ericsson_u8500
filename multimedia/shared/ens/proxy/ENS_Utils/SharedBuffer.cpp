...
ENS_API_EXPORT
SharedBuffer::SharedBuffer(OMX_U32 domain_id, 
                           t_cm_size byte_size, 
                           OMX_ERRORTYPE &error)
    : mOmxHeader(0),
      shared_buffer_arm(0),
      ...
      mIsBufferAllocated(false)
{
    ...
    DBC_ASSERT(bufferMpcAddr!=0 || bufferPhysicalAddr!=0);
...
OMX_ERRORTYPE
SharedBuffer::allocateArmSharedBuffer_t(OMX_U32 domain_id)
{
    return ENS::allocMpcMemory(domain_id, CM_MM_MPC_SDRAM24,
                               sizeof(ARMSharedBuffer_t)/4,
                               CM_MM_ALIGN_2WORDS,
                               &header_memory_handle);
}

void
SharedBuffer::initSharedBufferHeaders(t_cm_size byte_size)
{
    t_cm_system_address sysAddr;
    t_uint32 mpcBuffAddr = 0;
...
void SharedBuffer::updateOMXHeader(bool enableCopy)
{
    ...
    if(shared_buffer_arm->byteInLastWord == 1) filledLen--;
    mOmxHeader->nFilledLen = filledLen;

    // Offset
    if (mMpcBufAddr) {
        mOmxHeader->nOffset = 
            (shared_buffer_arm->data - mMpcBufAddr) * 2;
    } else {
        t_uint32 dataPhysicalAddress = 
            (shared_buffer_arm->addressh << 24) | (shared_buffer_arm->addressl & 0xFFFFFF);
    ...
    // Timestamp
    mOmxHeader->nTimeStamp =
        ((OMX_S64)shared_buffer_arm->nTimeStamph << 48) | 
        ((OMX_S64)shared_buffer_arm->nTimeStamplh << 24) | (shared_buffer_arm->nTimeStampll & 0xffffff);

    // if DSP buffer is not the OMX buffer we need a memcpy
    if (enableCopy && mBufLogicalAddr != mOmxHeader->pBuffer) {
        ::memcpy((char*)mOmxHeader->pBuffer + mOmxHeader->nOffset, 
                 (char*)mBufLogicalAddr + mOmxHeader->nOffset,
                 mOmxHeader->nFilledLen);
    }
}

ENS_API_EXPORT void
SharedBuffer::updateMPCHeader(void)
{
    // if DSP buffer is not the same as OMX buffer we need a memcpy
    if (mBufLogicalAddr != mOmxHeader->pBuffer) {
        ::memcpy((char*) mBufLogicalAddr + mOmxHeader->nOffset, 
                 (char*) mOmxHeader->pBuffer + mOmxHeader->nOffset,
                 mOmxHeader->nFilledLen);
    }
