...
export LOGFILE=/var/log/Primary_Camera.log
if [ -f $LOGFILE ]; then
  rm $LOGFILE
fi
...
echo " " | tee -a $LOGFILE
echo "*****************************************************************************" | tee -a $LOGFILE
echo "******************** Take Picrtures Primary camera tests ********************" | tee -a $LOGFILE
echo "*****************************************************************************" | tee -a $LOGFILE
...
sleep 2
CheckVerdict=$(grep "Initialize_primary" /var/log/Preview_Primary_640-480.log | cut -d"." -f 27)
...
CheckVerdict=$(grep "Set_preview" /var/log/Preview_Primary_640-480.log | cut -d"." -f 27)
if [ "$CheckVerdict" == "PASSED" ]
	then
	SetPrePrim="***PASSED***"
	else
...
CheckVerdict=$(grep "Set_picture" /var/log/Preview_Primary_640-480.log | cut -d"." -f 27)
if [ "$CheckVerdict" == "PASSED" ]
	then
	SetPicPrim="***PASSED***"
	else
...
rm -f /var/log/Preview_Primary_640-480.log
...
ls /mnt/UserFS2/frame*.bmp
ls /mnt/UserFS2/frame*.bmp > /usr/bin/HATS_validation/Camera/DTHscripts/checklog.log
...
# Verdict result
echo " "
echo "Initialize Primary Camera:...................................... $InitPrim" | tee -a $LOGFILE
echo "Set Preview Properties Primary Camera:.......................... $SetPrePrim" | tee -a $LOGFILE
echo "Set Picture properties Primary Camera:.......................... $SetPicPrim" | tee -a $LOGFILE
echo "Take Picture from primary sensor in single shot mode:........... $TakePicSing" | tee -a $LOGFILE
echo "Take Picture from primary sensor in finite burst shot mode:..... $TakePicFinBurst" | tee -a $LOGFILE
echo "Take Picture from primary sensor in infinite burst shot mode:... $TakePicInfBurst" | tee -a $LOGFILE
