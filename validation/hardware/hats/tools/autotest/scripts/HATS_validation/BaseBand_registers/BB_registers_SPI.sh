...
export LOGFILE=/var/log/BB_registerSPI.log
if [ -f $LOGFILE ]; then
  rm $LOGFILE
fi
...
echo " " | tee -a $LOGFILE
echo "*****************************************************************************" | tee -a $LOGFILE
echo "***************************** BB Registers SPI ******************************" | tee -a $LOGFILE
echo "*****************************************************************************" | tee -a $LOGFILE
...
while [ "$INC" != 4 ]
 ...
 then
  ...
  while [ "$NBline" != 0 ]
   ...
   case "$Reg" in
    SPI-0  )    echo "Read SPI-0 BB registers:.............. $verdict" | tee -a $LOGFILE;;
    SPI-1  )    echo "Read SPI-1 BB registers:.............. $verdict" | tee -a $LOGFILE;;
    SPI-2  )    echo "Read SPI-2 BB registers:.............. $verdict" | tee -a $LOGFILE;;
    SPI-3  )    echo "Read SPI-3 BB registers:.............. $verdict" | tee -a $LOGFILE;;
    * ) exit;;
  esac
   export LOGFILE
  fi
