...
export LOGFILE=/var/log/BB_registerHASH.log
if [ -f $LOGFILE ]; then
  rm $LOGFILE
fi
...
echo " " | tee -a $LOGFILE
echo "*****************************************************************************" | tee -a $LOGFILE
echo "***************************** BB Registers HASH *****************************" | tee -a $LOGFILE
echo "*****************************************************************************" | tee -a $LOGFILE
...
while [ "$INC" != 2 ]
 ...
 then
  ...
  while [ "$NBline" != 0 ]
   ...
   case "$Reg" in
    HASH-0  )    echo "Read HASH-0 BB registers:............. $verdict" | tee -a $LOGFILE;;
    HASH-1  )    echo "Read HASH-1 BB registers:............. $verdict" | tee -a $LOGFILE;;
    * ) exit;;
  esac
  fi
