...
export LOGFILE=/var/log/BB_registerPRCMU.log
if [ -f $LOGFILE ]; then
  rm $LOGFILE
fi
...
echo " " | tee -a $LOGFILE
echo "*****************************************************************************" | tee -a $LOGFILE
echo "**************************** BB Registers PRCMU *****************************" | tee -a $LOGFILE
echo "*****************************************************************************" | tee -a $LOGFILE
...
while [ "$INC" != 1 ]
 ...
 then
  ...
  while [ "$NBline" != 0 ]
   ...
   case "$Reg" in
    PRCMU  )    echo "Read PRCMU BB registers:.............. $verdict" | tee -a $LOGFILE;;
    * ) exit;;
  esac
  fi

done
REG=$(grep "PRCM_SEM " /usr/bin/HATS_validation/BaseBand_registers/BB_register.txt | cut -d' ' -f3)
sk.sh db w $REG 0
