...
export LOGFILE=/var/log/BB_registerRTT.log
if [ -f $LOGFILE ]; then
  rm $LOGFILE
fi
...
echo " " | tee -a $LOGFILE
echo "*****************************************************************************" | tee -a $LOGFILE
echo "***************************** BB Registers RTT ******************************" | tee -a $LOGFILE
echo "*****************************************************************************" | tee -a $LOGFILE
...
while [ "$INC" != 2 ]
 ...
 then
  ...
  while [ "$NBline" != 0 ]
   ...
   case "$Reg" in
    RTT0  )     echo "Read RTT0 BB registers:............... $verdict" | tee -a $LOGFILE;;
    RTT1  )     echo "Read RTT1 BB registers:............... $verdict" | tee -a $LOGFILE;;
    * ) exit;;
  esac
  fi
