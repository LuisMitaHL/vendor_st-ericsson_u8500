...
export LOGFILE=/var/log/BB_registerTIMER.log
if [ -f $LOGFILE ]; then
  rm $LOGFILE
fi
...
echo " " | tee -a $LOGFILE
echo "*****************************************************************************" | tee -a $LOGFILE
echo "***************************** BB Registers TIMER ****************************" | tee -a $LOGFILE
echo "*****************************************************************************" | tee -a $LOGFILE
...
while [ "$INC" != 2 ]
 ...
 then
  ...
  while [ "$NBline" != 0 ]
   ...
   case "$Reg" in
    TIMER-0  )   echo "Read TIMER-0 BB registers:............ $verdict" | tee -a $LOGFILE;;
    TIMER-1  )   echo "Read TIMER-1 BB registers:............ $verdict" | tee -a $LOGFILE;;
    * ) exit;;
  esac
  fi
