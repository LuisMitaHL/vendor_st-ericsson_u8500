...
export LOGFILE=/var/log/BB_registerPTM.log
if [ -f $LOGFILE ]; then
  rm $LOGFILE
fi
...
echo " " | tee -a $LOGFILE
echo "*****************************************************************************" | tee -a $LOGFILE
echo "***************************** BB Registers PTM ******************************" | tee -a $LOGFILE
echo "*****************************************************************************" | tee -a $LOGFILE
...
while [ "$INC" != 2 ]
 ...
 then
  ...
  while [ "$NBline" != 0 ]
   ...
   case "$Reg" in
    PTM0  )     echo "Read PTM0 BB registers:............... $verdict" | tee -a $LOGFILE;;
    PTM1  )     echo "Read PTM1 BB registers:............... $verdict" | tee -a $LOGFILE;;
    * ) exit;;
  esac
  fi
