...
export LOGFILE=/var/log/BB_registerUART.log
if [ -f $LOGFILE ]; then
  rm $LOGFILE
fi
...
echo " " | tee -a $LOGFILE
echo "*****************************************************************************" | tee -a $LOGFILE
echo "***************************** BB Registers UART *****************************" | tee -a $LOGFILE
echo "*****************************************************************************" | tee -a $LOGFILE
export LOGFILE
...
while [ "$INC" != 3 ]
 ...
 then
  ...
  while [ "$NBline" != 0 ]
   ...
   case "$Reg" in
    UART-0  )    echo "Read UART-0 BB registers:............. $verdict" | tee -a $LOGFILE;;
    UART-1  )    echo "Read UART-1 BB registers:............. $verdict" | tee -a $LOGFILE;;
    UART-2  )    echo "Read UART-2 BB registers:............. $verdict" | tee -a $LOGFILE;;
    * ) exit;;
  esac
  fi
