...
export LOGFILE=/var/log/BB_registerSSP.log
if [ -f $LOGFILE ]; then
  rm $LOGFILE
fi
...
echo " " | tee -a $LOGFILE
echo "*****************************************************************************" | tee -a $LOGFILE
echo "***************************** BB Registers SSP ******************************" | tee -a $LOGFILE
echo "*****************************************************************************" | tee -a $LOGFILE
...
while [ "$INC" != 2 ]
 ...
 then
  ...
  while [ "$NBline" != 0 ]
   ...
   case "$Reg" in
    SSP-0  )    echo "Read SSP-0 BB registers:.............. $verdict" | tee -a $LOGFILE;;
    SSP-1  )    echo "Read SSP-1 BB registers:.............. $verdict" | tee -a $LOGFILE;;
    * ) exit;;
  esac
  fi
