...
export LOGFILE=/var/log/BB_registerMSSETM11CS.log
if [ -f $LOGFILE ]; then
  rm $LOGFILE
fi
...
echo " " | tee -a $LOGFILE
echo "*****************************************************************************" | tee -a $LOGFILE
echo "************************** BB Registers MSSETM11CS **************************" | tee -a $LOGFILE
echo "*****************************************************************************" | tee -a $LOGFILE
...
while [ "$INC" != 2 ]
 ...
 let INC=INC+1
 case "$INC" in
  1 ) Reg=MSSETM11CS-1; NA=0;;
  2 ) Reg=MSSETM11CS-2; NA=0;;
 esac
 ...
 then
  ...
  while [ "$NBline" != 0 ]
   ...
   case "$Reg" in
    MSSETM11CS-1  ) echo "Read MSSETM11CS-1 BB registers:....... $verdict" | tee -a $LOGFILE;;
    MSSETM11CS-2  ) echo "Read MSSETM11CS-2 BB registers:....... $verdict" | tee -a $LOGFILE;;
    * ) exit;;
  esac
  fi
