...
export LOGFILE=/var/log/BB_registerMSSCTI.log
if [ -f $LOGFILE ]; then
  rm $LOGFILE
fi
...
echo " " | tee -a $LOGFILE
echo "*****************************************************************************" | tee -a $LOGFILE
echo "**************************** BB Registers MSSCTI ****************************" | tee -a $LOGFILE
echo "*****************************************************************************" | tee -a $LOGFILE
...
while [ "$INC" != 4 ]
 ...
 let INC=INC+1
 case "$INC" in
  1 ) Reg=MSSCTI-CS; NA=0;;
  2 ) Reg=MSSCTI-L1CTI; NA=0;;
  3 ) Reg=MSSCTI-L2CTI; NA=0;;
  4 ) Reg=MSSCTI-MSS; NA=0;;
 esac
 ...
 then
  ...
  while [ "$NBline" != 0 ]
   ...
   case "$Reg" in
    MSSCTI-CS  )  echo "Read MSSCTI-CS BB registers:.......... $verdict" | tee -a $LOGFILE;;
    MSSCTI-L1CTI  ) echo "Read MSSCTI-L1CTI BB registers:....... $verdict" | tee -a $LOGFILE;;
    MSSCTI-L2CTI  ) echo "Read MSSCTI-L2CTI BB registers:....... $verdict" | tee -a $LOGFILE;;
    MSSCTI-MSS  )  echo "Read MSSCTI-MSS BB registers:......... $verdict" | tee -a $LOGFILE;;
    * ) exit;;
  esac
  fi
