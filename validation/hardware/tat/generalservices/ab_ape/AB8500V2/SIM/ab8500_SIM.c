/****************************************************************************
 * File : c:/Lisp/Production/ab8500_V2/ab8500_SIM/Linux/ab8500_SIM.c
 * 
 *
...
unsigned char get_power_down_procedure()
  {
     ...
     * activated 
     */ 
    value = (I2CRead(SIM_INTERFACE_CONTROL_REG) & POWE_DOWN_PROCEDURE_ENABLE_PARAM_MASK) >> 7;
    return value;
  }
...
unsigned char get_reset_duration()
  {
     ...
     * clock is applied 
     */ 
    value = (I2CRead(SIM_INTERFACE_CONTROL_REG) & RESET_DURATION_PARAM_MASK) >> 6;
    return value;
  }
...
unsigned char get_warm_reset()
  {
     ...
     * 1: Warm reset of the card active 
     */ 
    value = (I2CRead(SIM_INTERFACE_CONTROL_REG) & WARM_RESET_ENABLE_PARAM_MASK) >> 5;
    return value;
  }
...
unsigned char get_internally_reset()
  {
     ...
     * 1: An internally reset card 
     */ 
    value = (I2CRead(SIM_INTERFACE_CONTROL_REG) & INTERNALLY_RESET_PARAM_MASK) >> 4;
    return value;
  }
...
unsigned char get_sim_power_on()
  {
     ...
     * 1: SIM power on by MCU 
     */ 
    value = (I2CRead(SIM_INTERFACE_CONTROL_REG) & SIM_POWER_ON_PARAM_MASK) >> 3;
    return value;
  }
...
unsigned char get_clock32_selected()
  {
     ...
     * 1: Clk32k selected to counter 
     */ 
    value = (I2CRead(SIM_INTERFACE_CONTROL_REG) & CLOCK_32_SELECTED_PARAM_MASK) >> 2;
    return value;
  }
...
unsigned char get_card_clock_selected()
  {
     ...
     * 1: SIMIfClk/2 selected to counter 
     */ 
    value = (I2CRead(SIM_INTERFACE_CONTROL_REG) & CARD_CLOCK_SELECTED_PARAM_MASK) >> 1;
    return value;
  }
...
unsigned char get_counter_reset()
  {
     ...
     * 1: Counter reset active 
     */ 
    value = (I2CRead(SIM_INTERFACE_CONTROL_REG) & COUNTER_RESET_PARAM_MASK);
    return value;
  }
...
unsigned char set_comp_value_low_byte( unsigned char param )
  {
    ...
    /* 
     * ATR error is generated, if the counter reaches this value 
     */ 
...
unsigned char get_vsim_active_mode()
  {
     ...
     * 1: VSIM always in active mode (if VSIM is on) 
     */ 
    value = (I2CRead(SIM_SUPPLY_MANAGEMENT_REG) & ACTIVE_MODE_PARAM_MASK) >> 7;
    return value;
  }
...
unsigned char get_sim_da_pull_up()
  {
     ...
     * 1: SIMDa pull up is R1 + R2 
     */ 
    value = (I2CRead(SIM_SUPPLY_MANAGEMENT_REG) & PULL_UP_PARAM_MASK) >> 6;
    return value;
  }
...
unsigned char get_sim_comp_detection()
  {
     ...
     * 1: SIMCardDetX comparator enabled 
     */ 
    value = (I2CRead(SIM_SUPPLY_MANAGEMENT_REG) & COMPARATOR_ENABLED_PARAM_MASK) >> 5;
    return value;
  }
...
unsigned char get_removal_detection()
  {
     ...
     * 1: SIMIf detects removal from SIMCarDetX also 
     */ 
    value = (I2CRead(SIM_SUPPLY_MANAGEMENT_REG) & REMOVAL_DETECTION_PARAM_MASK) >> 4;
    return value;
  }
...
unsigned char get_vsim_regulator()
  {
     ...
     * 1: VSIM regulator always on 
     */ 
    value = (I2CRead(SIM_SUPPLY_MANAGEMENT_REG) & VSIM_REGULATOR_PARAM_MASK) >> 3;
    return value;
  }
...
unsigned char get_vsim_voltage()
  {
     ...
     * 10: VSIM1 voltage level 3.0V 
     */ 
    value = (I2CRead(SIM_SUPPLY_MANAGEMENT_REG) & VSIM_VOLTAGE_PARAM_MASK) >> 1;
    return value;
  }
...
unsigned char get_sim_io_normal()
  {
     ...
     * 1: ISO-UICC disabled, SIM IOs gen. purpose outputs 
     */ 
    value = (I2CRead(SIM_SUPPLY_MANAGEMENT_REG) & SIM_IO_NORMAL_PARAM_MASK);
    return value;
  }
 ...
 *
 * Notes : From register 0x0485, bits 1:1
 *
 **************************************************************************/
unsigned char set_sim_card_det_polarity( enum switch_open param )
  {
    ...
    switch( param ){
        case SWITCH_OPEN_E: 
           value = old_value | SWITCH_OPEN_PARAM_MASK; 
           break;
        case SWITCH_CLOSE_E: 
           value = old_value & ~ SWITCH_OPEN_PARAM_MASK;
           break;
...
unsigned char get_sim_card_det_polarity()
  {
     ...
     * CUiccDetect ball is PullUp to Battery (Switch Open) 
     */ 
    value = (I2CRead(SIM_SUPPLY_MANAGEMENT_2_REG) & SWITCH_OPEN_PARAM_MASK) >> 1;
    return value;
  }
...
unsigned char get_select_vsim_power()
  {
     ...
     * 1: Select VinVsim ball to supply Vsim LDO 
     */ 
    value = (I2CRead(SIM_SUPPLY_MANAGEMENT_2_REG) & SELECT_VSIM_POWER_PARAM_MASK);
    return value;
  }
...
unsigned char set_sim_source_interrupts( unsigned char param )
  {
    ...
    value = I2CRead(SIM_SOURCE_INTERRUPTS_REG);

    /* 
     * SimSourceInterrupts 
     */ 
    I2CWrite(SIM_SOURCE_INTERRUPTS_REG, param);
    return value;
...
unsigned char set_intr_tx_error( unsigned char param )
  {
    ...
    old_value = I2CRead(SIM_SOURCE_INTERRUPTS_REG);
    ...
    value =  old_value & ~INTR_TX_ERROR_PARAM_MASK;
     ...
     param |= (value << 0x3);  
    I2CWrite(SIM_SOURCE_INTERRUPTS_REG, value);
...
unsigned char get_intr_tx_error()
  {
     ...
     * Tx Error Interrupt 
     */ 
    value = (I2CRead(SIM_SOURCE_INTERRUPTS_REG) & INTR_TX_ERROR_PARAM_MASK) >> 3;
    return value;
  }
...
unsigned char get_intr_atr_error()
  {
     ...
     * pending (Activates if no ATR received 
     */ 
    value = (I2CRead(SIM_SOURCE_INTERRUPTS_REG) & INTR_ATR_ERROR_PARAM_MASK) >> 2;
    return value;
  }
...
unsigned char get_intr_card_detected()
  {
     ...
     * edges) 
     */ 
    value = (I2CRead(SIM_SOURCE_INTERRUPTS_REG) & INTR_CARD_DETECTED_PARAM_MASK) >> 1;
    return value;
  }
...
unsigned char set_sim_latch_interrupts( unsigned char param )
  {
    ...
    value = I2CRead(SIM_LATCH_INTERRUPTS_REG);

    /* 
     * SimLatchInterrupts 
     */ 
    I2CWrite(SIM_LATCH_INTERRUPTS_REG, param);
    return value;
...
unsigned char set_latch_tx_error( unsigned char param )
  {
    ...
    old_value = I2CRead(SIM_LATCH_INTERRUPTS_REG);
    ...
    value =  old_value & ~LATCH_TX_ERROR_PARAM_MASK;
     ...
     param |= (value << 0x3);  
    I2CWrite(SIM_LATCH_INTERRUPTS_REG, value);
...
unsigned char get_latch_tx_error()
  {
     ...
     * Tx Error Interrupt 
     */ 
    value = (I2CRead(SIM_LATCH_INTERRUPTS_REG) & LATCH_TX_ERROR_PARAM_MASK) >> 3;
    return value;
  }
...
unsigned char get_latch_atr_error()
  {
     ...
     * pending (Activates if no ATR received 
     */ 
    value = (I2CRead(SIM_LATCH_INTERRUPTS_REG) & LATCH_ATR_ERROR_PARAM_MASK) >> 2;
    return value;
  }
...
unsigned char get_latch_card_detected()
  {
     ...
     * edges) 
     */ 
    value = (I2CRead(SIM_LATCH_INTERRUPTS_REG) & LATCH_CARD_DETECTED_PARAM_MASK) >> 1;
    return value;
  }
...
unsigned char set_sim_mask_interrupts( unsigned char param )
  {
    ...
    value = I2CRead(SIM_MASK_INTERRUPTS_REG);

    /* 
     * SimMaskInterrupts 
     */ 
    I2CWrite(SIM_MASK_INTERRUPTS_REG, param);
    return value;
...
unsigned char set_mask_tx_error( unsigned char param )
  {
    ...
    old_value = I2CRead(SIM_MASK_INTERRUPTS_REG);
    ...
    value =  old_value & ~MASK_TX_ERROR_PARAM_MASK;
     ...
     param |= (value << 0x3);  
    I2CWrite(SIM_MASK_INTERRUPTS_REG, value);
...
unsigned char get_mask_tx_error()
  {
     ...
     * Tx Error Interrupt 
     */ 
    value = (I2CRead(SIM_MASK_INTERRUPTS_REG) & MASK_TX_ERROR_PARAM_MASK) >> 3;
    return value;
  }
...
unsigned char get_mask_atr_error()
  {
     ...
     * pending (Activates if no ATR received 
     */ 
    value = (I2CRead(SIM_MASK_INTERRUPTS_REG) & MASK_ATR_ERROR_PARAM_MASK) >> 2;
    return value;
  }
...
unsigned char get_mask_card_detected()
  {
     ...
     * edges) 
     */ 
    value = (I2CRead(SIM_MASK_INTERRUPTS_REG) & MASK_CARD_DETECTED_PARAM_MASK) >> 1;
    return value;
  }
...
unsigned char get_weak_pull_down()
  {
     ...
     * 1: USB-UICC data lines weak pull down not active 
     */ 
    value = (I2CRead(SIM_IF_CONTOL_REG) & WEAK_PULL_DOWN_ACTIVE_PARAM_MASK) >> 5;
    return value;
  }
...
unsigned char get_sim_reset_value()
  {
     ...
     * 1: CIsoReset pad driven high if Simrst_force 
     */ 
    value = (I2CRead(SIM_IF_CONTOL_REG) & PAD_DRIVEN_PARAM_MASK) >> 4;
    return value;
  }
...
unsigned char get_usim_by_pass()
  {
     ...
     * bypassed. Only data drivers in use. 
     */ 
    value = (I2CRead(SIM_IF_CONTOL_REG) & USIM_BY_PASS_PARAM_MASK) >> 3;
    return value;
  }
...
unsigned char get_sim_reset_control()
  {
     ...
     * 1: CIsoreset ball driven by Simrst value 
     */ 
    value = (I2CRead(SIM_IF_CONTOL_REG) & SIM_RESET_CONTROL_PARAM_MASK) >> 2;
    return value;
  }
...
unsigned char get_usim_load()
  {
     ...
     * 1: Data line load 21-30pF 
     */ 
    value = (I2CRead(SIM_IF_CONTOL_REG) & USIM_LOAD_PARAM_MASK) >> 1;
    return value;
  }
...
unsigned char get_usim_power()
  {
     ...
     * released 
     */ 
    value = (I2CRead(SIM_IF_CONTOL_REG) & USIM_POWER_ON_PARAM_MASK);
    return value;
  }
 ...
 *
 * Notes : From register 0x048A, bits 3:3
 *
 **************************************************************************/
unsigned char set_filter_active( enum filter_enable param )
  {
    ...
    switch( param ){
        case FILTER_DISABLE_E: 
           value = old_value | FILTER_ENABLE_PARAM_MASK; 
           break;
...
unsigned char get_filter_active()
  {
     ...
     * 1:Filter disabled (bypass) 
     */ 
    value = (I2CRead(SIM_DELAY_REG) & FILTER_ENABLE_PARAM_MASK) >> 3;
    return value;
  }
...
unsigned char get_sim_delay()
  {
     ...
     * Sim Ad Delay 
     */ 
    value = (I2CRead(SIM_DELAY_REG) & SIM_DELAY_PARAM_MASK);
    return value;
  }
 ...
 * OUT  : 
 *
 * RET  : Return the value of register SimSupplyManagement3
 *
 * Notes : From register 0x048B, bits 2:2
 *
 **************************************************************************/
unsigned char set_v_mod_select( enum v_mod_select param )
  {
    ...
    switch( param ){
        case VMOD_SELECT_BY_VMODSEL2_E: 
           value = old_value | V_MOD_SELECT_PARAM_MASK; 
           break;
...
unsigned char get_v_mod_select()
  {
     ...
     * 1:Vmod ouput voltage is set by VmodSel2 register 
     */ 
    value = (I2CRead(SIM_SUPPLY_MANAGEMENT_3_REG) & V_MOD_SELECT_PARAM_MASK) >> 2;
    return value;
  }
