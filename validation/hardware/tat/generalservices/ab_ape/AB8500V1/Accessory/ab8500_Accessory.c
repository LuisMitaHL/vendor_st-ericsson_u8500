...
unsigned char set_acc_detect1_th( enum detection_threshold param )
  {
    ...
    old_value = SPIRead(ACC_DETECT_1_REG);
    ...
    value =  old_value & ~DETECTION_THRESHOLD_PARAM_MASK;
     ...
     value |= ( param << 0x3);  
    SPIWrite(ACC_DETECT_1_REG, value);
...
unsigned char get_acc_detect1_th()
  {
     ...
     * Accessory Detection threshold on AccDetect1 ball 
     */ 
    value = (SPIRead(ACC_DETECT_1_REG) & DETECTION_THRESHOLD_PARAM_MASK) >> 3;
    return value;
  }
...
unsigned char get_acc_detect_db()
  {
     ...
     * Accessory Detection threshold on AccDetect1 ball 
     */ 
    value = (SPIRead(ACC_DETECT_1_REG) & DETECTION_THRESHOLD_2_PARAM_MASK);
    return value;
  }
...
unsigned char set_acc_detect21_th( enum detection_threshold param )
  {
    ...
    old_value = SPIRead(ACC_DETECT_2_REG);
    ...
    value =  old_value & ~ACC_DETECT_21_TH_PARAM_MASK;
     ...
     value |= ( param << 0x4);  
    SPIWrite(ACC_DETECT_2_REG, value);
...
unsigned char get_acc_detect21_th()
  {
     ...
     * Accessory Detection threshold 2 on AccDetect1 ball 
     */ 
    value = (SPIRead(ACC_DETECT_2_REG) & ACC_DETECT_21_TH_PARAM_MASK) >> 4;
    return value;
  }
...
unsigned char get_acc_detect22_db()
  {
     ...
     * Accessory Detection threshold 2 on AccDetect1 ball 
     */ 
    value = (SPIRead(ACC_DETECT_2_REG) & ACC_DETECT_22_DB_PARAM_MASK);
    return value;
  }
...
unsigned char set_acc_detect22_comparator( enum comparator22_enable param )
  {
    ...
    old_value = SPIRead(ACC_DETECT_3_REG);
    ...
    SPIWrite(ACC_DETECT_3_REG, value);
...
unsigned char get_acc_detect22_comparator()
  {
     ...
     * 1: enable AccDetect22 comparator 
     */ 
    value = (SPIRead(ACC_DETECT_3_REG) & COMPARATOR_22_ENABLE_PARAM_MASK) >> 5;
    return value;
  }
...
unsigned char get_acc_detect21_comparator()
  {
     ...
     * 1: enable AccDetect21 comparator 
     */ 
    value = (SPIRead(ACC_DETECT_3_REG) & COMPARATOR_21_ENABLE_PARAM_MASK) >> 4;
    return value;
  }
...
unsigned char get_acc_detect_pullup()
  {
     ...
     * 1: enable AccDetect2 Pull-up 
     */ 
    value = (SPIRead(ACC_DETECT_3_REG) & ACC_DETECT_PULLUP_ENABLE_PARAM_MASK) >> 3;
    return value;
  }
...
unsigned char get_acc_detect_pullup_supply()
  {
     ...
     * 1: enable AccDetect2 Pull-up 2.1v supply 
     */ 
    value = (SPIRead(ACC_DETECT_3_REG) & V21_SUPPLY_ENABLE_PARAM_MASK) >> 1;
    return value;
  }
...
unsigned char get_acc_detect1_comparator()
  {
     ...
     * 1: enable AccDetect1 comparator 
     */ 
    value = (SPIRead(ACC_DETECT_3_REG) & DETECT_1_COMPARATOR_ENABLE_PARAM_MASK);
    return value;
  }
