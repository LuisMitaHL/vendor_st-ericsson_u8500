...
unsigned char set_arm_voltage_pwm_mode( enum forced param )
  {
    ...
    old_value = SPIRead(ARM_VOLTAGE_MANAGEMENT_REG);
    ...
    SPIWrite(ARM_VOLTAGE_MANAGEMENT_REG, value);
...
unsigned char get_arm_voltage_pwm_mode()
  {
     ...
     * 1: Varm is forced in PWM mode 
     */ 
    value = (SPIRead(ARM_VOLTAGE_MANAGEMENT_REG) & ARM_VOLTAGE_PWM_MODE_PARAM_MASK) >> 7;
    return value;
  }
...
unsigned char get_arm_voltage_auto_mode()
  {
     ...
     * 1: Varm is in AutoMode 
     */ 
    value = (SPIRead(ARM_VOLTAGE_MANAGEMENT_REG) & AUTO_MODE_ENABLE_PARAM_MASK) >> 6;
    return value;
  }
...
unsigned char get_vbbn_selection_control()
  {
     ...
     * 1: VBBN output voltage is defined by VBBNSel2 register 
     */ 
    value = (SPIRead(ARM_VOLTAGE_MANAGEMENT_REG) & VBBN_SELECTION_CONTROL_PARAM_MASK) >> 5;
    return value;
  }
...
unsigned char get_vbbp_selection_control()
  {
     ...
     * 1: VBBP output voltage is defined by VBBNSel2 register 
     */ 
    value = (SPIRead(ARM_VOLTAGE_MANAGEMENT_REG) & VBBP_SELECTION_CONTROL_PARAM_MASK) >> 4;
    return value;
  }
...
unsigned char get_arm_ouput_volage_select()
  {
     ...
     * 1x: Varm output voltage is defined by VarmSel3 register 
     */ 
    value = (SPIRead(ARM_VOLTAGE_MANAGEMENT_REG) & ARM_OUPUT_VOLAGE_SELECT_PARAM_MASK) >> 2;
    return value;
  }
...
unsigned char get_arm_ouput_regulation()
  {
     ...
     * 11: force enable in LP mode 
     */ 
    value = (SPIRead(ARM_VOLTAGE_MANAGEMENT_REG) & ARM_OUPUT_REGULATION_PARAM_MASK);
    return value;
  }
...
unsigned char get_force_no_step()
  {
     ...
     * is VbbpSelx[3:0] 
     */ 
    value = (SPIRead(ARM_VOLTAGE_MANAGEMENT_REG) & FORCE_NO_STEP_PARAM_MASK) >> 6;
    return value;
  }
...
unsigned char get_vbbp_pulldonw_enable()
  {
     ...
     * 1: enable pull-down (switch) between VBBN and gnd] 
     */ 
    value = (SPIRead(ARM_VOLTAGE_MANAGEMENT_REG) & VBBP_PULLDONW_ENABLE_PARAM_MASK) >> 5;
    return value;
  }
...
/***************************************************************************
 *
 * Function : set_switch_varm_and_vbbp
 *
 * IN   : param, a value to write to the regiter ArmVoltageManagement
 * OUT  : 
 *
 * RET  : Return the value of register ArmVoltageManagement
 *
 * Notes : From register 0x0401, bits 4:4
 *
 **************************************************************************/
unsigned char set_switch_varm_and_vbbp( enum switch_enable param )
  {
    ...
    /* 
     * 0: disable switch between VBBP and Varm 
     * 1: enable switch between VBBP and Varm 
     */ 
    switch( param )
      {
        case SWITCH_ENABLE_E: 
           value = old_value | SWITCH_ENABLE_PARAM_MASK; 
           break;
        case SWITCH_DISABLE_E: 
           value = old_value & ~ SWITCH_ENABLE_PARAM_MASK;
           break;
      }
...
/***************************************************************************
 *
 * Function : get_switch_varm_and_vbbp
 *
 * RET  : Return the value of register ArmVoltageManagement
 *
 * Notes : From register 0x0401, bits 4:4
...
unsigned char get_switch_varm_and_vbbp()
  {
     ...
     * 1: enable switch between VBBP and Varm 
     */ 
    value = (SPIRead(ARM_VOLTAGE_MANAGEMENT_REG) & SWITCH_ENABLE_PARAM_MASK) >> 4;
    return value;
  }
...
unsigned char get_vbbn_regulation()
  {
     ...
     * 11: force enable in LP mode 
     */ 
    value = (SPIRead(ARM_VOLTAGE_MANAGEMENT_REG) & VBBN_REGULATION_PARAM_MASK) >> 2;
    return value;
  }
...
unsigned char get_vbbp_regulation()
  {
     ...
     * 11: force enable in LP mode 
     */ 
    value = (SPIRead(ARM_VOLTAGE_MANAGEMENT_REG) & VBBP_REGULATION_PARAM_MASK);
    return value;
  }
...
unsigned char set_vape_pwm_mode_forced( enum forced param )
  {
    ...
    old_value = SPIRead(VAPE_VOLTAGE_MANAGEMENT_REG);
    ...
    SPIWrite(VAPE_VOLTAGE_MANAGEMENT_REG, value);
...
unsigned char get_vape_pwm_mode_forced()
  {
     ...
     * 1: Vape is forced in PWM mode 
     */ 
    value = (SPIRead(VAPE_VOLTAGE_MANAGEMENT_REG) & VAPE_PWM_MODE_FORCED_PARAM_MASK) >> 4;
    return value;
  }
...
unsigned char get_vape_auto_mode()
  {
     ...
     * 1: Vape is in AutoMode 
     */ 
    value = (SPIRead(VAPE_VOLTAGE_MANAGEMENT_REG) & VAPE_AUTO_MODE_PARAM_MASK) >> 3;
    return value;
  }
...
unsigned char get_vape_selection_control()
  {
     ...
     * 1: Vape output voltage is defined by VapeSel2 register 
     */ 
    value = (SPIRead(VAPE_VOLTAGE_MANAGEMENT_REG) & APE_SELECTION_PARAM_MASK) >> 2;
    return value;
  }
...
unsigned char get_vape_regulation()
  {
     ...
     * 11: force enable in LP mode 
     */ 
    value = (SPIRead(VAPE_VOLTAGE_MANAGEMENT_REG) & VAPE_REGULATION_PARAM_MASK);
    return value;
  }
...
unsigned char set_vsmps1_voltage_pwm_mode( enum forced param )
  {
    ...
    old_value = SPIRead(VSMPS_1_VOLTAGE_MANAGEMENT_REG);
    ...
    SPIWrite(VSMPS_1_VOLTAGE_MANAGEMENT_REG, value);
...
unsigned char get_vsmps1_voltage_pwm_mode()
  {
     ...
     * 1: Vsmps1 is forced in PWM mode 
     */ 
    value = (SPIRead(VSMPS_1_VOLTAGE_MANAGEMENT_REG) & VSMPS_1_VOLTAGE_PWM_MODE_PARAM_MASK) >> 5;
    return value;
  }
...
unsigned char get_vsmps1_auto_mode()
  {
     ...
     * 1: Vape is in AutoMode 
     */ 
    value = (SPIRead(VSMPS_1_VOLTAGE_MANAGEMENT_REG) & VSMPS_1_AUTO_MODE_PARAM_MASK) >> 4;
    return value;
  }
...
unsigned char get_vsmps1_ouput_volage_select()
  {
     ...
     * register 
     */ 
    value = (SPIRead(VSMPS_1_VOLTAGE_MANAGEMENT_REG) & VSMPS_1_OUPUT_VOLAGE_SELECT_PARAM_MASK) >> 2;
    return value;
  }
...
unsigned char get_vsmps1_regulation()
  {
     ...
     * 11: force enable in LP mode 
     */ 
    value = (SPIRead(VSMPS_1_VOLTAGE_MANAGEMENT_REG) & VSMPS_1_REGULATION_PARAM_MASK);
    return value;
  }
...
unsigned char set_vsmps2_voltage_pwm_mode( enum forced param )
  {
    ...
    old_value = SPIRead(VSMPS_2_VOLTAGE_MANAGEMENT_REG);
    ...
    SPIWrite(VSMPS_2_VOLTAGE_MANAGEMENT_REG, value);
...
unsigned char get_vsmps2_voltage_pwm_mode()
  {
     ...
     * 1: Vsmps2 is forced in PWM mode 
     */ 
    value = (SPIRead(VSMPS_2_VOLTAGE_MANAGEMENT_REG) & VSMPS_2_VOLTAGE_PWM_MODE_PARAM_MASK) >> 5;
    return value;
  }
...
unsigned char get_vsmps2_auto_mode()
  {
     ...
     * 1: Vape is in AutoMode 
     */ 
    value = (SPIRead(VSMPS_2_VOLTAGE_MANAGEMENT_REG) & AUTO_MODE_PARAM_MASK) >> 4;
    return value;
  }
...
unsigned char get_vsmps2_ouput_volage_select()
  {
     ...
     * register 
     */ 
    value = (SPIRead(VSMPS_2_VOLTAGE_MANAGEMENT_REG) & VSMPS_2_OUPUT_VOLAGE_SELECT_PARAM_MASK) >> 2;
    return value;
  }
...
unsigned char get_vsmps2_regulation()
  {
     ...
     * 11: force enable in LP mode 
     */ 
    value = (SPIRead(VSMPS_2_VOLTAGE_MANAGEMENT_REG) & VSMPS_2_REGULATION_PARAM_MASK);
    return value;
  }
...
unsigned char set_vsmps3_voltage_pwm_mode( enum forced param )
  {
    ...
    old_value = SPIRead(VSMPS_3_VOLTAGE_MANAGEMENT_REG);
    ...
    SPIWrite(VSMPS_3_VOLTAGE_MANAGEMENT_REG, value);
...
unsigned char get_vsmps3_voltage_pwm_mode()
  {
     ...
     * 1: Vsmps3 is forced in PWM mode 
     */ 
    value = (SPIRead(VSMPS_3_VOLTAGE_MANAGEMENT_REG) & VSMPS_3_VOLTAGE_PWM_MODE_PARAM_MASK) >> 5;
    return value;
  }
...
unsigned char get_vsmps3_auto_mode()
  {
     ...
     * 1: Vape is in AutoMode 
     */ 
    value = (SPIRead(VSMPS_3_VOLTAGE_MANAGEMENT_REG) & VSMPS_3_AUTO_MODE_PARAM_MASK) >> 4;
    return value;
  }
...
unsigned char get_vsmps3_ouput_volage_select()
  {
     ...
     * register 
     */ 
    value = (SPIRead(VSMPS_3_VOLTAGE_MANAGEMENT_REG) & VSMPS_3_OUPUT_VOLAGE_SELECT_PARAM_MASK) >> 2;
    return value;
  }
...
unsigned char get_vsmps3_regulation()
  {
     ...
     * 11: force enable in LP mode 
     */ 
    value = (SPIRead(VSMPS_3_VOLTAGE_MANAGEMENT_REG) & VSMPS_3_REGULATION_PARAM_MASK);
    return value;
  }
...
unsigned char set_vana_regulation( enum regulation param )
  {
    ...
    old_value = SPIRead(VANA_VOLTAGE_MANAGEMENT_REG);
    ...
    SPIWrite(VANA_VOLTAGE_MANAGEMENT_REG, value);
...
unsigned char get_vana_regulation()
  {
     ...
     * 11: force enable in LP mode 
     */ 
    value = (SPIRead(VANA_VOLTAGE_MANAGEMENT_REG) & VANA_REGULATION_PARAM_MASK) >> 2;
    return value;
  }
...
unsigned char get_vpll_regulation()
  {
     ...
     * 11: force enable in LP mode 
     */ 
    value = (SPIRead(VANA_VOLTAGE_MANAGEMENT_REG) & VPLL_REGULATION_PARAM_MASK);
    return value;
  }
...
unsigned char set_vref_ddr_sleep_mode( enum sleep_mode param )
  {
    ...
    old_value = SPIRead(VREF_DDR_VOLTAGE_MANAGEMENT_REG);
    ...
    SPIWrite(VREF_DDR_VOLTAGE_MANAGEMENT_REG, value);
...
unsigned char get_vref_ddr_sleep_mode()
  {
     ...
     * 1: enable 50KOhms pulldown 
     */ 
    value = (SPIRead(VREF_DDR_VOLTAGE_MANAGEMENT_REG) & SLEEP_MODE_PARAM_MASK) >> 1;
    return value;
  }
...
unsigned char get_vref_ddr_enable()
  {
     ...
     * 1: enable VrefDDR 
     */ 
    value = (SPIRead(VREF_DDR_VOLTAGE_MANAGEMENT_REG) & VREF_DDR_ENABLE_PARAM_MASK);
    return value;
  }
 ...
 *
 * IN   : param, a value to write to the regiter VextSuppply3VoltageManagement
 * OUT  : 
 *
 * RET  : Return the value of register VextSuppply3VoltageManagement
 *
...
unsigned char set_vext_suppply3_regulation( enum regulation param )
  {
    ...
    old_value = SPIRead(VEXT_SUPPPLY_3_VOLTAGE_MANAGEMENT_REG);
    ...
    SPIWrite(VEXT_SUPPPLY_3_VOLTAGE_MANAGEMENT_REG, value);
...
unsigned char get_vext_suppply3_regulation()
  {
     ...
     * 11: force enable in LP mode 
     */ 
    value = (SPIRead(VEXT_SUPPPLY_3_VOLTAGE_MANAGEMENT_REG) & VEXT_SUPPPLY_3_REGULATION_PARAM_MASK) >> 4;
    return value;
  }
 ...
 *
 * IN   : param, a value to write to the regiter VextSuppply2VoltageManagement
 * OUT  : 
 *
 * RET  : Return the value of register VextSuppply2VoltageManagement
 *
...
unsigned char set_vext_suppply2_regulation( enum regulation param )
  {
    ...
    old_value = SPIRead(VEXT_SUPPPLY_2_VOLTAGE_MANAGEMENT_REG);
    ...
    SPIWrite(VEXT_SUPPPLY_2_VOLTAGE_MANAGEMENT_REG, value);
...
unsigned char get_vext_suppply2_regulation()
  {
     ...
     * 11: force enable in LP mode 
     */ 
    value = (SPIRead(VEXT_SUPPPLY_2_VOLTAGE_MANAGEMENT_REG) & VEXT_SUPPPLY_2_REGULATION_PARAM_MASK) >> 2;
    return value;
  }
...
unsigned char set_vext_suppply1_regulation( enum regulation param )
  {
    ...
    old_value = SPIRead(VEXT_SUPPPLY_1_VOLTAGE_MANAGEMENT_REG);
    ...
    SPIWrite(VEXT_SUPPPLY_1_VOLTAGE_MANAGEMENT_REG, value);
...
unsigned char get_vext_suppply1_regulation()
  {
     ...
     * 11: force enable in LP mode 
     */ 
    value = (SPIRead(VEXT_SUPPPLY_1_VOLTAGE_MANAGEMENT_REG) & VEXT_SUPPPLY_1_REGULATION_PARAM_MASK);
    return value;
  }
...
unsigned char set_vaux2_regulation( enum regulation param )
  {
    ...
    old_value = SPIRead(VAUX_VOLTAGE_MANAGEMENT_REG);
    ...
    SPIWrite(VAUX_VOLTAGE_MANAGEMENT_REG, value);
...
unsigned char get_vaux2_regulation()
  {
     ...
     * 11: force enable in LP mode 
     */ 
    value = (SPIRead(VAUX_VOLTAGE_MANAGEMENT_REG) & VAUX_2_REGULATION_PARAM_MASK) >> 2;
    return value;
  }
...
unsigned char get_vaux1_regulation()
  {
     ...
     * 11: force enable in LP mode 
     */ 
    value = (SPIRead(VAUX_VOLTAGE_MANAGEMENT_REG) & VAUX_1_REGULATION_PARAM_MASK);
    return value;
  }
...
unsigned char set_vrf1_regulation( enum regulation param )
  {
    ...
    old_value = SPIRead(VAUX_3_VOLTAGE_MANAGEMENT_REG);
    ...
    SPIWrite(VAUX_3_VOLTAGE_MANAGEMENT_REG, value);
...
unsigned char get_vrf1_regulation()
  {
     ...
     * 11: force enable in LP mode 
     */ 
    value = (SPIRead(VAUX_3_VOLTAGE_MANAGEMENT_REG) & VRF_1_REGULATION_PARAM_MASK) >> 2;
    return value;
  }
...
unsigned char get_vaux3_regulation()
  {
     ...
     * 11: force enable in LP mode 
     */ 
    value = (SPIRead(VAUX_3_VOLTAGE_MANAGEMENT_REG) & VAUX_3_REGULATION_PARAM_MASK);
    return value;
  }
...
unsigned char set_varm_output_sel1( enum output_supply param )
  {
    ...
    old_value = SPIRead(VARM_OUTPUT_SEL_1_REG);
    ...
    value =  old_value & ~OUTPUT_SUPPLY_PARAM_MASK;
     ...
     value |=  param ;  
    SPIWrite(VARM_OUTPUT_SEL_1_REG, value);
...
unsigned char get_varm_output_sel1()
  {
     ...
     * 12.5mV step and from 1.8 to 1.875 by 12.5 mV step 
     */ 
    value = (SPIRead(VARM_OUTPUT_SEL_1_REG) & OUTPUT_SUPPLY_PARAM_MASK);
    return value;
  }
...
unsigned char set_varm_output_sel2( enum output_supply param )
  {
    ...
    old_value = SPIRead(VARM_OUTPUT_SEL_2_REG);
    ...
    value =  old_value & ~OUTPUT_SUPPLY_PARAM_MASK;
     ...
     value |=  param ;  
    SPIWrite(VARM_OUTPUT_SEL_2_REG, value);
...
unsigned char get_varm_output_sel2()
  {
     ...
     * 12.5mV step and from 1.8 to 1.875 by 12.5 mV step 
     */ 
    value = (SPIRead(VARM_OUTPUT_SEL_2_REG) & OUTPUT_SUPPLY_PARAM_MASK);
    return value;
  }
...
unsigned char set_varm_output_sel3( enum output_supply param )
  {
    ...
    old_value = SPIRead(VARM_OUTPUT_SEL_3_REG);
    ...
    value =  old_value & ~OUTPUT_SUPPLY_PARAM_MASK;
     ...
     value |=  param ;  
    SPIWrite(VARM_OUTPUT_SEL_3_REG, value);
...
unsigned char get_varm_output_sel3()
  {
     ...
     * 12.5mV step and from 1.8 to 1.875 by 12.5 mV step 
     */ 
    value = (SPIRead(VARM_OUTPUT_SEL_3_REG) & OUTPUT_SUPPLY_PARAM_MASK);
    return value;
  }
...
unsigned char set_vape_output_sel1( enum output_supply param )
  {
    ...
    old_value = SPIRead(VAPE_OUTPUT_SEL_1_REG);
    ...
    value =  old_value & ~OUTPUT_SUPPLY_PARAM_MASK;
     ...
     value |=  param ;  
    SPIWrite(VAPE_OUTPUT_SEL_1_REG, value);
...
unsigned char get_vape_output_sel1()
  {
     ...
     * 12.5mV step and from 1.8 to 1.875 by 12.5 mV step 
     */ 
    value = (SPIRead(VAPE_OUTPUT_SEL_1_REG) & OUTPUT_SUPPLY_PARAM_MASK);
    return value;
  }
...
unsigned char set_vape_output_sel2( enum output_supply param )
  {
    ...
    old_value = SPIRead(VAPE_OUTPUT_SEL_2_REG);
    ...
    value =  old_value & ~OUTPUT_SUPPLY_PARAM_MASK;
     ...
     value |=  param ;  
    SPIWrite(VAPE_OUTPUT_SEL_2_REG, value);
...
unsigned char get_vape_output_sel2()
  {
     ...
     * 12.5mV step and from 1.8 to 1.875 by 12.5 mV step 
     */ 
    value = (SPIRead(VAPE_OUTPUT_SEL_2_REG) & OUTPUT_SUPPLY_PARAM_MASK);
    return value;
  }
...
unsigned char set_vape_output_sel3( enum output_supply param )
  {
    ...
    old_value = SPIRead(VAPE_OUTPUT_SEL_3_REG);
    ...
    value =  old_value & ~OUTPUT_SUPPLY_PARAM_MASK;
     ...
     value |=  param ;  
    SPIWrite(VAPE_OUTPUT_SEL_3_REG, value);
...
unsigned char get_vape_output_sel3()
  {
     ...
     * 12.5mV step and from 1.8 to 1.875 by 12.5 mV step 
     */ 
    value = (SPIRead(VAPE_OUTPUT_SEL_3_REG) & OUTPUT_SUPPLY_PARAM_MASK);
    return value;
  }
...
unsigned char set_vbb_positive_selection1( enum positivie_voltage_selection param )
  {
    ...
    old_value = SPIRead(VBB_OUTPUT_SUPPLY_1_REG);
    ...
    switch(  param )
      {
        ...
        case VBBPFB_04V_E:
           ...
           break;
        case VBBPFB_05V_E:
            value =  value | VBBPFB_05V ;
           break;
        case VBBPFB_MINUS_05V_E:
            value =  value | VBBPFB_MINUS_05V ;
           break;
    ...
    SPIWrite(VBB_OUTPUT_SUPPLY_1_REG, value);
...
unsigned char get_vbb_positive_selection1()
  {
     ...
     * VBBP output supply value management 
     */ 
    value = (SPIRead(VBB_OUTPUT_SUPPLY_1_REG) & VBB_POSITIVE_SELECTION_1_PARAM_MASK);
    return value;
  }
    ...
    switch(  param )
      {
        ...
        case VBBN_MINUS_04V_E:
           ...
           break;
        case VBBN_MINUS_05V_E:
            value  = value  | (VBBN_MINUS_05V << 0x4);
           break;
...
unsigned char get_vbb_negative_selection1()
  {
     ...
     * VBBN output supply value management 
     */ 
    value = (SPIRead(VBB_OUTPUT_SUPPLY_1_REG) & VBB_NEGATIVE_SELECTION_1_PARAM_MASK) >> 4;
    return value;
  }
...
unsigned char set_vbb_positive_selection2( enum positivie_voltage_selection param )
  {
    ...
    old_value = SPIRead(VBB_OUTPUT_SUPPLY_2_REG);
    ...
    SPIWrite(VBB_OUTPUT_SUPPLY_2_REG, value);
...
unsigned char get_vbb_positive_selection2()
  {
     ...
     * VBBP output supply value management 
     */ 
    value = (SPIRead(VBB_OUTPUT_SUPPLY_2_REG) & VBB_POSITIVE_SELECTION_2_PARAM_MASK);
    return value;
  }
...
unsigned char get_vbb_negative_selection2()
  {
     ...
     * VBBN output supply value management 
     */ 
    value = (SPIRead(VBB_OUTPUT_SUPPLY_2_REG) & VBB_NEGATIVE_SELECTION_2_PARAM_MASK) >> 4;
    return value;
  }
...
unsigned char set_vsmps1_sel1_output( enum output_supply param )
  {
    ...
    old_value = SPIRead(VSMPS_1_SEL_1_OUTPUT_REG);
    ...
    value =  old_value & ~VSMPS_1_SEL_1_OUTPUT_PARAM_MASK;
     ...
     value |=  param ;  
    SPIWrite(VSMPS_1_SEL_1_OUTPUT_REG, value);
...
unsigned char get_vsmps1_sel1_output()
  {
     ...
     * 12.5mV step and from 1.8 to 1.875 by 12.5 mV step 
     */ 
    value = (SPIRead(VSMPS_1_SEL_1_OUTPUT_REG) & VSMPS_1_SEL_1_OUTPUT_PARAM_MASK);
    return value;
  }
...
unsigned char set_vsmps1_sel2_output( enum output_supply param )
  {
    ...
    old_value = SPIRead(VSMPS_1_SEL_2_OUTPUT_REG);
    ...
    value =  old_value & ~VSMPS_1_SEL_2_OUTPUT_PARAM_MASK;
     ...
     value |=  param ;  
    SPIWrite(VSMPS_1_SEL_2_OUTPUT_REG, value);
...
unsigned char get_vsmps1_sel2_output()
  {
     ...
     * 12.5mV step and from 1.8 to 1.875 by 12.5 mV step 
     */ 
    value = (SPIRead(VSMPS_1_SEL_2_OUTPUT_REG) & VSMPS_1_SEL_2_OUTPUT_PARAM_MASK);
    return value;
  }
...
unsigned char set_vsmps1_sel3_output( enum output_supply param )
  {
    ...
    old_value = SPIRead(VSMPS_1_SEL_3_OUTPUT_REG);
    ...
    value =  old_value & ~VSMPS_1_SEL_3_OUTPUT_PARAM_MASK;
     ...
     value |=  param ;  
    SPIWrite(VSMPS_1_SEL_3_OUTPUT_REG, value);
...
unsigned char get_vsmps1_sel3_output()
  {
     ...
     * 12.5mV step and from 1.8 to 1.875 by 12.5 mV step 
     */ 
    value = (SPIRead(VSMPS_1_SEL_3_OUTPUT_REG) & VSMPS_1_SEL_3_OUTPUT_PARAM_MASK);
    return value;
  }
...
unsigned char set_vsmps2_sel1_output( enum output_supply param )
  {
    ...
    old_value = SPIRead(VSMPS_2_SEL_1_OUTPUT_REG);

    /* 
     * Defined Vsmps2Sel1 output voltage from 0.7V to 1.3625V by 
     * 12.5mV step and from 1.8 to 1.875 by 12.5 mV step 
     */ 
    ...
    value =  old_value & ~VSMPS_2_SEL_1_OUTPUT_PARAM_MASK;
     ...
     value |=  param ;  
    SPIWrite(VSMPS_2_SEL_1_OUTPUT_REG, value);
...
unsigned char get_vsmps2_sel1_output()
  {
     ...
     * 12.5mV step and from 1.8 to 1.875 by 12.5 mV step 
     */ 
    value = (SPIRead(VSMPS_2_SEL_1_OUTPUT_REG) & VSMPS_2_SEL_1_OUTPUT_PARAM_MASK);
    return value;
  }
...
unsigned char set_vsmps2_sel2_output( enum output_supply param )
  {
    ...
    old_value = SPIRead(VSMPS_2_SEL_2_OUTPUT_REG);

    /* 
     * Defined Vsmps2Sel2 output voltage from 0.7V to 1.3625V by 
     * 12.5mV step and from 1.8 to 1.875 by 12.5 mV step 
     */ 
    ...
    value =  old_value & ~VSMPS_2_SEL_2_OUTPUT_PARAM_MASK;
     ...
     value |=  param ;  
    SPIWrite(VSMPS_2_SEL_2_OUTPUT_REG, value);
...
unsigned char get_vsmps2_sel2_output()
  {
     ...
     * 12.5mV step and from 1.8 to 1.875 by 12.5 mV step 
     */ 
    value = (SPIRead(VSMPS_2_SEL_2_OUTPUT_REG) & VSMPS_2_SEL_2_OUTPUT_PARAM_MASK);
    return value;
  }
...
unsigned char set_vsmps2_sel3_output( enum output_supply param )
  {
    ...
    old_value = SPIRead(VSMPS_2_SEL_3_OUTPUT_REG);

    /* 
     * Defined Vsmps2Sel3 output voltage from 0.7V to 1.3625V by 
     * 12.5mV step and from 1.8 to 1.875 by 12.5 mV step 
     */ 
    ...
    value =  old_value & ~VSMPS_2_SEL_3_OUTPUT_PARAM_MASK;
     ...
     value |=  param ;  
    SPIWrite(VSMPS_2_SEL_3_OUTPUT_REG, value);
...
unsigned char get_vsmps2_sel3_output()
  {
     ...
     * 12.5mV step and from 1.8 to 1.875 by 12.5 mV step 
     */ 
    value = (SPIRead(VSMPS_2_SEL_3_OUTPUT_REG) & VSMPS_2_SEL_3_OUTPUT_PARAM_MASK);
    return value;
  }
...
unsigned char set_vsmps3_sel1_output( enum output_supply2 param )
  {
    ...
    old_value = SPIRead(VSMPS_3_SEL_1_OUTPUT_REG);

    /* 
     * Defined Vsmps3 output voltage from 0.7V to 1.875V by 
     * 12.5mV steps 
     */ 
    ...
    value =  old_value & ~VSMPS_3_SEL_1_OUTPUT_PARAM_MASK;
     ...
     value |=  param ;  
    SPIWrite(VSMPS_3_SEL_1_OUTPUT_REG, value);
...
unsigned char get_vsmps3_sel1_output()
  {
     ...
     * 12.5mV steps 
     */ 
    value = (SPIRead(VSMPS_3_SEL_1_OUTPUT_REG) & VSMPS_3_SEL_1_OUTPUT_PARAM_MASK);
    return value;
  }
...
unsigned char set_vsmps3_sel2_output( enum output_supply2 param )
  {
    ...
    old_value = SPIRead(VSMPS_3_SEL_2_OUTPUT_REG);
    ...
    value =  old_value & ~VSMPS_3_SEL_2_OUTPUT_PARAM_MASK;
     ...
     value |=  param ;  
    SPIWrite(VSMPS_3_SEL_2_OUTPUT_REG, value);
...
unsigned char get_vsmps3_sel2_output()
  {
     ...
     * 12.5mV steps 
     */ 
    value = (SPIRead(VSMPS_3_SEL_2_OUTPUT_REG) & VSMPS_3_SEL_2_OUTPUT_PARAM_MASK);
    return value;
  }
...
unsigned char set_vsmps3_sel3_output( enum output_supply2 param )
  {
    ...
    old_value = SPIRead(VSMPS_3_SEL_3_OUTPUT_REG);
    ...
    value =  old_value & ~VSMPS_3_SEL_3_OUTPUT_PARAM_MASK;
     ...
     value |=  param ;  
    SPIWrite(VSMPS_3_SEL_3_OUTPUT_REG, value);
...
unsigned char get_vsmps3_sel3_output()
  {
     ...
     * 12.5mV steps 
     */ 
    value = (SPIRead(VSMPS_3_SEL_3_OUTPUT_REG) & VSMPS_3_SEL_3_OUTPUT_PARAM_MASK);
    return value;
  }
...
unsigned char set_vaux1_sel_output( enum vaux_output param )
  {
    ...
    old_value = SPIRead(VAUX_1_SEL_1_OUTPUT_REG);
    ...
    switch(  param )
      {
        case VAUX_1100MV_E:
            value =  value | VAUX_1100MV ;
           break;
        case VAUX_1200MV_E:
            value =  value | VAUX_1200MV ;
           break;
        case VAUX_1300MV_E:
            value =  value | VAUX_1300MV ;
           break;
        case VAUX_1400MV_E:
            value =  value | VAUX_1400MV ;
           break;
        case VAUX_1500MV_E:
        ...
        case VAUX_1800MV_E:
           ...
           break;
        case VAUX_1850MV_E:
            value =  value | VAUX_1850MV ;
           break;
        case VAUX_1900MV_E:
            value =  value | VAUX_1900MV ;
           break;
        case VAUX_2500MV_E:
            value =  value | VAUX_2500MV ;
           break;
        case VAUX_2650MV_E:
            value =  value | VAUX_2650MV ;
           break;
        case VAUX_2700MV_E:
            value =  value | VAUX_2700MV ;
           break;
        case VAUX_2750MV_E:
            value =  value | VAUX_2750MV ;
           break;
        case VAUX_2800MV_E:
            value =  value | VAUX_2800MV ;
           break;
        case VAUX_2900MV_E:
            value =  value | VAUX_2900MV ;
           break;
        case VAUX_3000MV_E:
            value =  value | VAUX_3000MV ;
           break;
        case VAUX_3300MV_E:
            value =  value | VAUX_3300MV ;
           break;
      }
  

    SPIWrite(VAUX_1_SEL_1_OUTPUT_REG, value);
...
unsigned char get_vaux1_sel_output()
  {
     ...
     * Vaux1 output supply value management 
     */ 
    value = (SPIRead(VAUX_1_SEL_1_OUTPUT_REG) & VAUX_OUTPUT_PARAM_MASK);
    return value;
  }
...
unsigned char set_vaux2_sel_output( enum vaux_output param )
  {
    ...
    old_value = SPIRead(VAUX_2_SEL_1_OUTPUT_REG);
    ...
    SPIWrite(VAUX_2_SEL_1_OUTPUT_REG, value);
...
unsigned char get_vaux2_sel_output()
  {
     ...
     * Vaux2 output supply value management 
     */ 
    value = (SPIRead(VAUX_2_SEL_1_OUTPUT_REG) & VAUX_OUTPUT_PARAM_MASK);
    return value;
  }
 ...
 *
 * Notes : From register 0x0421, bits 3:0
 *
 **************************************************************************/
unsigned char set_vaux3_sel_output( enum vaux_output param )
  {
    ...
    old_value = SPIRead(VAUX_3_SEL_1_OUTPUT_REG);
    ...
    SPIWrite(VAUX_3_SEL_1_OUTPUT_REG, value);
...
unsigned char get_vaux3_sel_output()
  {
     ...
     * Vaux2 output supply value management 
     */ 
    value = (SPIRead(VAUX_3_SEL_1_OUTPUT_REG) & VAUX_OUTPUT_PARAM_MASK);
    return value;
  }
...
unsigned char get_vrf1_sel_output()
  {
     ...
     * Vrf1 output supply value management 
     */ 
    value = (SPIRead(VAUX_3_SEL_1_OUTPUT_REG) & VRF_1_OUTPUT_PARAM_MASK) >> 4;
    return value;
  }
...
unsigned char set_ext_supply_force_low( enum ext_supply_force_low param )
  {
    ...
    old_value = SPIRead(EXTERNAL_SUPPLY_CONTROL_REG);
    ...
    SPIWrite(EXTERNAL_SUPPLY_CONTROL_REG, value);
...
unsigned char get_ext_supply_force_low()
  {
     ...
     * ExtSupply12LPn) 
     */ 
    value = (SPIRead(EXTERNAL_SUPPLY_CONTROL_REG) & EXT_SUPPLY_FORCE_LOW_PARAM_MASK);
    return value;
  }
...
unsigned char set_vmod_resquet_control( enum vmod_resquet_control param )
  {
    ...
    old_value = SPIRead(VMOD_SUPPLY_MANAGEMENT_REG);
    ...
    SPIWrite(VMOD_SUPPLY_MANAGEMENT_REG, value);
...
unsigned char get_vmod_resquet_control()
  {
     ...
     * 1x: force Vmod in HP whatever VmodRequest signal value 
     */ 
    value = (SPIRead(VMOD_SUPPLY_MANAGEMENT_REG) & VMOD_RESQUET_CONTROL_PARAM_MASK) >> 6;
    return value;
  }
...
unsigned char set_mod_sys_clk_req1_valid( enum modem_system_clock_request_valid param )
  {
    ...
    /* 
     * 0: Modem doesn't valid system clock request from 
     * SysClkReq1input 
     * 1: Modem valid system clock request from SysClkReq1input 
     */ 
    switch( param )
      {
...
unsigned char get_mod_sys_clk_req1_valid()
  {
     ...
     * 1: Modem valid system clock request from SysClkReq1input 
     */ 
    value = (SPIRead(VMOD_SUPPLY_MANAGEMENT_REG) & MODEM_SYSTEM_CLOCK_REQUEST_VALID_PARAM_MASK) >> 5;
    return value;
  }
...
unsigned char get_vmod_p_w_m_mode()
  {
     ...
     * programm to a lower value (energy sent to the battery 
     */ 
    value = (SPIRead(VMOD_SUPPLY_MANAGEMENT_REG) & VMOD_FORCED_ENABLE_PARAM_MASK) >> 4;
    return value;
  }
...
unsigned char get_vmod_auto_mode()
  {
     ...
     * 1: Vmod is in AutoMode 
     */ 
    value = (SPIRead(VMOD_SUPPLY_MANAGEMENT_REG) & VMOD_AUTO_MODE_ENABLE_PARAM_MASK) >> 3;
    return value;
  }
...
unsigned char get_vmod_selection_control()
  {
     ...
     * 1: Vmod output voltage is defined by VmodSel2 register 
     */ 
    value = (SPIRead(VMOD_SUPPLY_MANAGEMENT_REG) & VMOD_SELECTION_PARAM_MASK) >> 2;
    return value;
  }
...
unsigned char get_vmod_regulation()
  {
     ...
     * 11: force enable in LP mode 
     */ 
    value = (SPIRead(VMOD_SUPPLY_MANAGEMENT_REG) & VMOD_REGULATION_PARAM_MASK);
    return value;
  }
...
unsigned char set_vmod_sel1_output( enum output_supply param )
  {
    ...
    old_value = SPIRead(VMOD_SEL_1_OUTPUT_REG);
    ...
    value =  old_value & ~VMOD_SEL_1_OUTPUT_PARAM_MASK;
     ...
     value |=  param ;  
    SPIWrite(VMOD_SEL_1_OUTPUT_REG, value);
...
unsigned char get_vmod_sel1_output()
  {
     ...
     * 12.5mV step and from 1.8 to 1.875 by 12.5 mV step 
     */ 
    value = (SPIRead(VMOD_SEL_1_OUTPUT_REG) & VMOD_SEL_1_OUTPUT_PARAM_MASK);
    return value;
  }
...
unsigned char set_vmod_sel2_output( enum output_supply param )
  {
    ...
    old_value = SPIRead(VMOD_SEL_2_OUTPUT_REG);
    ...
    value =  old_value & ~VMOD_SEL_2_OUTPUT_PARAM_MASK;
     ...
     value |=  param ;  
    SPIWrite(VMOD_SEL_2_OUTPUT_REG, value);
...
unsigned char get_vmod_sel2_output()
  {
     ...
     * 12.5mV step and from 1.8 to 1.875 by 12.5 mV step 
     */ 
    value = (SPIRead(VMOD_SEL_2_OUTPUT_REG) & VMOD_SEL_2_OUTPUT_PARAM_MASK);
    return value;
  }
...
unsigned char set_vaudio_discharge_time( enum supply_discharge_time param )
  {
    ...
    old_value = SPIRead(SUPPLY_DISCHARGE_TIME_REG);
    ...
    SPIWrite(SUPPLY_DISCHARGE_TIME_REG, value);
...
unsigned char get_vaudio_discharge_time()
  {
     ...
     * 1: long discharge time when disabled 
     */ 
    value = (SPIRead(SUPPLY_DISCHARGE_TIME_REG) & VAUDIO_DISCHARGE_TIME_PARAM_MASK) >> 7;
    return value;
  }
...
unsigned char get_vtv_out_discharge_time()
  {
     ...
     * 1: long discharge time when disabled 
     */ 
    value = (SPIRead(SUPPLY_DISCHARGE_TIME_REG) & VTV_OUT_DISCHARGE_TIME_PARAM_MASK) >> 6;
    return value;
  }
...
unsigned char get_vint_core12_discharge_time()
  {
     ...
     * 1: long discharge time when disabled 
     */ 
    value = (SPIRead(SUPPLY_DISCHARGE_TIME_REG) & VINT_CORE_12_DISCHARGE_TIME_PARAM_MASK) >> 5;
    return value;
  }
...
unsigned char get_vaux3_discharge_time()
  {
     ...
     * 1: long discharge time when disabled 
     */ 
    value = (SPIRead(SUPPLY_DISCHARGE_TIME_REG) & VAUX_3_DISCHARGE_TIME_PARAM_MASK) >> 4;
    return value;
  }
...
unsigned char get_vaux2_discharge_time()
  {
     ...
     * 1: long discharge time when disabled 
     */ 
    value = (SPIRead(SUPPLY_DISCHARGE_TIME_REG) & VAUX_2_DISCHARGE_TIME_PARAM_MASK) >> 3;
    return value;
  }
...
unsigned char get_vaux1_discharge_time()
  {
     ...
     * 1: long discharge time when disabled 
     */ 
    value = (SPIRead(SUPPLY_DISCHARGE_TIME_REG) & VAUX_1_DISCHARGE_TIME_PARAM_MASK) >> 2;
    return value;
  }
...
unsigned char get_vrf1_discharge_time()
  {
     ...
     * 1: long discharge time when disabled 
     */ 
    value = (SPIRead(SUPPLY_DISCHARGE_TIME_REG) & VRF_1_DISCHARGE_TIME_PARAM_MASK) >> 1;
    return value;
  }
...
unsigned char get_vpll_discharge_time()
  {
     ...
     * 1: long discharge time when disabled 
     */ 
    value = (SPIRead(SUPPLY_DISCHARGE_TIME_REG) & VPLL_DISCHARGE_TIME_PARAM_MASK);
    return value;
  }
...
unsigned char set_vana_discharge_time( enum supply_discharge_time param )
  {
    ...
    old_value = SPIRead(SUPPLY_DISCHARGE_TIME_2_REG);
    ...
    SPIWrite(SUPPLY_DISCHARGE_TIME_2_REG, value);
...
unsigned char get_vana_discharge_time()
  {
     ...
     * 1: long discharge time when disabled 
     */ 
    value = (SPIRead(SUPPLY_DISCHARGE_TIME_2_REG) & VANA_DISCHARGE_TIME_PARAM_MASK) >> 1;
    return value;
  }
...
unsigned char get_vsim_discharge_time()
  {
     ...
     * 1: long discharge time when disabled 
     */ 
    value = (SPIRead(SUPPLY_DISCHARGE_TIME_2_REG) & VSIM_DISCHARGE_TIME_PARAM_MASK);
    return value;
  }
