...
unsigned char get_coulomb_counter_mux_offset()
  {
     ...
     * 1: select internal average offset 
     */ 
    value = (SPIRead(COULOMB_COUNTER_CONTROL_REG) & SELECT_PARAM_MASK) >> 7;
    return value;
  }
...
unsigned char get_coulomb_counter_number_of_convertion()
  {
     ...
     * 10: 16 
     */ 
    value = (SPIRead(COULOMB_COUNTER_CONTROL_REG) & NUMBER_CONVERTION_PARAM_MASK) >> 5;
    return value;
  }
...
unsigned char get_coulomb_counter_offset_computation()
  {
     ...
     * 1: enable offset average computation 
     */ 
    value = (SPIRead(COULOMB_COUNTER_CONTROL_REG) & COMPUTATION_ENABLE_PARAM_MASK) >> 4;
    return value;
  }
...
unsigned char get_coulomb_counter_calibration_mode()
  {
     ...
     * 1: set ADconverter in calibration mode 
     */ 
    value = (SPIRead(COULOMB_COUNTER_CONTROL_REG) & CALIBRATION_ENABLE_PARAM_MASK) >> 3;
    return value;
  }
...
unsigned char get_coulomb_counter_reset_accumlator()
  {
     ...
     * counters when a reading is done.r 
     */ 
    value = (SPIRead(COULOMB_COUNTER_CONTROL_REG) & RESET_ACCUMLATOR_ENABLE_PARAM_MASK) >> 1;
    return value;
  }
...
unsigned char get_coulomb_counter_read_enable()
  {
     ...
     * registers 
     */ 
    value = (SPIRead(COULOMB_COUNTER_CONTROL_REG) & READ_ENABLE_PARAM_MASK);
    return value;
  }
...
unsigned char set_coulomb_counter_low( unsigned char param )
  {
    ...
    value = SPIRead(COULOMB_COUNTER_LOW_REG);

    /* 
     * Coulomb counter accumulator value (low). 
     */ 
    SPIWrite(COULOMB_COUNTER_LOW_REG, param);
    return value;
...
unsigned char set_coulomb_counter_mid( unsigned char param )
  {
    ...
    value = SPIRead(COULOMB_COUNTER_MID_REG);

    /* 
     * Coulomb counter accumulator value (High). 
     */ 
    SPIWrite(COULOMB_COUNTER_MID_REG, param);
    return value;
...
unsigned char set_coulomb_sample_counter_average_offset( unsigned char param )
  {
    ...
    value = SPIRead(COULOMB_SAMPLE_COUNTER_AVERAGE_OFFSET_REG);

    /* 
     * Coulomb counter average offset 
     */ 
    SPIWrite(COULOMB_SAMPLE_COUNTER_AVERAGE_OFFSET_REG, param);
    return value;
...
unsigned char set_coulomb_sample_counter_offset( unsigned char param )
  {
    ...
    value = SPIRead(COULOMB_SAMPLE_COUNTER_OFFSET_REG);

    /* 
     * Coulomb counter offset 
     */ 
    SPIWrite(COULOMB_SAMPLE_COUNTER_OFFSET_REG, param);
    return value;
...
unsigned char set_coulomb_sample_number_conversion( unsigned char param )
  {
    ...
    value = SPIRead(COULOMB_SAMPLE_NUMBER_CONVERSION_REG);

    /* 
     * Defined number of sample conversion to be accumulated in 
     * 21bits accumulator 
     */ 
    SPIWrite(COULOMB_SAMPLE_NUMBER_CONVERSION_REG, param);
    return value;
...
unsigned char get_comlomb_data_transfert()
  {
     ...
     * registers 
     */ 
    value = (SPIRead(COULOMB_DATA_MANAGEMENT_REG) & DATA_TRANSFERT_PARAM_MASK) >> 1;
    return value;
  }
...
unsigned char get_comlomb_reset_bit_data()
  {
     ...
     * allows to reset the 21bits accumulator data 
     */ 
    value = (SPIRead(COULOMB_DATA_MANAGEMENT_REG) & BIT_DATA_RESET_PARAM_MASK);
    return value;
  }
...
unsigned char set_coulomb_sample_number_conversion_low( unsigned char param )
  {
    ...
    value = SPIRead(COULOMB_SAMPLE_NUMBER_CONVERSION_LOW_REG);

    /* 
     * Sample number conversion low part 
     */ 
    SPIWrite(COULOMB_SAMPLE_NUMBER_CONVERSION_LOW_REG, param);
    return value;
...
unsigned char set_coulomb_sample_number_conversion_mid( unsigned char param )
  {
    ...
    value = SPIRead(COULOMB_SAMPLE_NUMBER_CONVERSION_MID_REG);

    /* 
     * Sample number conversion middle Part 
     */ 
    SPIWrite(COULOMB_SAMPLE_NUMBER_CONVERSION_MID_REG, param);
    return value;
...
unsigned char set_coulomb_sample_number_conversion_high( unsigned char param )
  {
    ...
    value = SPIRead(COULOMB_SAMPLE_NUMBER_CONVERSION_HIGH_REG);

    /* 
     * Sample number conversion high part 
     */ 
    SPIWrite(COULOMB_SAMPLE_NUMBER_CONVERSION_HIGH_REG, param);
    return value;
