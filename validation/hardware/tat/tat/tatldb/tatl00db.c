...
char * filename = "/var/local/tat/tatldb/DB8500_V2.xml";
...
int dth_init_service()
{
	...
	printf("DB8500 : Counted %d elements\n", result);
	p_BF_elements = dth_element_alloc2(result);
	printf("DB8500 : Allocated %i elements\n", result);

	ptr = p_BF_elements;
	/* Version element */
	strcpy((char*)ptr->path, "/DTH/DB/VERSION/Version");
	ptr->type = DTH_TYPE_STRING;
	ptr->enumString = NULL;
	ptr->info = NULL;
	ptr->get = get_version;

	printf("Will explore %s\n", filename);
	v_NbParams = dth_xml_init_service(p_BF_elements, filename, (ptf_get)DthDB_get, (ptf_set)DthDB_set, (ptf_exec)DthDB_exec);
	printf("DB8500 : Created %d elements\n\n", v_NbParams);	
	printf("DB8500 : Saved %d elements\n\n", result);
    	if(v_NbParams > 0)
    	{
		/*printf("So there are %d elements to register\n\n", result);*/
        	...
        	{
			/*if (strncmp(ptr->path, "/DTH/DB/config", 14) == 0)
...
{
	int result = 0;
        result = tatl1_00ActDB_exec(elem) ;
	return result;
}
...
{
	int result = 0;
        result = tatl1_02ActDB_Set(elem,value) ;
	return result;
}
...
{
	int result = 0;
        result = tatl1_03ActDB_Get(elem,value);
	return result;
}
...
struct dth_element * dth_element_alloc2(int nb_element) 
{
    ...
    for(i=0; i<nb_element; i++)
    {
        ...
        p_elements->enumString = calloc(256,sizeof(char));
