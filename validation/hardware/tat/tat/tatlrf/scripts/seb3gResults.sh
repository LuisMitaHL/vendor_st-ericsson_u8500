...
#	Prints HATS RF Single BER 3G results for one or all pages.
...
{
	echo "$0 [page]"
	echo "page: a number within 1-5. If the argument is missing then all pages"
	echo "are printed."
}
...
then
	...
	then
		# unitary
		page=$1
		page_result $DTH9P/RF/BER/3G/SLOT$page
	else
		# global
		for slot in `ls $DTH9P/RF/BER/3G/SLOT* -d`
		do
			page_result $slot
		done
	fi
