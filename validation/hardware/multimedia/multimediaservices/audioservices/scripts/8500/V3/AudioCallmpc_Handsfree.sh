...
echo AudioCallmpc_Handsfree.sh $1
...
do /usr/local/MMTE_scripts/cscall_mpc_avsourcesink.ate 0 2 2 5000 8000 multimic 0xFFFF handsfree
ENDCAT
mmte_bellagio 0</tmp/mmte_pipe 1>/dev/null 2>&1 &
cat /tmp/StartCSCallBg.ate
cat /tmp/StartCSCallBg.ate >/tmp/mmte_pipe
#DA3 DA4 Slots configuration
amixer -q cset iface=MIXER,name='Digital Interface DA 3 From Slot Map' 'SLOT10'
amixer -q cset iface=MIXER,name='Digital Interface DA 4 From Slot Map' 'SLOT11'
sleep 2
;;
