...
echo AudioCallmpc_Headset.sh $1
...
do /usr/local/MMTE_scripts/cscall_mpc_avsourcesink.ate 0 2 2 5000 8000 multimic 0xFFFF headset
ENDCAT
mmte_bellagio 0</tmp/mmte_pipe 1>/dev/null 2>&1 &
cat /tmp/StartCSCallBg.ate
cat /tmp/StartCSCallBg.ate >/tmp/mmte_pipe
#DA1 DA2 Slots configuration
amixer -q cset iface=MIXER,name='Digital Interface DA 1 From Slot Map' 'SLOT8'
amixer -q cset iface=MIXER,name='Digital Interface DA 2 From Slot Map' 'SLOT9'
sleep 2
;;
