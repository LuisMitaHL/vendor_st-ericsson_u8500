amixer cset iface=MIXER,name='Headset Left' Disabled
amixer cset iface=MIXER,name='Headset Right' Disabled
amixer cset iface=MIXER,name='Earpiece' Disabled
amixer cset iface=MIXER,name='IHF Left' Disabled
amixer cset iface=MIXER,name='IHF Right' Disabled
amixer cset iface=MIXER,name='Vibra' Disabled
amixer cset iface=MIXER,name='USB Headset Left' Disabled
amixer cset iface=MIXER,name='USB Headset Right' Disabled

amixer cset iface=MIXER,name='Digital Interface DA 1 From Slot Map' 'SLOT9'
amixer cset iface=MIXER,name='Digital Interface DA 2 From Slot Map' 'SLOT11'


