...
*/
typedef enum{
	NORMAL_PLAYBACK_MODE  /**< In this mode ARM will send decoded audio packets to AB audio codec and audio codec 30 ms FIFO will not be used. 
	...
	,DIGITAL_LOOPBACK_MODE    /**< This mode provides digital loopback in AP. PCM data from MSP1 interface will be looped back to AB8500 device. */
	...
	,AB_DIGITAL_LOOPBACK_MODE    /**< This mode provides digital loopback in AB. PCM data from audio codec interface IF0 are looped back to DAC interface. */
...
typedef enum{
	...
	,CODEC_DEST_EARPIECE  		/**< 	to select handset organ */
	,CODEC_DEST_HANDSFREE	 /**< to select hansfree mode */
	,CODEC_DEST_VIBRATOR1  	/**< to select vibrator 1 */
	,CODEC_DEST_FM_TX  /**< to select FM_TX */
	,CODEC_DEST_LINE_OUT	/**< to select Car Kit Organ */
	,CODEC_DEST_PDM	/**< to select Digital output organ*/
	,CODEC_DEST_USB_HS	/**< to select USB Headset organ */
	,CODEC_DEST_USB_CK	/**< to select USB Carkit organ */
