...
int main(int argc, char **argv)
{
  ...
  int device;
	...
	//log=fopen("/tmp/trace_mmte.txt","w");


	param=(char*)malloc(strlen(argv[3])+1);
	memset (param,0,strlen(argv[3])+1);
	strncpy(param,argv[3],strlen(argv[3]));
	...
	sprintf(FileName,"/tmp/trace_mmte%s.txt",param);
	...
	//fflush(stdout);
     ...
     //write_checked(fd_output_MMTE_task_pipe,"Command executed           ",27);
     write_checked(fd_output_MMTE_task_pipe,"Command executed                  ",34);
     fclose(log);
     fclose(console);
     free(chaine_read);
