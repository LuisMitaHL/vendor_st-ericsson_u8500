...
#define dispatchVoiceRadioTech dispatchVoid
...
static void *dispatchDial(void *data, size_t datalen)
{
    ...
    if (ret->uusInfo) {
        RIL_UUS_Info *uus_info_p = calloc(1,sizeof(RIL_UUS_Info));
        if (uus_info_p == NULL) {
            goto finally;
        }
        char *uus_data_p = calloc(1,ret->uusInfo->uusLength);
        if (uus_data_p == NULL) {
            free(uus_info_p);
            goto finally;
        }

        uus_info_p->uusLength = ret->uusInfo->uusLength;
        uus_info_p->uusDcs = ret->uusInfo->uusDcs;
        uus_info_p->uusType = ret->uusInfo->uusType;
        memmove(uus_data_p, ret->uusInfo->uusData,ret->uusInfo->uusLength);
        uus_info_p->uusData = uus_data_p;
        ret->uusInfo = uus_info_p;
    }
  finally:
    return ret;
}
...
static void *dispatchDataCall(void *data, size_t datalen)
{
  return dispatchStrings(data, datalen);
}
