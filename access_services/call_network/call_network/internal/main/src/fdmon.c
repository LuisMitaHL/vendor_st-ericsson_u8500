...
pthread_mutex_t select_cb_mutex = PTHREAD_MUTEX_INITIALIZER;
cn_bool_t select_cb_finished = FALSE;
#endif
                ...
                if (FD_ISSET(element->fd, &readfds)) {
                    if (element->cb_event) {
                        if (element->cb_event(element->fd, element->context) < 0) {
                            ...
                            CN_LOG_E("element->cb_event() failed for element->fd = %d", element->fd);
...
#ifdef ENABLE_MODULE_TEST
            /* Inform the test thread that the callback(s) have been invoked */
            (void)pthread_mutex_lock(&select_cb_mutex);
            select_cb_finished = TRUE;
            ...
            if (errno == EINTR) {
                ...
                for (element = list_anchor; element != NULL; element = next_element) {
                    ...
                    if (fcntl(element->fd, F_GETFL, 0) < 0) {
                        if (errno == EBADF) {
                            CN_LOG_E("fd = %d is invalid, removing", element->fd);
                            fdmon_del(element->fd);
                        }
                    }
                }
                ...
                errnum = errno;
                CN_LOG_E("errno(%d)=\"%s\"", errnum, strerror(errnum));
                goto error_return;
            }
        } else {
...
int fdmon_add(const int fd, const void *context, fdmon_callback_t *cb_event, fdmon_callback_t *cb_remove)
{
    ...
    if (0 > fd || FD_SETSIZE <= fd) {
        CN_LOG_E("out-of-range descriptor fd = %d, rejected!", fd);
        goto error_return;
    }

#ifndef ENABLE_MODULE_TEST
    if (fcntl(fd, F_GETFL, 0) < 0 && errno == EBADF) {
        CN_LOG_E("invalid descriptor fd = %d, rejected!", fd);
        goto error_return;
    }
    ...
    if (new_element) {
        ...
        CN_LOG_D("registering fd = %d, new_element = %p", fd, new_element);
...
int fdmon_del(int fd)
{
    ...
    CN_LOG_D("unregistering fd = %d", fd);
