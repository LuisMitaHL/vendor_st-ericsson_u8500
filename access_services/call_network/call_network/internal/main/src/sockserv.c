...
int sockserv_init(const int max_servers)
{
    ...
    if (max_servers < 1 || max_servers > SOCKSERV_MAX_SERVERS) {
        CN_LOG_E("max_servers=%d is out of range (1-%d)!", max_servers, SOCKSERV_MAX_SERVERS);
        goto error_return;
    }
...
int sockserv_create(const char *name, const int max_clients, sockserv_callback_t *cb_func)
{
    ...
    if (!name || strlen(name) < 1) {
        CN_LOG_E("no socket name supplied!");
        goto error_return;
    }

    if (max_clients < 1 || max_clients > SOCKSERV_MAX_CLIENTS) {
        CN_LOG_E("max_clients = %d is out of range (1-%d)!", max_clients, SOCKSERV_MAX_CLIENTS);
        goto error_return;
    }

    if (!cb_func) {
        CN_LOG_E("no callback function supplied!!");
        goto error_return;
    }
    ...
    if (instance_p) {
        ...
        CN_LOG_E("unable to allocate new instance=%d!", instance_no);
        goto error_return;
    }
    ...
    if (sd < 0) {
        errnum = errno;
        CN_LOG_E("socket() returns error(%d)=\"%s\"", errnum, strerror(errnum));
        goto cleanup_return;
    }
    ...
    if (bind(sd, (struct sockaddr *)&uaddr, sizeof(uaddr)) < 0) {
        errnum = errno;
        CN_LOG_E("bind() returns error(%d)=\"%s\"", errnum, strerror(errnum));
        goto cleanup_return;
    }
    ...
    if (listen(sd, max_clients) < 0) {
        errnum = errno;
        CN_LOG_E("listen() returns error(%d)=\"%s\"", errnum, strerror(errnum));
        goto cleanup_return;
    }
    ...
    if (flags < 0 || fcntl(sd, F_SETFL, flags | O_NONBLOCK) < 0) {
        errnum = errno;
        CN_LOG_E("fcntl() returns error(%d)=\"%s\"", errnum, strerror(errnum));
        goto cleanup_return;
    }
...
int sockserv_cb_accept(const int sd, const void *context)
{
    ...
    if (!instance_p) {
        CN_LOG_E("invalid context %p!", context);
        goto error_return;
    }
    ...
    if (instance_p->cb_func(instance_no, -1, (void *)NULL,
        ...
        CN_LOG_D("instance=%d, client connect request rejected.", instance_no);
        /* Reject connection */
        goto error_return;
    }
    ...
    if (!client_p) {
        CN_LOG_E("unable to allocate new client for instance=%d!", instance_no);
        goto error_return;
    }
    ...
    if (csd < 0) {
        errnum = errno;
        CN_LOG_E("accept() returns error(%d)=\"%s\"", errnum, strerror(errnum));
        goto cleanup_return;
    }
...
int sockserv_cb_recv(const int sd, const void *context)
{
    ...
    if (ioctl(sd, SIOCINQ, &unread) < 0) {
        /* Error handling */
        errnum = errno;
        CN_LOG_E("ioctl(%d) returns error(%d)=\"%s\".", sd, errnum, strerror(errnum));
        goto error_return;
    }
    ...
    if ((client_p->used + unread) > client_p->size) {
        ...
        if (buf) {
            ...
            CN_LOG_E("buffer (re-)allocation failure!");
            goto error_return;
        }
    }
    ...
    do {
        ...
        if (rc > 0) {
             ...
             * messsages in the buffer, if any.
             */
            ...
            if (rc > 0) {
                ...
                CN_LOG_E("Unrecoverable error occurred, aborting");
                break;
            } else {
        ...
        /* recv() Error handling */
            ...
            if (errno == EINTR) {
                ...
                errnum = errno;
                CN_LOG_E("recv(%d) returns error(%d)=\"%s\".", sd, errnum, strerror(errnum));
                break;
            }
        } else {                /* recv() rc == 0 */
            /* Orderly shutdown by peer, shut down this end */
            CN_LOG_D("Orderly shutdown by peer");
            break;
        }
    } while (unread > 0);
...
int sockserv_send(const int instance_no, const int client_no, const void *buf, const size_t length)
{
    ...
    if (length < 1) {
        CN_LOG_E("Buffer length < 1");
        goto error_return;
    }

    if (!client_p) {
        CN_LOG_E("client_p is NULL!");
        goto error_return;
    }
    ...
    do {
        ...
        if (rc > 0) {
            ...
            CN_LOG_E("send(%d) socket error: ",
                     client_p->sd, strerror(errnum));
            break;
        }
    } while (to_send > 0);
...
int sockserv_close(const int instance_no, const int client_no)
{
    ...
    if (!instance_p) {
        CN_LOG_E("invalid instance_no = %d!", instance_no);
        goto error_return;
    }
    ...
    if (!client_p) {
        CN_LOG_E("invalid client_no = %d!", client_no);
        goto error_return;
    }
...
int sockserv_cb_close(const int sd, const void *context)
{
    ...
    res = sockserv_close(instance_no, client_no);
...
int sockserv_destroy(const int instance_no)
{
    ...
    CN_LOG_D("Closing down sockserv instance: %d", instance_no);
...
{
    sockserv_instance_t *instance_p = NULL;
    int instance_no = 0;
    int result = -1;
...
{
    sockserv_instance_t *result = NULL;


    if (instance_no >= 0 && instance_no < instance_max) {
        result = instances[instance_no];
    }
...
int sockserv_free_instance(int instance_no)
{
    ...
    if (instance_p) {
        if (instance_p->clients_used > 0) {
            CN_LOG_E("cannot free instance=%d (%p), clients connected", instance_no, instance_p);
            goto exit;
        }
...
int sockserv_alloc_client(int instance_no)
{
    ...
    if (!instance_p->clients) {
        ...
        if (!instance_p->clients) {
            CN_LOG_E("unable to allocate client array for instance=%d!", instance_no);
            goto exit;
        }
    }
...
{
    sockserv_instance_t *instance_p = sockserv_get_instance(instance_no);
    sockserv_client_t *result = NULL;


    if (instance_p) {
        if (client_no >= 0 && client_no < instance_p->client_max) {
            result = instance_p->clients[client_no];
        }
    }
...
int sockserv_free_client(int instance_no, int client_no)
{
    ...
    if (!instance_p) {
        CN_LOG_E("instance_p is NULL!");
        goto exit;
    }

    if (!instance_p->clients) {
        CN_LOG_E("instance_p->clients is NULL!");
        goto exit;
    }

    if (client_no < 0 || client_no >= instance_p->client_max) {
        CN_LOG_E("client_no out of range! (client_no=%d)", client_no);
        goto exit;
    }
