...
static int uiccd_encode_and_send_smsc_restore_from_record_result(int fd,
                                                                 ...
                                                                 sim_uicc_status_word_t status_word)
{
    ...
    if (buf_p == NULL){
        catd_log_f(SIM_LOGGING_E, "uicc : uiccd_encode_and_send_smsc_restore_from_record_result, memory allocation failed");
        return -1;
    }
    p = buf_p;
    ...
    if (buf_len != (size_t)(p-buf_p)) {
        catd_log_f(SIM_LOGGING_E, "uicc: uiccd_encode_and_send_smsc_restore_from_record_result, assert failure");
    }
...
/**
 * @brief                  Handler function for smsc restore from record responses.
 ...
 * Transaction states:
 *
 * 1. UICCD_TRANS_SMSC_READ_RECORD. The client provided record ID was used for retireving the record that is going
 *                                  to be set as the active one. If the read was successful, record 1 is updated with
 *                                  the raw record data of the one that was read.
 * 2. UICCD_TRANS_SMSC_UPDATE_RECORD. The result of the update operation is sent to the client.
 */
static int uiccd_smsc_restore_from_record_transaction_handler(ste_msg_t * ste_msg)
{
    ...
    switch (data_p->trans_state)
    {
        case UICCD_TRANS_SMSC_READ_RECORD:
        {
            ...
            if (rsp_p->uicc_status_code != SIM_UICC_STATUS_CODE_OK) {
                if (uiccd_encode_and_send_smsc_restore_from_record_result(ctrl_p->fd,
                                                                          ctrl_p->client_tag,
                                                                          rsp_p->uicc_status_code,
                                                                          rsp_p->uicc_status_code_fail_details,
                                                                          rsp_p->status_word) != 0) {
                    catd_log_f(SIM_LOGGING_E, "uicc : uiccd_encode_and_send_smsc_restore_from_record_result failed");
                }
                free(data_p);
                free(ctrl_p);
                break;
            ...
            // Update the default record with the record data identified by the client provided record_id
            rv = uiccd_update_smsp_record(ctrl_p, 1, rsp_p->data, rsp_p->len);
       ...
       default:
       {
            catd_log_f(SIM_LOGGING_E, "uicc : wrong transaction state for restore from record = %d", data_p->trans_state);
            free(data_p);
            free(ctrl_p);
       }
       break;
...
/**
 * @brief                  Main function for smsc restore from record
 * @param ste_msg          The original message from client.
 * @return                 Status of the operation. Success: 0; Failed: -1
 *
 * This function starts the transaction handler that is used for restoring the SMSC parameters found in a specific record in the SMSP file
 * to a record 1 (the active one) in the same file.
 *
...
int uiccd_main_smsc_restore_from_record(ste_msg_t * ste_msg)
{
    ...
    if ( !p ) {
        if (uiccd_encode_and_send_smsc_restore_from_record_result(msg->fd,
                                                                       msg->client_tag,
                                                                       SIM_UICC_STATUS_CODE_FAIL,
                                                                       SIM_UICC_STATUS_CODE_FAIL_DETAILS_INTERNAL_ERROR,
                                                                       status_word) != 0) {
        ...
        catd_log_f(SIM_LOGGING_E, "uicc : uiccd_main_smsc_restore_from_record failed");
        return -1;
    }
    ...
    ctrl_p = ste_sim_create_ctrl_block(msg->client_tag, msg->fd, msg->type, uiccd_smsc_restore_from_record_transaction_handler, data_p);

    if (!ctrl_p) {
        catd_log_f(SIM_LOGGING_E, "uicc : create ctrl block for smsc restore from record transaction failed");
        if (uiccd_encode_and_send_smsc_restore_from_record_result(msg->fd,
                                                                  msg->client_tag,
                                                                  SIM_UICC_STATUS_CODE_FAIL,
                                                                  SIM_UICC_STATUS_CODE_FAIL_DETAILS_INTERNAL_ERROR,
    ...
    // Read out the record_id given by the client so it can be restored to the default record
    result = uiccd_read_one_smsp_record(ctrl_p, data_p->client_record_id, 0, SMSP_FILE_ID);
