...
static int uiccd_encode_and_send_pin_verify_response(int fd,
                                                     ...
                                                     sim_uicc_status_word_t status_word)
{
    ...
    if (buf_p == NULL){
        catd_log_f(SIM_LOGGING_E, "uicc : uiccd_encode_and_send_pin_verify_response, memory allocation failed");
        return -1;
    }
    p = buf_p;
...
int uiccd_main_pin_verify(const ste_msg_t * ste_msg)
{
    ...
    catd_log_f(SIM_LOGGING_I, "uicc : PIN verify message received from %d ", msg->fd);
    m = catd_get_modem();
    if (!m) {
        catd_log_f(SIM_LOGGING_E, "uicc : No modem");
        uiccd_encode_and_send_pin_verify_response(msg->fd,
                                                  msg->client_tag,
...
int uiccd_main_pin_verify_response(ste_msg_t * ste_msg)
{
    ...
    msg = (uiccd_msg_pin_verify_response_t*)ste_msg;

    ctrl_p = (ste_sim_ctrl_block_t*)(msg->client_tag);

    catd_log_f(SIM_LOGGING_I, "uicc : Pin verify response message received");
    ...
    if (ctrl_p->transaction_data_p) {
        ...
        catd_log_f(SIM_LOGGING_I, "uicc : pin verify response: pin_info_p->pin_id = %d", pin_info_p->pin_id);
        //check the status code and pin_id
        if (pin_info_p->pin_id != SIM_UICC_PIN_ID_PIN2 &&
                SIM_UICC_STATUS_CODE_OK == msg->uicc_status_code) {
            catd_log_f(SIM_LOGGING_I, "uicc : cache pin.");
    ...
    buf = malloc(buf_len);
    if (!buf) {
      catd_log_f(SIM_LOGGING_E, "uicc : Could not allocate memory in uiccd_main_pin_verify_response");
    ...
    if (buf_len != (size_t)(p-buf)) {
        catd_log_f(SIM_LOGGING_E, "uiccd_main_pin_verify_response, assert failure");
    }

    assert( buf_len == (size_t)(p-buf) );

    sim_send_generic(ctrl_p->fd,
                     STE_UICC_RSP_PIN_VERIFY, buf, buf_len, ctrl_p->client_tag);
