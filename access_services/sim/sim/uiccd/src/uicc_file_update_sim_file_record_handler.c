...
static int uiccd_encode_and_send_update_sim_file_record_response(int fd,
                                                                 ...
                                                                 size_t  data_len)
{
    ...
    if (buf_p == NULL){
        catd_log_f(SIM_LOGGING_E, "uicc : uiccd_encode_and_send_update_sim_file_record_response, memory allocation failed");
        return -1;
    }
    p = buf_p;
...
int uiccd_main_update_sim_file_record(ste_msg_t * ste_msg)
{
    ...
    msg = (uiccd_msg_update_sim_file_record_t *) ste_msg;

    catd_log_f(SIM_LOGGING_I, "uicc : UPDATE EF record message received");

    m = catd_get_modem();
    if (!m) {
        catd_log_f(SIM_LOGGING_E, "uicc : No modem");
        uiccd_encode_and_send_update_sim_file_record_response(msg->fd,
                                                              msg->client_tag,
        ...
        if (!data) {
            uiccd_encode_and_send_update_sim_file_record_response(msg->fd,
                                                                  ...
                                                                  NULL,
                                                                  0);
            catd_log_f(SIM_LOGGING_E, "uicc : uiccd_main_update_sim_file_record, memory allocation failed");
            return -1;
        }
        ...
        i = ste_modem_file_update_record(m,
                                         (uintptr_t)ctrl_p,
                                         uicc_get_app_id(),
                                         file_id,
                                         record_id,
        ...
        if (i != 0) {
            uiccd_encode_and_send_update_sim_file_record_response(msg->fd,
                                                                  ...
                                                                  NULL,
                                                                  0);
            catd_log_f(SIM_LOGGING_E, "uicc : uiccd_sig_update_sim_file_record failed");
            free(data);
            free(ctrl_p);
            free(file_path);
            return -1;
...
int uiccd_main_update_sim_file_record_response(ste_msg_t * ste_msg)
{
    ...
    msg = (uiccd_msg_update_sim_file_record_response_t*)ste_msg;

    catd_log_f(SIM_LOGGING_I, "uicc : EF Record update response message received");
    ...
    buf = malloc(buf_len);
    if (buf == NULL){
        catd_log_f(SIM_LOGGING_E, "uicc : EF Record update response, memory allocatin failed");
        return -1;
    }
    p = buf;
    ...
    if (buf_len != (size_t)(p-buf)) {
        catd_log_f(SIM_LOGGING_E, "uicc: uiccd_main_update_sim_file_record_response, assert failure");
    }
