...
TEEC_UUID SHA_AES_uuid = TA_SHA_AES_UUID;
...
int getSessionCommandInfo(enum TestTypes tt, SessionCommandInfo *sci)
{
    ...
    case SHA_256: {
        ...
        sci->commandID = TA_SHA_AES_CMD_SHA256;
        ...
        for (i = 0; i < numOperationInfos; i++) {
            ...
            /* For this TA, memRef[1] is output */
    ...
    case AES256ECB_ENCRYPT: {
        sci->uuid = SHA_AES_uuid;
        sci->commandID = TA_SHA_AES_CMD_AES256ECB_ENC;
    ...
    case AES256ECB_DECRYPT: {
        sci->uuid = SHA_AES_uuid;
        sci->commandID = TA_SHA_AES_CMD_AES256ECB_DEC;
...
int cleanUpSessionCommandInfo(enum TestTypes tt, SessionCommandInfo *sci)
{
    ...
    case AES256ECB_DECRYPT:
        for (i = 0; i < numOperationInfos; i++) {
            for (j = 0; j < 4; j++) {
                /*
                 * This functions only cleans output buffers, since
                 * it uses static buffers for the input buffers in
                 * this test code.
                 */
...
int compareResult(enum TestTypes tt, OperationInfo *opInfo)
{
    ...
    bufsize = MAX(opInfo->operation.memRefs[i].size,
                  opInfo->referenceDataLen);

    if (memcmp(opInfo->operation.memRefs[i].buffer,
    ...
    dump_buf("Got",
             (uint8_t *) opInfo->operation.memRefs[i].buffer, bufsize);
#endif
