...
for (my $i = 0; $i < length($file); $i++) {
	...
	my $j = 0;
	for (; $j < $column; $j++) {
		my $hex_val = "0x" . sprintf("%02X,", ord(substr($file, $i+$j, 1)));
	...
	foreach my $byte (@ascii) {
		# Special handling since :print: actually print a newline in windows.
		if ($byte ne '*' and $byte =~ m/[[:print:]]/ and ord($byte) > 0x1F) {
			print $byte;
		} else {
			print ".";
		}
